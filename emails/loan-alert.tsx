...
> = {
  disbursed: {
    ...
    chip: "Disbursed",
    tone: "positive",
    amountLabel: "Amount disbursed",
  },
  due: {
    title: "Loan repayment due soon",
    lead: (n) =>
      `Hi ${n}, a friendly reminder that your next instalment is coming up.`,
    chip: "Due soon",
    tone: "pending",
    amountLabel: "Instalment due",
  },
  overdue: {
    title: "Loan repayment overdue",
    lead: (n) =>
      `Hi ${n}, your instalment is past due. Please pay as soon as you can to avoid penalties.`,
    chip: "Overdue",
    tone: "negative",
...
export default function LoanAlert({
  ...
  return (
    <EmailLayout preview={`${copy.title} — ${KES(amount)}`}>
      ...
      {dueDate && (
        <InfoRow
          label={kind === "overdue" ? "Was due" : "Due date"}
          value={dueDate}
        />
      )}

      {kind !== "disbursed" && payUrl && (
        <CtaButton href={payUrl}>Pay now via M-Pesa</CtaButton>
      )}
