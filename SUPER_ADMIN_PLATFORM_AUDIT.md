...
- Link the existing `credit-scores.service.ts` score onto the member profile page, and fix the "Organization Growth"/"Top Organizations" mislabeling (both are actually group-scoped).
- Wire the audit-log page's already-supported `groupId` filter, and add organization/IP-address display (IP already exists as a column, just not rendered).
- Either wire the Risk Center's action buttons to real mutations against `governance_alerts`' existing ack/resolve workflow (once Phase 2 exists) or remove them until then — a button that doesn't do anything is worse than no button.
- Add breadcrumbs to `(admin)` pages using the already-built `PageHeader` component instead of the hand-rolled back-arrow.

### Phase 1 — Member & group drill-down completion
- Build a real admin-side member detail page (profile + savings/loan/shares/welfare + attendance + the existing credit score), reachable from both `admin/users` and a new member table on `admin/groups/[id]` (which currently has none).
- Extend route nesting to a real 3-level drill-down: Organizations → {org} → Groups → {group} → Members → {member}.

### Phase 2 — Activate the governance engine (highest leverage in this report)
- Build the computation job (mirroring the `lib/jobs` pattern already used throughout this codebase) that populates `governance_snapshots` from the 19 already-seeded metrics, evaluates the already-seeded threshold bands, writes `governance_health_scores`, and raises `governance_alerts`.
- Replace the dead `groups.risk_score`/`engagement_score` display with the real computed health score.
- Rebuild the Risk Center's heatmap and alert feed on top of real `governance_alerts` rows instead of the current mix of real-but-narrow and always-zero signals.
