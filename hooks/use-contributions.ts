...
export function useSetSavingsPolicy() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: SetSavingsLimitsPayload) =>
      contributionsApi.setPolicy(body),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: [...contributionKeys.all, "policy"] }),
  });
}
