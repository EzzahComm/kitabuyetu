...
import type {
  welfareService,
  CreateWelfareRequestPayload,
  ReviewWelfareRequestPayload,
  RecordWelfarePoolPayload,
...
export type WelfarePoolSummary = Awaited<
  ReturnType<typeof welfareService.getPoolSummary>
>;
...
export const welfareKeys = {
  ...
  list: (p?: Record<string, unknown>) =>
    [...welfareKeys.all, "list", p] as const,
...
export function useWelfareRequests(
  ...
  return useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResult<WelfareRequestRow>>(
...
export function useCreateWelfareRequest() {
  ...
  return useMutation({
    mutationFn: (body: CreateWelfareRequestPayload) =>
      api.post<WelfareRequestRow>(BASE, body),
...
export function useReviewWelfareRequest(id: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: ReviewWelfareRequestPayload) =>
      api.patch<WelfareRequestRow>(`${BASE}/${id}`, body),
    onSuccess: () => {
...
export function useRecordWelfarePoolContribution() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: RecordWelfarePoolPayload) =>
      api.post<unknown>(`${BASE}/pool`, body),
