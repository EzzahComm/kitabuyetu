...
export type InvestmentSummary = Awaited<
  ReturnType<typeof investmentsService.getSummary>
>;
...
export const investmentKeys = {
  ...
  list: (p?: Record<string, unknown>) =>
    [...investmentKeys.all, "list", p] as const,
...
export function useInvestments(params?: Record<string, unknown>) {
  return useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResult<InvestmentRow>>(
...
export function useCreateInvestment() {
  ...
  return useMutation({
    mutationFn: (body: CreateInvestmentPayload) =>
      api.post<InvestmentRow>(BASE, body),
    onSuccess: () => {
...
export function useUpdateInvestment(id: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: UpdateInvestmentPayload) =>
      api.patch<InvestmentRow>(`${BASE}/${id}`, body),
    onSuccess: () => {
...
export function useRecordInvestmentReturn(investmentId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: RecordReturnPayload) =>
      api.post<unknown>(`${BASE}/${investmentId}/returns`, body),
    onSuccess: () => {
...
export function useRecordInvestmentExpense(investmentId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: RecordExpensePayload) =>
      api.post<unknown>(`${BASE}/${investmentId}/expenses`, body),
