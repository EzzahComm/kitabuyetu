...
export const meetingKeys = {
  ...
  list: (p?: Record<string, unknown>) =>
    [...meetingKeys.all, "list", p] as const,
...
export function useMeetings(params?: Record<string, unknown>) {
  return useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResult<MeetingRow>>(
...
export function useUpdateMeeting(id: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: UpdateMeetingPayload) =>
      api.patch<MeetingRow>(`${BASE}/${id}`, body),
    onSuccess: () => {
...
export function useRecordAttendance(meetingId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: RecordAttendancePayload) =>
      api.post<MeetingRow>(`${BASE}/${meetingId}/attendance`, body),
    onSuccess: () => {
...
export function useAddResolution(meetingId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: AddResolutionPayload) =>
      api.post<unknown>(`${BASE}/${meetingId}/resolutions`, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: meetingKeys.detail(meetingId) });
      qc.invalidateQueries({ queryKey: meetingKeys.lists() });
      qc.invalidateQueries({ queryKey: meetingKeys.stats });
...
export function useUpdateResolution(meetingId: string) {
  ...
  return useMutation({
    mutationFn: ({
      resolutionId,
      ...body
    }: UpdateResolutionPayload & { resolutionId: string }) =>
      api.patch<unknown>(
        `${BASE}/${meetingId}/resolutions/${resolutionId}`,
        body,
      ),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: meetingKeys.detail(meetingId) });
