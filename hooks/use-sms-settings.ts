...
export function useUpdateSmsSettings() {
  ...
  return useMutation({
    mutationFn: (body: SmsGroupSettingsUpdateInput) =>
      smsApi.updateSettings(body),
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: smsSettingsKeys.settings }),
  });
}
...
export function useBirthdays() {
  return useQuery({
    queryKey: smsSettingsKeys.birthdays,
    queryFn: smsApi.birthdays,
  });
}
