...
export const meKeys = {
  ...
  passbook: (params?: Partial<MemberPassbookQueryInput>) =>
    [...meKeys.all, "passbook", params] as const,
  notifications: (params?: { page?: number; limit?: number }) =>
    [...meKeys.all, "notifications", params] as const,
...
export function useCreateGoal() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: Parameters<typeof meApi.goals.create>[0]) =>
      meApi.goals.create(body),
...
export function useLogGoalProgress(id: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (body: LogGoalProgressInput) =>
      meApi.goals.logProgress(id, body),
    onSuccess: () => qc.invalidateQueries({ queryKey: meKeys.goals() }),
  });
}
