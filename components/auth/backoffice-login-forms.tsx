...
const THEME = {
  ...
  light: {
    ...
    input:
      "bg-white border-brand-blue-900/15 text-brand-blue-900 placeholder:text-brand-blue-900/30",
    error: "text-red-600",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-1.5">
        ...
        />
        {errors.email && (
          <p className={cn("text-xs", t.error)}>{errors.email.message}</p>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        ...
        {errors.password && (
          <p className={cn("text-xs", t.error)}>{errors.password.message}</p>
        )}
      </div>
      ...
      <p className="text-center text-sm">
        <Link
          ...
          className={cn(t.link, "hover:underline")}
        >
          Forgot password?
        </Link>
      </p>
...
export function EnrollForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="space-y-1.5">
        <Label className={cn("text-xs uppercase", t.labelUpper)}>
      ...
      <div className={cn("space-y-2 rounded-md border p-3", t.warningBox)}>
        <div className="flex items-center justify-between">
          <p className={cn("text-xs font-semibold uppercase", t.warningText)}>
        ...
        <p className={cn("text-xs", t.warningSub)}>
          Single-use. Stored hashed; we can&apos;t show them again. Lose your
          phone, use a code.
        </p>
        <div
          className={cn(
            "grid grid-cols-2 gap-1 font-mono text-xs",
            t.recoveryText,
          )}
        >
      ...
      <div className="space-y-1.5">
        ...
        />
        {errors.code && (
          <p className={cn("text-xs", t.error)}>{errors.code.message}</p>
        )}
      </div>
      ...
      <div className="flex gap-2">
        <Button
          ...
          variant="ghost"
          onClick={onBack}
          className={cn("flex-1", t.buttonGhost)}
        >
          Back
        </Button>
        <Button
  ...
  return (
    <div className="space-y-2">
      {organizations.map((o) => (
        ...
        >
          <p className={cn("text-sm font-semibold", t.orgTitle)}>
            {o.organizationName}
          </p>
          <p className={cn("mt-0.5 text-xs capitalize", t.orgSub)}>
            {o.orgRole}
          </p>
        </button>
      ))}
      <button
        type="button"
        onClick={onBack}
        className={cn("mt-2 text-sm", t.link)}
      >
        ← Back to sign in
      </button>
    </div>
