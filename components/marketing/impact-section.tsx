    ...
    <Section tone="paper-deep" labelledBy="impact-heading">
      <Container>
        <RevealedHeading
          ...
          lede={
            <>
              Real figures, pulled from the same ledger the platform runs on —
              published here the moment there is a track record worth reporting.
              See{" "}
              <Link
          ...
          {IMPACT_STATS.map((stat, i) => (
            <Reveal key={stat.label} delay={i * 60}>
              <div className="rounded-lg border border-brand-blue-900/10 bg-white px-3 py-6 text-center">
                <p className="font-display text-3xl font-light text-brand-blue-900">
                  {stat.value}
                </p>
