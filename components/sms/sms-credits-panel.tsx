...
const featureColumns: PaginatedTableColumn<FeatureUsage & { id: string }>[] = [
  {
    ...
    render: (f) => (
      <span
        className={
          f.feature === null ? "text-muted-foreground" : "text-foreground"
        }
      >
  ...
  },
  {
    key: "messages",
    header: "Messages",
    render: (f) => f.messages.toLocaleString(),
  },
  {
    key: "credits",
    header: "Credits",
    render: (f) => Math.round(f.credits).toLocaleString(),
  },
];
    ...
    <div className="space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              ...
              {data && data.allowanceRemaining > 0 && (
                <>
                  {" "}
                  · {data.allowanceRemaining.toLocaleString()} included in your
                  plan
                  {data.purchasedBalance > 0 && (
                    <>
                      {" "}
                      + {Math.floor(
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard
          ...
          hint={
            data
              ? `${Math.round(data.usageLastMonth).toLocaleString()} last month`
              : undefined
          }
        />
        <StatCard
        ...
        <StatCard
          ...
          hint={
            data?.daysRemaining == null ? "Not enough activity yet" : undefined
          }
        />
      </div>
