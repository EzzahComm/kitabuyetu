...
export function StkPromptDialog({
  open,
  onClose,
  member,
}: StkPromptDialogProps) {
  ...
  function onPaymentCompleted() {
    ...
    toast({
      ...
      description: "Balances and records updated.",
    });
  }
    ...
    for (let i = 0; i < POLL_ATTEMPTS; i++) {
      ...
      try {
        ...
        if (res.status === "failed") {
          setStep("failed");
          setFailReason(
            "The prompt was cancelled, timed out, or the payment failed on M-Pesa.",
          );
          return;
        }
      } catch {
  ...
  async function send() {
    ...
    setFailReason(null);
    try {
      const res = await api.post<{ checkoutRequestId?: string }>(
        "/mpesa/stk-push",
        {
          phone,
          amount: amt,
      ...
      toast({
        ...
        description: "Ask the member to enter their M-Pesa PIN.",
      });
      if (res.checkoutRequestId) void pollStatus(res.checkoutRequestId);
    } catch (err) {
      setStep("failed");
      setFailReason(
        err instanceof Error ? err.message : "The STK push could not be sent.",
      );
    }
  }
          ...
          <div className="space-y-4">
            {member ? (
              <p className="text-sm text-muted-foreground">
                ...
                ({member.phone}) to pay a contribution.
              </p>
            ) : (
              <div className="space-y-1">
                ...
                {!loadingMembers && pickable.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    No active members with a phone number found.
                  </p>
                )}
              </div>
            )}
            ...
            <p className="text-xs text-muted-foreground">
              ...
              · Ref: CONTRIB. The payment posts to savings and the ledger
          ...
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <div>
              <p className="text-sm font-medium">
                {step === "sending"
                  ? "Sending STK prompt…"
                  : `Waiting for ${payerName ?? phone} to enter their PIN…`}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                ...
                {reference && (
                  <>
                    {" "}
                    · Ref{" "}
                    <span className="font-mono">{reference.slice(-10)}</span>
                  </>
                )}
              </p>
            </div>
