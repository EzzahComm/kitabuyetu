...
const V = {
  light: {
    ...
    signOut:
      'w-full flex items-center gap-2.5 px-2 py-1.5 rounded-md text-sm font-medium text-muted-foreground hover:bg-red-50 hover:text-red-600 transition-colors',
    signOutIcon: 15,
...
export function PortalSidebar({
  ...
  return (
    <>
      {open && <div className={cn('fixed inset-0 z-40 lg:hidden', v.overlay)} onClick={onClose} />}
        ...
        <nav className={v.nav}>
          {filtered.map((section, si) => (
            <div key={section.title ?? si} className={cn(section.title && v.sectionWrap)}>
              {section.title && !collapsed && <p className={v.sectionTitle}>{section.title}</p>}
                ...
                {section.items.map((item) => {
                            ...
                            {item.children.map((child) => {
                              ...
                              return (
                                <Link
                                  ...
                                  href={child.href}
                                  onClick={onClose}
                                  className={cn(v.link, childActive ? v.linkActive : v.linkInactive)}
                                >
                                  <ChildIcon
                                    size={v.iconSize - 2}
                                    className={cn(childActive ? v.iconActive : v.iconInactive)}
                                  />
                  ...
                  return (
                    <Link
                      ...
                      onClick={onClose}
                      title={collapsed ? item.label : undefined}
                      className={cn(v.link, active ? v.linkActive : v.linkInactive, collapsed && 'justify-center')}
                    >
