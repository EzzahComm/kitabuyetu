  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <SectionHeader
          title="Plan"
          subtitle="Assigned by Kitabu Yetu staff — contact them to change your plan."
        />
        <Card>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-lg font-semibold text-foreground">
                  {planLoading || !plan
                    ? "—"
                    : `${formatKES(plan.monthly_fee)}/mo`}
                </p>
              </div>
            </div>
            {plan && !planLoading && (
              <p className="text-xs text-muted-foreground">
                {CAP_LABEL(plan.max_linked_groups)} groups ·{" "}
                {CAP_LABEL(plan.max_staff)} staff ·{" "}
                {CAP_LABEL(plan.max_funding_programs)} funding programs ·{" "}
                {Number(plan.sms_allowance_included).toLocaleString()} SMS/mo
                included ·{" "}
                <span className="capitalize">
                  {plan.support_tier.replace("_", "+")}
                </span>{" "}
                support
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <StatCard
          ...
          value={
            isLoading ? "—" : `${(data?.balance ?? 0).toLocaleString()} credits`
          }
          icon={MessageSquare}
        />
        <StatCard
          title="Rate per SMS"
          value={
            isLoading || data?.rate == null
              ? "—"
              : `KES ${data.rate.toFixed(4)}`
          }
                ...
                {isLoading ? (
                  ...
                  data.recent.map((r) => (
                    <tr key={r.id} className="border-b last:border-0">
                      ...
                      <td className="px-4 py-3">
                        {Number(r.credits_added).toLocaleString()}
                      </td>
                      <td className="px-4 py-3">
                        {Number(r.rate_applied).toFixed(4)}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground">
                        {r.notes ?? "—"}
                      </td>
...
function SmsCreditsTopUpDialog({
  open,
  onClose,
}: {
  open: boolean;
    ...
    >
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Recorded as a manual top-up — no M-Pesa payment is collected here.
            Bank/M-Pesa settlement is reconciled separately.
          </p>
        </div>
        <DialogFooter>
