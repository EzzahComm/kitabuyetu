...
export default function MembersPage() {
  ...
  const { data, isLoading, isError, error } = useQuery<
    PaginatedResult<OrganizationMemberRow>
  >({
    queryKey: ["enterprise", "members", page, search],
    queryFn: () =>
      organizationApi.members({ page, limit: 25, search: search || undefined }),
  });
      ...
      <PaginatedTable<OrganizationMemberRow & { id: string }>
        data={
          data
            ? {
                ...data,
                items: data.items.map((m) => ({ ...m, id: m.memberId })),
              }
            : data
        }
        isLoading={isLoading}
        ...
        emptyDescription={
          search
            ...
            : "Members appear here once branches enrol them."
        }
        emptyIcon={Users2}
        columns={[
          {
