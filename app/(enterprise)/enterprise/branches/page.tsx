...
export default function BranchesPage() {
  ...
  const regions = React.useMemo(
    () => [
      ...
      ...Array.from(
        new Set(
          (groups ?? []).map((g) => g.county).filter((c): c is string => !!c),
        ),
      ),
    ],
    [groups],
  ...
  const filtered = rows.filter((g) => {
    const matchesQuery = (g.groupName + " " + (g.county ?? ""))
      .toLowerCase()
                ...
                <tbody>
                  {filtered.map((g) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-right">
                        <MoneyDisplay
                          amount={parseFloat(g.activeLoanPortfolio)}
                          size="sm"
                        />
                      </td>
                      <td className="px-4 py-3 text-right tabular-nums">
                      ...
                      <td className="px-4 py-3">
                        <StatusPill
                          status={
                            g.defaultedLoanCount > 0 ? "review" : "active"
                          }
                          tone={
                            g.defaultedLoanCount > 0 ? "warning" : "positive"
                          }
