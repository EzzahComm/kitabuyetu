...
export default function BrandingPage() {
  ...
  const save = async () => {
    if (!colorValid) {
      toast({
        variant: "destructive",
        title: "Enter a valid hex color, e.g. #16a34a",
      });
      return;
    }
    if (!logoValid) {
      toast({
        variant: "destructive",
        title: "Logo must be a full URL (https://…)",
      });
      return;
    }
    setSaving(true);
      ...
      {isLoading ? (
        ...
        <Card className="max-w-xl">
          <CardHeader>
            ...
            <CardDescription>
              Logo must be a hosted image URL — there&apos;s no upload here yet.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              {!colorValid && (
                <p className="text-xs text-destructive">
                  Must be a hex color like #16a34a
                </p>
              )}
            </div>
            ...
            <div className="rounded-lg border p-4">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm font-semibold text-foreground">
                  Kitabu{" "}
                  <span
                    style={{ color: colorValid ? primaryColor : DEFAULT_COLOR }}
                  >
                    Enterprise
                  </span>
                </span>
