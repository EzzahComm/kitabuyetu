...
export default function DisbursementsPage() {
  ...
  const [disbursementType, setDisbursementType] =
    useState<(typeof DISBURSEMENT_TYPES)[number]>("grant");
  ...
  const { data, isLoading, isError, error } = useQuery<
    PaginatedResult<DisbursementRow>
  >({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        totalPages: Math.max(1, Math.ceil(res.total / (res.limit ?? 20))),
      };
    },
  });
  ...
  const feePct = selectedProgram?.processing_fee_pct
    ? parseFloat(selectedProgram.processing_fee_pct)
    : 0;
  const parsedAmountPreview = parseFloat(amount);
  const netPreview =
    feePct > 0 && parsedAmountPreview > 0
      ? parsedAmountPreview - Math.round(parsedAmountPreview * feePct) / 100
      : null;
  const availableBalance = walletData
    ? parseFloat(walletData.wallet.available_balance)
    : null;

  const refresh = () =>
    qc.invalidateQueries({ queryKey: ["enterprise", "disbursements"] });
  const refreshWallet = () =>
    qc.invalidateQueries({ queryKey: ["enterprise", "wallet"] });
  ...
  const submitDisbursement = async () => {
    ...
    if (!groupId || !(parsedAmount > 0)) {
      toast({
        variant: "destructive",
        title: "Pick a branch and enter a valid amount",
      });
      return;
    }
    setBusy(true);
  ...
  const reject = async () => {
    if (!rejecting || rejectReason.trim().length < 5) {
      toast({
        variant: "destructive",
        title: "Give a rejection reason (5+ chars)",
      });
      return;
    }
    setBusy(true);
    try {
      await organizationApi.disbursementAction(rejecting.id, {
        action: "reject",
          ...
          },
          {
            ...
            render: (r) => (
              <span className="capitalize text-muted-foreground">
                {r.disbursement_type.replace(/_/g, " ")}
              </span>
            ),
          },
          {
            ...
            render: (r) => (
              <MoneyDisplay amount={parseFloat(r.amount)} size="sm" />
            ),
          },
          {
              ...
              r.payment_method ? (
                <span className="text-muted-foreground">
                  {PAYMENT_METHOD_LABELS[
                    r.payment_method as (typeof PAYMENT_METHODS)[number]
                  ] ?? r.payment_method}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setDisbursementType(e.target.value as typeof disbursementType)
                }
                  ...
                  {programs.map((p) => (
                    <option key={p.id} value={p.id}>
                      ...
                      {p.processing_fee_pct
                        ? ` (${p.processing_fee_pct}% processing fee)`
                        : ""}
                    </option>
                  ))}
                </select>
            ...
            {netPreview !== null && (
              <Alert>
                ...
                <AlertDescription>
                  Branch receives{" "}
                  <strong>KES {netPreview.toLocaleString()}</strong> in cash
                  (KES {parsedAmountPreview.toLocaleString()} minus the {feePct}
                  % processing fee) — the branch still owes the full KES{" "}
                  {parsedAmountPreview.toLocaleString()} as principal. Enter a
                  larger amount if you need a specific net figure to reach the
                  branch.
                </AlertDescription>
              </Alert>
            )}
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (!next || !METHODS_WITH_REFERENCE.includes(next as never))
                    setPaymentReference("");
                }}
              >
              ...
              <p className="text-xs text-muted-foreground">
                Cash and cheque leave no receipt of their own — recording it
                here is the only trace.
              </p>
            </div>
            ...
            <p className="text-xs text-muted-foreground">
              Disbursements above your organization&apos;s approval threshold
              need a second officer to approve.
            </p>
          </div>
          <DialogFooter>
            <Button
              ...
              loading={busy}
              disabled={!canManageDisbursements}
            >
              Send disbursement
            </Button>
          </DialogFooter>
      ...
      {approving && (
        <MoneyActionDialog
          ...
          details={[
            ...
            {
              label: "Type",
              value: approving.disbursement_type.replace(/_/g, " "),
            },
