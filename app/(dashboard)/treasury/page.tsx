...
const fetchTransactions = (page: number) => api.get<TransactionPage>(`/mpesa/transactions?page=${page}&limit=20`);
...
const txColumns = [
  {
    key: 'type',
    header: 'Type',
    render: (r: MpesaTransaction) => <TypeBadge type={r.transaction_type} direction={r.direction} />,
  },
  {
    key: 'amount',
    header: 'Amount',
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
        ...
        <TabsContent value="transactions">
          <PaginatedTable
            data={
              txQ.data
                ? { items: txQ.data.items, total: txQ.data.total, page, pageSize: 20, totalPages: txQ.data.totalPages }
                : null
            }
            ...
            <CardContent className="p-5 flex items-start justify-between gap-4">
              <div className="flex-1 space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Finds STK Push requests pending for &gt;5 minutes, queries Daraja for their actual status, and
                ...
                columns={[
                  {
                    key: 'started_at',
                    header: 'Started',
                    render: (run: ReconciliationRun) => formatDateTime(run.started_at),
                  },
                  {
                    key: 'status',
                    header: 'Status',
                    render: (run: ReconciliationRun) => <StatusPill status={run.status} size="sm" />,
                  },
                  {
                    key: 'transactions_checked',
                    header: 'Checked',
                    render: (run: ReconciliationRun) => run.transactions_checked,
                  },
                  {
              ...
              <PaginatedTable
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    render: (r: ReversalRecord) => <ExpandableText>{r.remarks}</ExpandableText>,
                  },
                  {
                    key: 'status',
                    header: 'Status',
                    render: (r: ReversalRecord) => <StatusPill status={r.status} size="sm" />,
                  },
                  {
            ...
            <CardContent className="p-5">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Grants, revolving funds and other capital injected by organizations linked to this group. Each
                    receipt is posted to your books automatically (Cash / External Funding).
                  </p>
                </div>
              ...
              <PaginatedTable
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    render: (d: ExternalFundingItem) => d.disbursement_type.replace(/_/g, ' '),
                  },
                  {
                    key: 'amount',
                    header: 'Amount',
                  ...
                  },
                  {
                    key: 'status',
                    header: 'Status',
                    render: (d: ExternalFundingItem) => <StatusPill status={d.status} size="sm" />,
                  },
                  {
                    key: 'reference',
                    header: 'Reference',
                  ...
                  },
                  {
                    ...
                    header: 'Date',
                    className: 'text-muted-foreground',
                    render: (d: ExternalFundingItem) => formatDate(d.created_at),
                  },
                ]}
              />
            </CardContent>
  ...
  return (
    <div className="space-y-4">
          ...
          <PaginatedTable
            ...
            columns={[
              ...
              },
              {
                key: 'status',
                header: 'Status',
                render: (r: BankAccountRow) => <StatusPill status={r.status} size="sm" />,
              },
              {
                key: 'created_at',
                header: 'Added',
                  ...
                  <div className="flex gap-2 justify-end">
                    {r.status === 'pending_approval' && (
                      <>
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => setConfirmTarget({ id: r.id, action: 'activate' })}
                        >
                          Activate
                        </Button>
                        <Button
                    ...
                    {r.status === 'active' && (
                      <Button
                        size="sm"
                        variant="outline"
                        onClick={() => setConfirmTarget({ id: r.id, action: 'disable' })}
                      >
                        Disable
                      </Button>
                    )}
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (confirmTarget) await actionMut.mutateAsync({ id: confirmTarget.id, action: confirmTarget.action });
        }}
      />
      ...
      <ConfirmDialog
        ...
        onConfirm={async () => {
          if (rejectTarget && rejectReason)
            await actionMut.mutateAsync({ id: rejectTarget, action: 'reject', reason: rejectReason });
        }}
      />
    </div>
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Funds are reserved on request. The sweep only reaches M-Pesa after a second officer approves.
            </p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setCreateOpen(false)}>
      ...
      <MoneyActionDialog
        ...
        onConfirm={async () => {
          if (approveTarget) await actionMut.mutateAsync({ id: approveTarget.id, action: 'approve' });
        }}
      />
...
function VendorPaymentsTab() {
  ...
  const createMut = useMutation({
    ...
    onError: (err) =>
      toast({ variant: 'destructive', title: 'Failed to request payment', description: getErrorMessage(err) }),
  });
