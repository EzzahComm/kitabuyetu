...
const triggerBalance = () =>
  api.post<{ message: string }>("/mpesa/balance", {});
...
const triggerReconcile = () =>
  api.post<ReconcileResult>("/mpesa/reconcile", {});
const fetchFunding = () =>
  api.get<ExternalFundingPage>("/treasury/external-funding?limit=50");
...
function TypeBadge({ type, direction }: { type: string; direction: string }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span
        className={`text-xs ${direction === "inbound" ? "text-green-600" : "text-amber-600"}`}
      >
        {direction === "inbound" ? "↓" : "↑"}
      </span>
    </div>
...
const txColumns = [
  {
    ...
    render: (r: MpesaTransaction) => (
      <TypeBadge type={r.transaction_type} direction={r.direction} />
    ),
  },
  {
    key: "amount",
  ...
  },
  {
    key: "phone",
    header: "Phone",
    render: (r: MpesaTransaction) => r.phone_number ?? "—",
  },
  {
    key: "receipt",
    header: "Receipt",
    render: (r: MpesaTransaction) => (
      <span className="font-mono text-xs">{r.mpesa_receipt_number ?? "—"}</span>
    ),
  },
  {
    key: "status",
    ...
    render: (r: MpesaTransaction) => (
      <div>
        ...
        {r.failure_reason && (
          <ExpandableText
            lines={2}
            className="text-[10px] text-destructive mt-0.5 max-w-[180px]"
          >
            {r.failure_reason}
          </ExpandableText>
        )}
  ...
  },
  {
    key: "date",
    header: "Date",
    render: (r: MpesaTransaction) => formatDate(r.created_at),
  },
];
...
export default function TreasuryPage() {
  ...
  const fundingQ = useQuery({
    queryKey: ["treasury-external-funding"],
  ...
  const balanceMut = useMutation({
    ...
    onSuccess: () =>
      setTimeout(
        () => qc.invalidateQueries({ queryKey: ["mpesa-balance"] }),
        35_000,
      ),
  });
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {accounts.map(({ label, key, icon }) => (
          <StatCard
            ...
            icon={icon}
            value={
              balanceQ.isLoading ? "…" : balance ? formatKES(balance[key]) : "—"
            }
            description={
              balance?.queriedAt
                ? `As of ${formatDateTime(balance.queriedAt)}`
                : undefined
            }
          />
        ))}
      ...
      {balanceMut.isSuccess && (
        <p className="text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg px-4 py-2">
          Balance query sent. Results appear within ~30 s — refresh to see
          updated figures.
        </p>
      )}
            ...
            data={
              txQ.data
                ? {
                    ...
                    page,
                    pageSize: 20,
                    totalPages: txQ.data.totalPages,
                  }
                : null
            }
            ...
            <CardContent className="p-5 flex items-start justify-between gap-4">
              <div className="flex-1 space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Finds STK Push requests pending for &gt;5 minutes, queries
                  Daraja for their actual status, and resolves mismatches
                  automatically.
                </p>
                {reconcileMsg && (
            ...
            <CardContent className="p-4 pt-0">
              <PaginatedTable
                ...
                columns={[
                  ...
                  {
                    key: "transactions_checked",
                    header: "Checked",
                    render: (run: ReconciliationRun) =>
                      run.transactions_checked,
                  },
                  {
                    key: "mismatches_found",
                    header: "Mismatches",
                    render: (run: ReconciliationRun) => run.mismatches_found,
                  },
                  {
                    key: "resolved_count",
                    header: "Resolved",
                    className: "font-medium text-green-700",
                    render: (run: ReconciliationRun) => run.resolved_count,
                  },
                  {
                    key: "duration",
                    header: "Duration",
                    className: "text-muted-foreground",
                    render: reconciliationDuration,
                  },
                ]}
              />
            </CardContent>
                ...
                columns={[
                  {
                    ...
                    render: (r: ReversalRecord) => r.original_receipt_number,
                  },
                  {
                    key: "amount",
                    header: "Amount",
                    className: "font-medium",
                    render: (r: ReversalRecord) => formatKES(r.amount),
                  },
                  {
                    key: "remarks",
                    header: "Remarks",
                    className: "max-w-[180px]",
                    render: (r: ReversalRecord) => (
                      <ExpandableText>{r.remarks}</ExpandableText>
                    ),
                  },
                  {
                    key: "status",
                  ...
                  },
                  {
                    ...
                    header: "Requested By",
                    render: (r: ReversalRecord) => r.requested_by_name,
                  },
                  {
                    key: "created_at",
                    header: "Date",
                    className: "text-muted-foreground",
                    render: (r: ReversalRecord) => formatDate(r.created_at),
                  },
                ]}
              />
            </CardContent>
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h3 className="font-semibold">
                    Funding received from partner organizations
                  </h3>
                  <p className="text-sm text-muted-foreground">
                    Grants, revolving funds and other capital injected by
                    organizations linked to this group. Each receipt is posted
                    to your books automatically (Cash / External Funding).
                  </p>
                </div>
                <div className="text-right shrink-0">
                  ...
                  <p className="text-xl font-bold text-green-700">
                    {fundingQ.isLoading
                      ? "…"
                      : formatKES(fundingQ.data?.totalReceived ?? 0)}
                  </p>
                </div>
              </div>
            </CardContent>
                ...
                columns={[
                  {
                    ...
                    header: "Organization",
                    className: "font-medium",
                    render: (d: ExternalFundingItem) => d.organization_name,
                  },
                  {
                    key: "program_name",
                    header: "Program",
                  ...
                  },
                  {
                    ...
                    header: "Amount",
                    className: "font-medium",
                    render: (d: ExternalFundingItem) => formatKES(d.amount),
                  },
                  {
                    key: "status",
                    header: "Status",
                  ...
                  },
                  {
                    ...
                    header: "Reference",
                    className: "font-mono text-xs",
                    render: (d: ExternalFundingItem) => d.reference,
                  },
                  {
                    key: "created_at",
                    header: "Date",
...
function BankAccountsTab() {
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Awaiting a second officer’s activation.",
      });
    },
    onError: (err) =>
      toast({
  ...
  const actionMut = useMutation({
    mutationFn: (args: {
      id: string;
      action: "activate" | "reject" | "disable";
      reason?: string;
    }) =>
      api.post(`/treasury/bank-accounts/${args.id}`, {
        action: args.action,
        reason: args.reason,
      }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4">
            ...
            columns={[
              {
                ...
                header: "Bank",
                className: "font-medium",
                render: (r: BankAccountRow) => r.bank_name,
              },
              {
                key: "shortcode",
                header: "Shortcode",
                className: "font-mono text-xs",
                render: (r: BankAccountRow) => r.shortcode,
              },
              {
                key: "account_number",
                header: "Account No.",
                className: "font-mono text-xs",
                render: (r: BankAccountRow) => r.account_number,
              },
              {
                key: "label",
                header: "Label",
              ...
              },
              {
                ...
                header: "Added",
                className: "text-muted-foreground",
                render: (r: BankAccountRow) => formatDate(r.created_at),
              },
              {
                key: "actions",
                header: "",
                  ...
                  <div className="flex gap-2 justify-end">
                    {r.status === "pending_approval" && (
                      <>
                        <Button
                          ...
                          variant="outline"
                          onClick={() =>
                            setConfirmTarget({ id: r.id, action: "activate" })
                          }
                        >
                          Activate
                        </Button>
                    ...
                    {r.status === "active" && (
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setConfirmTarget({ id: r.id, action: "disable" })
                        }
                      >
                        Disable
                      </Button>
      ...
      <ConfirmDialog
        ...
        confirmLabel={
          confirmTarget?.action === "activate" ? "Activate" : "Disable"
        }
        onConfirm={async () => {
          if (confirmTarget)
...
function SettlementsTab() {
  ...
  const [approveTarget, setApproveTarget] = useState<SettlementRow | null>(
    null,
  );
  const [rejectTarget, setRejectTarget] = useState<string | null>(null);
  const [rejectReason, setRejectReason] = useState("");
  ...
  const invalidate = () =>
    qc.invalidateQueries({ queryKey: ["treasury-settlements"] });
  ...
  const createMut = useMutation({
    ...
    onError: (err) =>
      toast({
        variant: "destructive",
        title: "Failed to request settlement",
        description: getErrorMessage(err),
      }),
  });
  ...
  const actionMut = useMutation({
    mutationFn: (args: {
      ...
      api.post(`/treasury/settlements/${args.id}`, {
        action: args.action,
        reason: args.reason,
      }),
    onSuccess: () => {
    ...
    <div className="space-y-4">
      ...
      {activeBanks.length === 0 && !banksQ.isLoading && (
        <p className="text-sm text-muted-foreground bg-muted/40 border rounded-lg px-4 py-3">
          Add and activate a bank account first — settlements can only be sent
          ...
          <PaginatedTable
            ...
            columns={[
              ...
              },
              {
                ...
                header: "Amount",
                className: "font-medium",
                render: (r: SettlementRow) => formatKES(r.amount),
              },
              {
                key: "status",
                header: "Status",
              ...
              },
              {
                ...
                header: "Requested",
                className: "text-muted-foreground",
                render: (r: SettlementRow) => formatDate(r.requested_at),
              },
              {
                key: "completed_at",
                header: "Completed",
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Funds are reserved on request. The sweep only reaches M-Pesa after
        ...
        details={
          approveTarget
            ? [
                ...
                {
                  label: "Requested",
                  value: formatDate(approveTarget.requested_at),
                },
              ]
            : []
        }
...
function VendorPaymentsTab() {
  ...
  const [approveTarget, setApproveTarget] = useState<VendorPaymentRow | null>(
    null,
  );
  const [rejectTarget, setRejectTarget] = useState<string | null>(null);
  const [rejectReason, setRejectReason] = useState("");
  ...
  const invalidate = () =>
    qc.invalidateQueries({ queryKey: ["treasury-vendor-payments"] });

  const createMut = useMutation({
    mutationFn: () =>
      api.post(
        "/treasury/vendor-payments",
        channel === "b2c"
          ? {
              channel,
              payeeName,
  ...
  const actionMut = useMutation({
    mutationFn: (args: {
      ...
      api.post(`/treasury/vendor-payments/${args.id}`, {
        action: args.action,
        reason: args.reason,
      }),
    onSuccess: () => {
  ...
  const destination = (r: VendorPaymentRow) =>
    r.channel === "b2c"
      ? (r.payee_phone ?? "—")
      : `${r.payee_shortcode ?? "—"} / ${r.payee_account ?? "—"}`;
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button size="sm" onClick={() => setCreateOpen(true)}>
          <Plus size={15} /> New vendor payment
        </Button>
      </div>
            ...
            columns={[
              {
                ...
                header: "Payee",
                className: "font-medium",
                render: (r: VendorPaymentRow) => r.payee_name,
              },
              {
                key: "channel",
                header: "Channel",
                render: (r: VendorPaymentRow) => (
                  <Badge variant="outline" className="uppercase text-[10px]">
                    {r.channel}
                  </Badge>
                ),
              ...
              },
              {
                ...
                header: "Amount",
                className: "font-medium",
                render: (r: VendorPaymentRow) => formatKES(r.amount),
              },
              {
                key: "status",
                header: "Status",
              ...
              },
              {
                ...
                header: "Requested",
                className: "text-muted-foreground",
                render: (r: VendorPaymentRow) => formatDate(r.requested_at),
              },
              {
                key: "actions",
                header: "",
            ...
            {channel === "b2c" ? (
              ...
              <>
                <div className="space-y-1">
                  ...
                  <Input
                    value={payeeShortcode}
                    onChange={(e) => setPayeeShortcode(e.target.value)}
                  />
                </div>
                <div className="space-y-1">
                  <Label>Account number</Label>
        ...
        details={
          approveTarget
            ? [
                ...
                {
                  label: "Channel",
                  value: approveTarget.channel.toUpperCase(),
                },
