...
export default function CreateAdditionalGroupPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      const data = await authApi.createGroup(
        values as CreateAdditionalGroupPayload,
      );
      login(data);
      toast({
      ...
      router.push(
        postLoginPath(data.member.groupRole, {
          signupProduct: data.signupProduct,
        }),
      );
    } catch (err) {
      toast({
    ...
    <div className="space-y-6">
      ...
      <Card className="max-w-2xl">
        <CardHeader>
          ...
          <CardDescription>
            You&rsquo;ll be switched into the new group once it&rsquo;s created.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              ...
              <select
                ...
                {...register("product")}
              >
                {(Object.keys(PRODUCT_LABELS) as FormValues["product"][]).map(
                  (p) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  aria-label="Your role in this group"
                  className={selectCls}
