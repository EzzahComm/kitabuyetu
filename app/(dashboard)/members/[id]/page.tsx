...
export default function MemberDetailPage({
  ...
  const currentStatus: string =
    m.group_status ?? (m.is_active ? "active" : "inactive");
            ...
            <div className="flex gap-2 flex-shrink-0">
              ...
              {currentStatus === "archived" ? (
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setStatusDialog({ target: "active" })}
                >
                ...
                <>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => setStatusDialog({ target: "suspended" })}
                  >
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => setStatusDialog({ target: "archived" })}
                  >
              ...
              {creditScoreQ.isLoading ? (
                ...
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-xl font-bold">
                    {Number(creditScoreQ.data.overall_score).toFixed(0)}
                  </p>
                  <StatusPill
      ...
      <Tabs defaultValue="contributions">
        <TabsList>
          <TabsTrigger value="contributions">
            <CreditCard size={14} className="mr-1" /> Contributions (
          ...
          <Card>
            ...
            <CardContent>
              {loadingKin ? (
                ...
                <p className="text-sm text-muted-foreground py-6 text-center">
                  No emergency contacts on file. Add at least one primary
                  contact (priority 1).
                </p>
              ) : (
                <div className="divide-y">
            ...
            <CardContent className="pt-4">
              <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                {[
                  ...
                  [
                    "Date of Birth",
                    m.date_of_birth ? formatDate(m.date_of_birth) : "—",
                  ],
...
function AddKinDialog({
  ...
  const create = useMutation({
    mutationFn: (body: CreateNextOfKinPayload) =>
      nextOfKinApi.create(memberId, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["next-of-kin", memberId] });
...
function StatusDialog({
  ...
  const apply = useMutation({
    mutationFn: () =>
      membersApi.transitionStatus(memberId, target!, reason || undefined),
    onSuccess: () => {
  ...
  return (
    ...
    >
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>
            {target ? (STATUS_LABELS[target] ?? `Change to ${target}`) : ""}
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <p className="text-sm text-muted-foreground">
            This will set the member&apos;s status to{" "}
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={
              apply.isPending || (reasonRequired && reason.trim().length === 0)
            }
