...
export default function MembersImportPage() {
    ...
    async (file: File) => {
      if (!file.name.toLowerCase().endsWith(".csv")) {
        toast({
          ...
          title: "CSV only",
          description:
            "Excel (.xlsx) support is coming in a follow-up phase. Save as CSV and try again.",
        });
        return;
      }
      setPhase("uploading");
  ...
  const commit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${result.imported ?? 0} member(s) added`,
      });
    } catch (err) {
      setPhase("preview");
  ...
  const rollback = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${result.deleted ?? 0} member(s) removed`,
      });
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {phase === "idle" && (
        <IdleView onUpload={uploadFile} canPreview={canPreview} />
      )}
          ...
          >
            ...
            {!canPreview && (
              <p className="mt-2 text-xs text-amber-600">
                Requires secretary, treasurer, or chairperson role.
              </p>
            )}
            <input
              ref={inputEl}
      ...
      {visible.length > 0 && (
        <Card>
          ...
          <CardContent>
            <PaginatedTable
              data={singlePage(
                visible.map((r) => ({ ...r, id: String(r.row_num) })),
              )}
              isLoading={false}
              onPageChange={() => {}}
            ...
            {hiddenCount > 0 && (
              <p className="mt-3 text-xs text-muted-foreground">
                …and {hiddenCount} more row(s). They’ll all be imported on
                confirm.
              </p>
            )}
          </CardContent>
      ...
      <div className="flex justify-end gap-3">
        ...
        {canCommit && (
          <Button onClick={onCommit} disabled={job.valid_rows === 0}>
            Confirm import ({job.valid_rows} member
            {job.valid_rows === 1 ? "" : "s"})
          </Button>
        )}
      </div>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {isRolledBack && job.deleted !== undefined && (
          <p className="text-sm">
            ...
            {(job.blocked?.length ?? 0) > 0 && (
              <>
                {" "}
                <strong>{job.blocked!.length}</strong> kept because they have
                dependent records (membership removed from this group).
              </>
            )}
          </p>
        )}
        ...
        <div className="flex justify-end gap-3">
          ...
          {isCommitted &&
            (job.created_member_ids?.length ?? 0) > 0 &&
            canRollback && (
