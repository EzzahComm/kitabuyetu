...
export default function ContributionSplitsPage() {
  ...
  const { data, isLoading } = useQuery<{ items: ServerSplit[] }>({
    ...
    queryFn: () =>
      api.get<{ items: ServerSplit[] }>("/settings/contribution-splits"),
  });
  ...
  const [seededFrom, setSeededFrom] = useState<{ items: ServerSplit[] } | null>(
    null,
  );
  ...
  const save = async () => {
    ...
    for (const r of rows) {
      if (!r.accountCode.trim()) {
        toast({
          variant: "destructive",
          title: "Every row needs an account code",
        });
        return;
      }
      if (!r.value || parseFloat(r.value) <= 0) {
        toast({
          variant: "destructive",
          title: `Enter a positive value for ${r.accountCode}`,
        });
        return;
      }
    }
    if (pctTotal > 100.01) {
      toast({
        variant: "destructive",
        title: `Percentages total ${pctTotal.toFixed(2)}% — must be ≤ 100`,
      });
      return;
    }
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Card className="bg-muted/40 border-dashed">
        <CardContent className="py-3 flex gap-2 text-sm text-muted-foreground">
          ...
          <span>
            Fixed amounts are taken first, then the rest is split by percentage.
            Anything left over (or all of it, if no rules) goes to{" "}
            <strong>Member Contributions (4001)</strong>.
          </span>
        </CardContent>
      </Card>
        ...
        <div className="space-y-3">
          {rows.length === 0 && (
            <p className="text-sm text-muted-foreground">
              No splits configured — 100% goes to {DEFAULT_CODE} (Member
              Contributions).
            </p>
          )}
          {rows.map((row, i) => (
            ...
            >
              <div className="space-y-1 flex-1 min-w-[140px]">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateRow(i, { accountCode: e.target.value })
                  }
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  value={row.mode}
                  onChange={(e) =>
                    updateRow(i, { mode: e.target.value as SplitRow["mode"] })
                  }
              ...
              <div className="space-y-1 w-28">
                <Label className="text-xs">
                  {row.mode === "percentage" ? "Percent" : "Amount"}
                </Label>
                <Input
                  type="number"
                  step="0.01"
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <span
              className={`text-sm ${pctTotal > 100.01 ? "text-destructive" : "text-muted-foreground"}`}
            >
