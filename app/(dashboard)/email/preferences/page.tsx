...
export default function EmailPreferencesPage() {
  ...
  function togglePref(category: string, field: 'enabled' | 'frequency', value: string | boolean) {
    setEdits(prefs.map((p) => (p.category === category ? { ...p, [field]: value } : p)));
  }
            ...
            : prefs.map((pref) => (
                <div key={pref.category} className="py-3 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-card shadow transition-transform ${pref.enabled ? 'translate-x-4' : 'translate-x-0'}`}
                      />
                    </button>
                    <span className="text-sm font-medium">{CATEGORY_LABELS[pref.category] ?? pref.category}</span>
                  </div>
                  <Select
