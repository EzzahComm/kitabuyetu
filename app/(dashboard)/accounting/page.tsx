...
export default function AccountingPage() {
  ...
  const {
    ...
    error: accountsError,
  } = useAccounts({ enabled: tab === 'accounts' || tab === 'policies' });
  const {
    data: trialBalance,
    isLoading: loadingTB,
    isError: errorTB,
    error: tbError,
  } = useTrialBalance({ enabled: tab === 'trial' });
  ...
  const {
    ...
    error: journalsError,
  } = useJournals({ page: journalPage, pageSize: 20 }, { enabled: tab === 'journals' });
  const {
    data: pnl,
    isLoading: loadingPnl,
    isError: errorPnl,
    error: pnlError,
  } = useProfitAndLoss(from, to, { enabled: tab === 'pnl' });
  ...
  const {
    ...
    isError: errorBS,
    error: bsError,
  } = useBalanceSheet(asOfToday, { enabled: tab === 'balance' });
  const {
    data: cashFlow,
    isLoading: loadingCF,
    isError: errorCF,
    error: cfError,
  } = useCashFlow(from, to, { enabled: tab === 'cashflow' });
  ...
  const {
    ...
    isError: errorFP,
    error: fpError,
  } = useFiscalPeriods({ enabled: tab === 'periods' });
  ...
  const {
    ...
    error: policiesError,
  } = useApprovalPolicies({ enabled: tab === 'policies' });
  ...
  const handleSubmitJournal = async () => {
    if (!balanced) {
      toast({ variant: 'destructive', title: 'Journal must balance (debits = credits)' });
      return;
    }
                  ...
                  columns={[
                    {
                      key: 'accountCode',
                      header: 'Code',
                      render: (row) => <span className="font-mono text-xs">{row.accountCode}</span>,
                    },
                    { key: 'accountName', header: 'Account' },
                    {
                      ...
                      render: (row) => <Badge variant="outline">{row.accountType}</Badge>,
                    },
                    {
                      key: 'totalDebits',
                      header: 'Debit',
          ...
          <PaginatedTable<JournalEntry>
            ...
            columns={[
              ...
              { key: 'memo', header: 'Memo', className: 'max-w-[200px] truncate', render: (j) => j.memo ?? '—' },
              {
                ...
                header: 'Status',
                render: (j) => (
                  <Badge variant={j.status === 'posted' ? 'success' : 'warning'} className="capitalize text-xs">
                    {j.status}
                  </Badge>
                ),
              },
                  ...
                  <tbody>
                    ...
                    <tr className="border-t font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(balanceSheet?.totalAssets ?? '0'))}
                      </td>
                    </tr>
                    ...
                    <tr className="border-t font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(balanceSheet?.totalLiabilities ?? '0'))}
                      </td>
                    </tr>
                    ...
                    <tr className="border-t font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(balanceSheet?.totalEquity ?? '0'))}
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <Card className="mt-4">
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  ...
                  income/expense accounts until a closing entry moves it to Retained Surplus.
                </p>
              </CardHeader>
              <CardContent className="overflow-x-auto p-0">
                <table className="w-full text-sm">
          ...
          <Card>
            <CardHeader className="flex-row items-center justify-between space-y-0">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Closing a period blocks new manual journal entries dated inside it. Automated postings from real
                  M-Pesa events are never blocked.
                </p>
              </div>
              {canManageAccounting && (
              ...
              <PaginatedTable
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    header: 'Status',
                    render: (p) => (
                      <Badge variant={p.status === 'closed' ? 'destructive' : 'success'} className="text-xs capitalize">
                        {p.status}
                      </Badge>
                    ),
                  },
            ...
            columns={[
              {
                key: 'account_code',
                header: 'Code',
                render: (a) => <span className="font-mono text-xs">{a.account_code}</span>,
              },
              ...
              },
              {
                ...
                header: 'Balance',
                className: 'text-right',
                render: (a) => <span className="font-mono">{formatKES(a.balance)}</span>,
              },
            ]}
          />
        </TabsContent>
                        ...
                        <tr key={p.key} className="border-t hover:bg-muted/20">
                          ...
                          <td className="px-4 py-2 text-right">
                            <Button
                              ...
                              className="h-8 gap-1.5"
                              disabled={
                                !canManageAccounting || !dirty || setPolicy.isPending || !(parseFloat(editValue) >= 0)
                              }
        ...
        <DialogContent className="max-w-2xl">
              ...
              <table className="w-full text-sm">
                    ...
                    <tr key={idx}>
                      ...
                      <td className="px-2 pb-2">
                        <Input
                          ...
                          onChange={(e) => {
                            const nl = [...lines];
                            nl[idx] = { ...nl[idx], debit: +e.target.value, credit: 0 };
                            setLines(nl);
                          }}
                        />
                      </td>
                      <td className="px-2 pb-2">
                        <Input
                          ...
                          onChange={(e) => {
                            const nl = [...lines];
                            nl[idx] = { ...nl[idx], credit: +e.target.value, debit: 0 };
                            setLines(nl);
                          }}
                        />
                      </td>
                      <td className="pb-2">
                        <Button
                          ...
                          title={lines.length <= 2 ? 'A journal entry needs at least 2 lines' : 'Remove line'}
                ...
                <tfoot>
                  <tr className="border-t text-sm font-semibold">
                    ...
                    <td className={`pt-2 text-right px-2 ${!balanced ? 'text-red-600' : ''}`}>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                !balanced || !entryDate || description.trim().length < 3 || lines.filter((l) => l.accountId).length < 2
              }
...
function FineScheduleCard() {
  ...
  const rows =
    edits ??
    (schedule ? Object.entries(schedule).map(([category, amount]) => ({ category, amount: String(amount) })) : null);
              ...
              {rows.map((row, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <Input
                    ...
                    value={row.category}
                    onChange={(e) => {
                      const next = rows.map((r, i) => (i === idx ? { ...r, category: e.target.value } : r));
                      setEdits(next);
                    }}
                  />
                  <Input
            ...
            <div className="flex items-center justify-between">
              {canSetFineSchedule && (
                <Button
                  ...
                  variant="ghost"
                  className="gap-1.5"
                  onClick={() => setEdits([...rows, { category: '', amount: '0' }])}
                >
              ...
              <Button
                ...
                disabled={
                  ...
                  save.isPending ||
                  rows.length === 0 ||
                  rows.some((r) => !r.category.trim() || !(parseFloat(r.amount) >= 0))
                }
