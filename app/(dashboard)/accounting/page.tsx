...
export default function AccountingPage() {
  ...
  const {
    ...
    error: journalsError,
  } = useJournals({ page: journalPage, pageSize: 20 });
  const {
    data: pnl,
    isLoading: loadingPnl,
    isError: errorPnl,
    error: pnlError,
  } = useProfitAndLoss(from, to);
  ...
  const handleSubmitJournal = async () => {
    if (!balanced) {
      toast({
        variant: "destructive",
        title: "Journal must balance (debits = credits)",
      });
      return;
    }
    ...
    if (payloadLines.length < 2) {
      toast({
        variant: "destructive",
        title: "Pick an account on at least 2 lines",
      });
      return;
    }
    ...
    <div className="space-y-6">
        ...
        actions={
          canManageAccounting ? (
            <Button onClick={() => setOpen(true)}>
              <Plus size={16} className="mr-2" /> New journal
            </Button>
          ) : undefined
        }
      />
            ...
            columns={[
              {
                key: "entryDate",
                header: "Date",
                render: (j) => formatDate(j.entryDate),
              },
              {
                key: "reference",
                header: "Reference",
              ...
              },
              {
                ...
                render: (j) => (
                  <Badge
                    variant={j.status === "posted" ? "success" : "warning"}
                    className="capitalize text-xs"
                  >
                    {j.status}
                  </Badge>
        ...
        <TabsContent value="balance" className="mt-4">
          {errorBS ? (
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">
                  Balance Sheet — as of{" "}
                  {balanceSheet ? formatDate(balanceSheet.asOf) : ""}
                </CardTitle>
              </CardHeader>
                ...
                <table className="w-full text-sm">
                  <tbody>
                    ...
                    {(balanceSheet?.assets ?? []).map((a) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-right font-mono">
                          {formatKES(parseFloat(a.balance))}
                        </td>
                      </tr>
                    ))}
                    <tr className="border-t font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(
                          parseFloat(balanceSheet?.totalAssets ?? "0"),
                        )}
                      </td>
                    </tr>
                    ...
                    <tr className="border-t font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(
                          parseFloat(balanceSheet?.totalLiabilities ?? "0"),
                        )}
                      </td>
                    </tr>
                    ...
                    <tr className="border-t font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(
                          parseFloat(balanceSheet?.totalEquity ?? "0"),
                        )}
                      </td>
                    </tr>
                  </tbody>
          ...
          {equityChanges ? (
            <Card className="mt-4">
              <CardHeader>
                <CardTitle className="text-base">
                  Statement of Changes in Equity — {now.getFullYear()}
                </CardTitle>
                <p className="text-sm text-muted-foreground mt-1">
                  Movement per equity account this year. The period&apos;s net
                  surplus of{" "}
                  <span className="font-mono">
                    {formatKES(parseFloat(equityChanges.periodNetProfit))}
                  </span>{" "}
                  remains in income/expense accounts until a closing entry moves
                  it to Retained Surplus.
                </p>
              </CardHeader>
              <CardContent className="overflow-x-auto p-0">
                <table className="w-full text-sm">
                  <thead className="bg-muted/50">
                    <tr>
                      {[
                        ...
                        <th
                          key={h}
                          className={`px-4 py-2 text-xs font-medium text-muted-foreground ${i === 0 ? "text-left" : "text-right"}`}
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {equityChanges.lines.map((l) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-2 text-right font-mono">
                          {formatKES(parseFloat(l.opening))}
                        </td>
                        <td className="px-4 py-2 text-right font-mono">
                          {formatKES(parseFloat(l.increases))}
                        </td>
                        <td className="px-4 py-2 text-right font-mono">
                          {formatKES(parseFloat(l.decreases))}
                        </td>
                        <td className="px-4 py-2 text-right font-mono font-semibold">
                          {formatKES(parseFloat(l.closing))}
                        </td>
                      </tr>
                    ))}
                    <tr className="border-t font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(equityChanges.totalOpening))}
                      </td>
                      <td className="px-4 py-2" colSpan={2} />
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(equityChanges.totalClosing))}
                      </td>
                    </tr>
                  </tbody>
                </table>
        ...
        <TabsContent value="cashflow" className="mt-4">
          {errorCF ? (
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base">
                  Cash Flow Statement — {now.getFullYear()}
                </CardTitle>
                <p className="text-sm text-muted-foreground mt-1">
                  ...
                  <tbody>
                    {(
                      [
                        ...
                        ["Financing activities", "financing", "netFinancing"],
                      ] as const
                    ).map(([label, key, netKey]) => (
                      <React.Fragment key={key}>
                        ...
                        {(cashFlow?.[key] ?? []).length === 0 ? (
                          ...
                          (cashFlow?.[key] ?? []).map((l) => (
                            ...
                            >
                              <td className="px-4 py-2">{l.accountName}</td>
                              <td
                                className={`px-4 py-2 text-right font-mono ${parseFloat(l.amount) < 0 ? "text-destructive" : ""}`}
                              >
                        ...
                        <tr className="border-t font-semibold">
                          <td className="px-4 py-2">
                            Net cash from {label.toLowerCase()}
                          </td>
                          <td className="px-4 py-2 text-right font-mono">
                            {formatKES(parseFloat(cashFlow?.[netKey] ?? "0"))}
                          </td>
                        </tr>
                      </React.Fragment>
                    ))}
                    <tr className="bg-muted/50 font-semibold">
                      ...
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(cashFlow?.netChange ?? "0"))}
                      </td>
                    </tr>
                    <tr className="border-t">
                      <td className="px-4 py-2">Opening cash</td>
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(cashFlow?.openingCash ?? "0"))}
                      </td>
                    </tr>
                    <tr className="border-t font-semibold">
                      <td className="px-4 py-2">Closing cash</td>
                      <td className="px-4 py-2 text-right font-mono">
                        {formatKES(parseFloat(cashFlow?.closingCash ?? "0"))}
                      </td>
                    </tr>
                    {cashFlow && !cashFlow.reconciles && (
                      <tr className="border-t">
                        ...
                        >
                          Opening + net change does not equal closing — some
                          cash movement could not be classified. Contact
                          support.
                        </td>
                      </tr>
                    )}
          ...
          <Card>
            <CardHeader className="flex-row items-center justify-between space-y-0">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Closing a period blocks new manual journal entries dated
                  inside it. Automated postings from real M-Pesa events are
                  never blocked.
                </p>
              </div>
              {canManageAccounting && (
                <Button size="sm" onClick={() => setCloseOpen(true)}>
                  <Lock size={15} className="mr-2" /> Close a period
                </Button>
              )}
            </CardHeader>
                ...
                columns={[
                  ...
                  },
                  {
                    ...
                    render: (p) => (
                      <Badge
                        variant={
                          p.status === "closed" ? "destructive" : "success"
                        }
                  ...
                  },
                  {
                    ...
                    className: "text-right",
                    render: (p) =>
                      p.status === "closed" && canManageAccounting ? (
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-8 gap-1.5"
                          onClick={() => {
                            setReopenTarget(p.id);
                            setReopenReason("");
                          }}
                        >
          ...
          <PaginatedTable<Account>
            ...
            columns={[
              ...
              },
              {
                ...
                className: "text-right",
                render: (a) => (
                  <span className="font-mono">{formatKES(a.balance)}</span>
                ),
              },
            ]}
          />
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground mt-1">
                ...
                one shows where its current value comes from — override it here
                to set your own group-specific limit.
              </p>
            </CardHeader>
            <CardContent className="overflow-x-auto p-0">
              {errorPolicies ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(policies ?? []).map((p) => {
                      const editValue =
                        policyEdits[p.key] ?? String(p.threshold);
                      ...
                      return (
                        <tr key={p.key} className="border-t hover:bg-muted/20">
                          ...
                          <td className="px-4 py-2">
                            <Badge
                              variant={
                                p.source === "group" ? "success" : "outline"
                              }
                              className="text-xs capitalize"
                            >
                              {p.source === "group"
                          ...
                          <td className="px-4 py-2 text-right">
                            <Button
                              ...
                              title={
                                canManageAccounting
                                  ? undefined
                                  : "Requires treasurer or chairperson"
                              }
                              onClick={async () => {
                                try {
              ...
              <table className="w-full text-sm">
                    ...
                    <tr key={idx}>
                      <td className="pr-2 pb-2">
                        <select
                          ...
                          onChange={(e) => {
                            const nl = [...lines];
                            nl[idx] = { ...nl[idx], accountId: e.target.value };
                            setLines(nl);
                          }}
                ...
                <tfoot>
                  <tr className="border-t text-sm font-semibold">
                    ...
                    <td
                      className={`pt-2 text-right px-2 ${!balanced ? "text-red-600" : ""}`}
                    >
      ...
      <Dialog open={closeOpen} onOpenChange={setCloseOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Close a fiscal period</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              No new manual journal entries can be dated inside this range until
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Reopen this period</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
              <Label>Reason</Label>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Which accounts each automatic posting hits. You can point an event at
          a different account in your chart — the debit/credit structure itself
          is fixed, so entries always balance.
        </p>
      </CardHeader>
      <CardContent className="overflow-x-auto p-0">
        {isError ? (
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-2 whitespace-nowrap">
                      {POSTING_EVENT_LABELS[t.event] ?? t.event}
                    </td>
                    <td className="px-4 py-2">
                      <Badge
                        variant={t.source === "group" ? "success" : "outline"}
                        className="text-xs capitalize"
                      >
                        {t.source === "group"
                          ? "Your override"
                      ...
                      <div className="space-y-1">
                        {lines.map((line, idx) => (
                          <div key={idx} className="flex items-center gap-2">
                            <span
                              className={`w-8 shrink-0 text-[11px] font-mono font-semibold ${line.side === "debit" ? "text-emerald-700 dark:text-emerald-500" : "text-sky-700 dark:text-sky-500"}`}
                            >
                            ...
                            >
                              {!accounts.some(
                                (a) => a.account_code === line.accountCode,
                              ) && (
                                <option value={line.accountCode}>
                                  {line.accountCode} — (not in your chart)
                                </option>
                              )}
                              {accounts.map((a) => (
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between gap-2">
          ...
          {source && (
            <Badge
              variant={source === "group" ? "success" : "outline"}
              className="text-xs capitalize"
            >
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, interestRate: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, interestMethod: e.target.value })
                  }
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, maxTermMonths: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, loanMultiplier: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1 sm:col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, termOptions: e.target.value })
                  }
                />
                {/* Unlike the rate, this one is enforced — worth saying so
                    plainly on the screen that sets it. */}
                <p className="text-xs text-muted-foreground">
                  The loan form offers exactly these lengths and applications
                  are held to them. Leave blank to allow any term up to the
                  maximum.
                </p>
              </div>
            </div>
                ...
                onClick={async () => {
                  try {
                    await save.mutateAsync({
                      ...
                      interestMethod: form.interestMethod as
                        | "flat"
                        | "reducing_balance",
  ...
  const form =
    ...
    (limits
      ? {
          ...
          maxContribution:
            limits.maxContribution === null
              ? ""
              : String(limits.maxContribution),
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, minContribution: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, maxContribution: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setEdits({ ...form, gracePeriodDays: e.target.value })
                  }
                />
              </div>
            </div>
                    ...
                    await save.mutateAsync({
                      ...
                      maxContribution:
                        form.maxContribution === ""
                          ? null
                          : parseFloat(form.maxContribution),
...
function FineScheduleCard() {
  ...
  const rows =
    edits ??
    (schedule
      ? Object.entries(schedule).map(([category, amount]) => ({
          category,
            ...
            <div className="space-y-2">
              {rows.map((row, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <Input
                    className="flex-1"
                    placeholder="Offence (e.g. late_attendance)"
                    value={row.category}
                    onChange={(e) => {
            ...
            <div className="flex items-center justify-between">
              {canSetFineSchedule && (
                <Button
                  ...
                  className="gap-1.5"
                  onClick={() =>
                    setEdits([...rows, { category: "", amount: "0" }])
                  }
                >
              ...
              <Button
                ...
                disabled={
                  ...
                  rows.length === 0 ||
                  rows.some(
                    (r) => !r.category.trim() || !(parseFloat(r.amount) >= 0),
                  )
                }
                ...
                onClick={async () => {
                  try {
                    ...
                    for (const r of rows)
                      schedule[r.category.trim()] = parseFloat(r.amount);
