...
const ContributionsChart = dynamic(
  () => import("./_charts").then((m) => m.ContributionsChart),
  { ssr: false },
);
const RepaymentsChart = dynamic(
  () => import("./_charts").then((m) => m.RepaymentsChart),
  { ssr: false },
);
const PortfolioDonutChart = dynamic(
  () => import("./_charts").then((m) => m.PortfolioDonutChart),
  { ssr: false },
);
const CreditTierChart = dynamic(
  () => import("./_charts").then((m) => m.CreditTierChart),
  { ssr: false },
);
...
export default function AnalyticsPage() {
  ...
  const summaryQ = useQuery<ExecutiveSummary>({
    ...
    queryFn: () =>
      api.get<ExecutiveSummary>(`/analytics/executive?period=${period}`),
  });
          ...
          <>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onSelect={() => exportCsv("members")}>
                  Members
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => exportCsv("contributions")}>
                  Contributions
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => exportCsv("loans")}>
                  Loans
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => exportCsv("share_holdings")}>
                  Share holdings
                </DropdownMenuItem>
                <DropdownMenuItem onSelect={() => exportCsv("credit_scores")}>
                  Credit scores
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      {summaryQ.isError ? (
        ...
        <>
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
            <StatCard
              title="Total members"
              value={fmtInt(s.members.total)}
              description={`${fmtInt(s.members.active)} active`}
              icon={Users}
            />
            <StatCard
              title="New this period"
              value={fmtInt(s.members.joinedInPeriod)}
              description={periodLabel(period)}
              icon={Activity}
            />
            <StatCard
              title="Contributions"
              value={fmtMoney(s.contributions.periodAmount)}
              description={`${fmtInt(s.contributions.periodCount)} payments`}
              icon={TrendingUp}
            />
            <StatCard
            ...
            <StatCard
              ...
              value={fmtMoney(s.dividends.totalPaid)}
              description={
                s.dividends.lastDeclarationLabel
                  ? `Last: ${s.dividends.lastDeclarationLabel}`
                  : "No declarations yet"
              }
              icon={ReceiptText}
            />
            <StatCard
              title="Welfare fund"
              value={fmtMoney(s.welfare.poolBalance)}
              description={`${fmtInt(s.welfare.pendingRequests)} pending requests`}
              icon={Heart}
            />
            <StatCard
              ...
              value={
                s.creditScores.scoredMembers > 0
                  ? Number(s.creditScores.averageOverall).toFixed(0)
                  : "—"
              }
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                <span className="ml-2 text-xs font-normal text-muted-foreground">
                  Contributions + share capital − outstanding loans (welfare
            ...
            <CardContent>
              <div className="grid gap-3 md:grid-cols-3">
                <HealthMetric
                  label="Gross assets"
                  value={fmtMoney(s.financialHealth.grossAssets)}
                  tone="positive"
                />
                <HealthMetric
                  label="Liabilities"
                  value={fmtMoney(s.financialHealth.liabilities)}
                  tone="warning"
                />
                <HealthMetric
                  label="Net position"
                  value={fmtMoney(s.financialHealth.netPosition)}
                  tone={
                    Number(s.financialHealth.netPosition) >= 0
                      ? "positive"
                      : "negative"
                  }
                  highlight
              ...
              <CardContent>
                <div className="h-64">
                  {s.contributions.monthlyBuckets.length === 0 ? (
                    ...
                    <ContributionsChart
                      buckets={s.contributions.monthlyBuckets}
                      grain={s.grain}
                    />
                  )}
                </div>
              ...
              <CardContent>
                <div className="h-64">
                  {s.loans.monthlyRepayments.length === 0 ? (
                    ...
                    <RepaymentsChart
                      buckets={s.loans.monthlyRepayments}
                      grain={s.grain}
                    />
                  )}
                </div>
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <Card>
              ...
              <CardContent>
                <div className="h-64">
                  {Number(s.contributions.totalAmount) +
                    Number(s.shares.shareCapital) +
                    Number(s.loans.outstandingBalance) ===
                  0 ? (
