    ...
    <div className="space-y-6 max-w-3xl">
        ...
        <DialogContent className="max-w-sm">
          ...
          <form onSubmit={handleSubmit(onRepay)} className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1">
                <Label>
                  Receipt no. <span className="text-muted-foreground">(optional)</span>
                </Label>
                <Input {...register('mpesaReceiptNumber')} />
              </div>
            </div>
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setRepayOpen(false)}>
                Cancel
              </Button>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                ...
                if (!b2cPhone.trim()) {
                  toast({ variant: 'destructive', title: 'Enter the recipient phone' });
                  return;
                }
                if (!amt || amt <= 0) {
                  toast({ variant: 'destructive', title: 'Enter a whole-shilling amount' });
                  return;
                }
                setMpesaOpen(false);
      ...
      <Dialog open={disburseOpen} onOpenChange={setDisburseOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Records that {formatKES(l.principal_amount)} was handed to {l.member_name ?? 'the member'} outside M-Pesa
              and posts the disbursement to the books. Use &ldquo;Disburse via M-Pesa&rdquo; instead if the money should
              actually be sent.
            </p>
            <div className="space-y-1">
      ...
      <Dialog open={defaultOpen} onOpenChange={setDefaultOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Flags this loan as uncollectible under normal repayment. It can then be written off by a different
              officer.
            </p>
            <div className="space-y-1">
              <Label>Reason</Label>
      ...
      <Dialog open={writeOffOpen} onOpenChange={setWriteOffOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Posts the outstanding balance ({formatKES(l.outstanding_balance ?? 0)}) to Loan Write-offs and removes it
              from Loans Receivable. This cannot be undone. Maker-checker: you cannot write off a loan you yourself
              marked defaulted.
            </p>
            <div className="space-y-1">
              <Label>Reason</Label>
