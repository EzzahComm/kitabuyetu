...
export default function LoanDetailPage() {
  ...
  const [disburseMethod, setDisburseMethod] = useState<
    "cash" | "bank_transfer" | "cheque" | "standing_order"
  >("cash");
  ...
  return (
    <div className="space-y-6 max-w-3xl">
          ...
          <CardContent className="p-4 space-y-2">
            ...
            {l.repayment_frequency && l.repayment_frequency !== "monthly" && (
              <p className="text-xs text-muted-foreground">
                Repaid{" "}
                {l.repayment_frequency === "biweekly"
                  ? "every 2 weeks"
        ...
        <Card>
          <CardContent className="p-4 space-y-2">
            ...
            <p className="font-bold text-xl text-red-600">
              {formatKES(l.outstanding_balance ?? l.principal_amount)}
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <>
            ...
            <Button
              onClick={() => {
                ...
                setB2cAmount(
                  String(Math.round(Number(l.principal_amount ?? 0))),
                );
          ...
          <CardContent className="p-0">
            <PaginatedTable
              data={singlePage(
                schedule.map((s) => ({
                  ...s,
                  id: String(s.installment_number),
                })),
              )}
              isLoading={false}
              onPageChange={() => {}}
              ...
              columns={[
                {
                  ...
                  header: "#",
                  render: (s) => s.installment_number,
                },
                {
                  key: "due_date",
                  header: "Due Date",
                  render: (s) => formatDate(s.due_date),
                },
                {
                  key: "principal_component",
                  header: "Principal",
                  render: (s) => formatKES(s.principal_component),
                },
                {
                  key: "interest_component",
                  header: "Interest",
                  render: (s) => formatKES(s.interest_component),
                },
                {
                  key: "total_due",
                  header: "EMI",
                  className: "font-semibold",
                  render: (s) => formatKES(s.total_due),
                },
                {
                  key: "opening_balance",
                  header: "Balance",
                  render: (s) => formatKES(s.opening_balance),
                },
                {
                  key: "status",
                  header: "Status",
            ...
            <div className="space-y-1">
              ...
              >
                {unpaidSchedule.length === 0 && (
                  <option value="">No unpaid installments</option>
                )}
                {unpaidSchedule.map((row) => (
                  ...
                  >
                    #{row.installment_number} · due {formatDate(row.due_date)} ·{" "}
                    {formatKES(row.total_due ?? 0)}
                  </option>
                ))}
              </select>
              <p className="text-xs text-muted-foreground">
                Defaults to the earliest unpaid installment.
              </p>
            </div>
            <div className="space-y-1">
      ...
      <Dialog open={mpesaOpen} onOpenChange={setMpesaOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Disburse via M-Pesa (B2C)</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              Sends the loan amount straight to the member&apos;s phone. The
              loan flips to
              <span className="font-medium text-foreground">
                {" "}
                disbursed
              </span>{" "}
              when Safaricom confirms.
            </p>
            <div className="space-y-1">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setMpesaOpen(false)}>
              Cancel
            </Button>
            <Button
              onClick={() => {
                ...
                if (!b2cPhone.trim()) {
                  toast({
                    variant: "destructive",
                    title: "Enter the recipient phone",
                  });
                  return;
                }
                if (!amt || amt <= 0) {
                  toast({
                    variant: "destructive",
                    title: "Enter a whole-shilling amount",
                  });
                  return;
                }
                setMpesaOpen(false);
      ...
      <MoneyActionDialog
        ...
        onConfirm={async () => {
          const amt = parseInt(b2cAmount, 10);
          try {
            const res = await api.post<{ needsApproval: boolean }>(
              "/mpesa/b2c",
              {
                phone: b2cPhone.trim(),
                amount: amt,
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Reject this loan</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              The member will be notified that their application was rejected.
              The reason is recorded on the loan.
            </p>
            <div className="space-y-1">
              <Label>Reason</Label>
              <Textarea
      ...
      <Dialog open={disburseOpen} onOpenChange={setDisburseOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Mark this loan disbursed</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Records that {formatKES(l.principal_amount)} was handed to{" "}
              {l.member_name ?? "the member"} outside M-Pesa and posts the
              disbursement to the books. Use &ldquo;Disburse via M-Pesa&rdquo;
              instead if the money should actually be sent.
            </p>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                onChange={(e) =>
                  setDisburseMethod(e.target.value as typeof disburseMethod)
                }
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
              >
                <option value="cash">Cash</option>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setDisburseOpen(false)}>
              Cancel
            </Button>
            <Button
              disabled={!disburseDate}
              loading={loanAction.isPending}
              onClick={onDisburse}
            >
              Mark disbursed
      ...
      <Dialog open={defaultOpen} onOpenChange={setDefaultOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Mark this loan defaulted</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Flags this loan as uncollectible under normal repayment. It can
              then be written off by a different officer.
            </p>
            <div className="space-y-1">
              <Label>Reason</Label>
              <Textarea
                ...
                placeholder="Why is this loan in default?"
                rows={3}
              />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDefaultOpen(false)}>
              Cancel
            </Button>
            <Button
      ...
      <Dialog open={writeOffOpen} onOpenChange={setWriteOffOpen}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Posts the outstanding balance (
              {formatKES(l.outstanding_balance ?? 0)}) to Loan Write-offs and
              removes it from Loans Receivable. This cannot be undone.
              Maker-checker: you cannot write off a loan you yourself marked
              defaulted.
            </p>
            <div className="space-y-1">
              <Label>Reason</Label>
              <Textarea
                ...
                placeholder="Confirm why this debt is being written off"
                rows={3}
              />
            </div>
          </div>
