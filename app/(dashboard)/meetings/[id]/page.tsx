...
const resolutionSchema = z.object({
  resolutionText: z
    .string()
    .min(10, "Give the resolution at least 10 characters"),
...
export default function MeetingDetailPage() {
  ...
  const toggleImplemented = async (r: MeetingResolutionRow) => {
    try {
      ...
      toast({
        title: r.implemented ? "Marked outstanding" : "Marked implemented",
      });
    } catch (e) {
      toast({
        variant: "destructive",
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <CardContent className="p-4 grid sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">
              {meeting.is_virtual ? "Online" : "Venue"}
            </span>
              ...
              {resolutions.map((r) => (
                <li key={r.id} className="py-3 first:pt-0 last:pb-0 flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p
                      className={`text-sm ${r.implemented ? "text-muted-foreground line-through" : ""}`}
                    >
                      {r.resolution_text}
                    </p>
                    <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1 text-xs text-muted-foreground">
                      ...
                      {(r.votes_for > 0 ||
                        ...
                        <span>
                          ...
                          {r.votes_abstain} abstain
                        </span>
                      )}
                      {r.responsible_party_name && (
                        <span>Owner: {r.responsible_party_name}</span>
                      )}
                      {r.implementation_deadline && (
                        <span>Due {formatDate(r.implementation_deadline)}</span>
                      )}
                      {r.implemented && r.implemented_at && (
                        <span>Done {formatDate(r.implemented_at)}</span>
                      )}
                    </div>
                    {r.notes && (
                      <p className="text-xs text-muted-foreground mt-1">
      ...
      <ConfirmDialog
        ...
        confirmLabel="Start"
        onConfirm={() =>
          runUpdate({ status: "in_progress" }, "Meeting started")
        }
      />
      ...
      <ConfirmDialog
        ...
        variant="danger"
        onConfirm={() =>
          runUpdate({ status: "cancelled" }, "Meeting cancelled")
        }
      />
      ...
      <Dialog open={resolutionOpen} onOpenChange={setResolutionOpen}>
        <DialogContent className="max-w-lg">
          ...
          <form
            onSubmit={resolutionForm.handleSubmit(onAddResolution)}
            className="space-y-4"
          >
            <div className="space-y-1">
              ...
              {resolutionForm.formState.errors.resolutionText && (
                <p className="text-xs text-destructive">
                  {resolutionForm.formState.errors.resolutionText.message}
                </p>
              )}
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Input
                  ...
                  type="date"
                  {...resolutionForm.register("implementationDeadline")}
                />
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  ...
                  min="0"
                  placeholder="0"
                  {...resolutionForm.register("votesFor")}
                />
              </div>
              <div className="space-y-1">
            ...
            <DialogFooter>
              ...
              <Button
                type="submit"
                loading={resolutionForm.formState.isSubmitting}
              >
                Record resolution
              </Button>
            </DialogFooter>
