...
export default function MeetingsPage() {
  ...
  const columns = [
      ...
      render: (row: MeetingRow) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {new Date(row.scheduled_at).toLocaleTimeString("en-KE", {
              hour: "2-digit",
              minute: "2-digit",
            })}
          </p>
    ...
    },
    {
      ...
      header: "Chair",
      render: (row: MeetingRow) => (
        <span className="text-sm">{row.chaired_by_name ?? "—"}</span>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Resolutions",
      render: (row: MeetingRow) => (
        <span className="text-sm">{row.resolution_count ?? 0}</span>
      ),
    },
    {
      key: "status",
      ...
      render: (row: MeetingRow) => (
        <StatusPill
          status={row.status}
          tone={MEETING_STATUS_TONE[row.status]}
          size="sm"
        />
      ),
    },
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          description={`${stats?.completedMeetings ?? 0} completed`}
        />
        <Card>
        ...
        <StatCard
          ...
          value={`${stats?.avgAttendancePct ?? 0}%`}
        />
        <StatCard
          title="Resolutions"
          value={stats?.totalResolutions ?? 0}
          description={`${stats?.implementedResolutions ?? 0} implemented`}
        />
      </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1">
              ...
              <Input
                {...form.register("title")}
                placeholder="e.g. June 2026 Monthly Meeting"
              />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                {form.formState.errors.scheduledAt && (
                  <p className="text-xs text-destructive">
                    {form.formState.errors.scheduledAt.message as string}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input
                type="number"
                min={1}
                placeholder="Minimum members needed"
