...
export default function SharesPage() {
  ...
  const classesQ = useQuery<{ items: ShareClass[] }>({
    ...
    queryFn: () =>
      api.get<{ items: ShareClass[] }>("/share-classes?active=true"),
  });
  const holdingsQ = useQuery<PaginatedResult<Holding>>({
    queryKey: ["shares", "holdings"],
    queryFn: () =>
      api.get<PaginatedResult<Holding>>("/shares/holdings?limit=50"),
  });
  const ledgerQ = useQuery<PaginatedResult<ShareTxn>>({
    queryKey: ["shares", "ledger"],
    queryFn: () =>
      api.get<PaginatedResult<ShareTxn>>("/shares/transactions?limit=50"),
  });
        ...
        actions={
          <>
            <Button asChild variant="outline">
              <Link href="/shares/classes">
                <SettingsIcon size={16} className="mr-2" /> Share classes
              </Link>
            </Button>
            {canManage && (
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-10 text-center">
            ...
            <p className="max-w-md text-sm text-muted-foreground">
              Create at least one share class (e.g. &ldquo;Ordinary
              Shares&rdquo; at KES 100 par value) before you can allocate,
        ...
        <>
          ...
          <div className="grid gap-3 md:grid-cols-4">
            <StatCard
              title="Share capital"
              value={
                summaryQ.isLoading ? "—" : fmtMoney(summary?.totalShareCapital)
              }
              icon={Coins}
            />
            <StatCard
              title="Shares issued"
              value={
                summaryQ.isLoading ? "—" : fmtInt(summary?.totalShares ?? 0)
              }
              icon={TrendingUp}
            />
            <StatCard
              ...
              value={
                summaryQ.isLoading
                  ? "—"
                  : fmtInt(summary?.totalShareholders ?? 0)
              }
              icon={Users}
            />
            <StatCard
              title="Share classes"
              value={
                summaryQ.isLoading ? "—" : fmtInt(summary?.totalClasses ?? 0)
              }
              icon={Layers}
            />
          </div>
                      ...
                      <div className="mt-2 grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                        <div>
                          ...
                          <p className="text-sm text-foreground">
                            {fmtInt(c.sharesIssued)}
                          </p>
                        </div>
                        <div>
                          <p>Holders</p>
                          <p className="text-sm text-foreground">
                            {fmtInt(c.shareholders)}
                          </p>
                        </div>
                        <div>
                          <p>Capital</p>
                          <p className="text-sm text-foreground">
                            {fmtMoney(c.capitalAtValue)}
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              <PaginatedTable
                data={singlePage(
                  (holdingsQ.data?.items ?? []).map((h) => ({
                    ...h,
                    id: `${h.member_id}-${h.share_class_id}`,
                  })),
                )}
                ...
                columns={[
                    ...
                    render: (h) => (
                      <>
                        <p className="font-medium">
                          {h.member_first_name} {h.member_last_name}
                        </p>
                  ...
                  },
                  {
                    ...
                    header: "Class",
                    render: (h) => (
                      <Badge variant="outline">{h.share_class_code}</Badge>
                    ),
                  },
                  {
                    key: "shares",
                    ...
                    render: (h) => (
                      <span className="font-mono">{fmtInt(h.quantity)}</span>
                    ),
                  },
                  {
                    key: "invested",
                  ...
                  },
                  {
                    ...
                    render: (h) => {
                      const value =
                        h.quantity *
                        Number(h.share_class_current ?? h.share_class_par);
                      return (
                  ...
                  },
                  {
                    ...
                    render: (h) => {
                      ...
                      return (
                        <span
                          className={`font-mono ${appreciation > 0 ? "text-green-600" : appreciation < 0 ? "text-red-600" : ""}`}
                        >
                          {appreciation > 0 ? "+" : ""}
                          {fmtMoney(appreciation)}
                        </span>
                      );
                    },
                  },
                ...
                columns={[
                  {
                    ...
                    render: (t) => (
                      <span
                        className={`font-mono text-xs ${t.status === "reversed" ? "opacity-60 line-through" : ""}`}
                      >
                  ...
                  },
                  {
                    ...
                    render: (t) => (
                      ...
                      >
                        <Badge
                          variant={TYPE_BADGE[t.type] ?? "outline"}
                  ...
                  },
                  {
                    ...
                    render: (t) => (
                      ...
                      >
                        <p>
                          {t.member_first_name} {t.member_last_name}
                        </p>
                        {t.counterparty_first_name && (
                          <p className="text-xs text-muted-foreground">
                            ↔ {t.counterparty_first_name}{" "}
                            {t.counterparty_last_name}
                          </p>
                        )}
                      </span>
                    ),
                  },
                  {
                    ...
                    render: (t) => (
                      ...
                      >
                        <Badge variant="outline">{t.share_class_code}</Badge>
                      </span>
                    ),
                  },
                  {
                    ...
                    render: (t) => (
                      <span
                        className={`font-mono ${t.quantity > 0 ? "text-green-600" : "text-red-600"} ${t.status === "reversed" ? "opacity-60 line-through" : ""}`}
                      >
                        {t.quantity > 0 ? "+" : ""}
                        {fmtInt(t.quantity)}
                      </span>
                  ...
                  },
                  {
                    ...
                    render: (t) => (
                      <span
                        className={`font-mono ${t.status === "reversed" ? "opacity-60 line-through" : ""}`}
                      >
                        {fmtMoney(t.total_amount)}
                      </span>
                    ),
                  },
                  {
                ...
                columns={[
                  {
                    key: "rank",
                    header: "Rank",
                    render: (h) => <span className="font-mono">#{h.rank}</span>,
                  },
                  {
                    key: "member",
                    header: "Member",
                  ...
                  },
                  {
                    ...
                    className: "text-right",
                    render: (h) => (
                      <span className="font-mono">{fmtInt(h.totalShares)}</span>
                    ),
                  },
                  {
                    key: "totalInvested",
...
const txnSchema = z.object({
  ...
  quantity: z.coerce
    .number()
    .int()
    .refine((n) => n !== 0, "Quantity is required"),
  ...
  paymentMethod: z
    .enum(["mpesa", "cash", "bank_transfer", "cheque", "other"])
    .optional()
    .or(z.literal("")),
...
function NewTransactionDialog({
  ...
  const membersQ = useQuery<{ items: MemberRow[] }>({
    ...
    queryFn: () =>
      api.get<{ items: MemberRow[] }>("/members?status=active&limit=200"),
    enabled: open,
  });
  ...
  const onSubmit = async (values: TxnForm) => {
    try {
      ...
      const msg =
        err instanceof ApiError ? err.message : "Failed to post transaction";
  ...
  return (
    ...
    >
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>New share transaction</DialogTitle>
        </DialogHeader>
              ...
              >
                ...
                {classes.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name} ({c.code}) —{" "}
                    {fmtMoney(c.current_value ?? c.par_value)}
                  </option>
                ))}
              </select>
              {errors.shareClassId && (
                <p className="text-xs text-red-600">
                  {errors.shareClassId.message}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="memberId">
                {type === "transfer" ? "From member" : "Member"}
              </Label>
              <select
                id="memberId"
                {...register("memberId")}
            ...
            <div className="space-y-1">
              ...
              {type === "adjustment" && (
                <p className="text-xs text-muted-foreground">
                  Negative values reduce the holding.
                </p>
              )}
            </div>
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                type="number"
                step={0.01}
                placeholder="Defaults to class current/par value"
                {...register("unitPrice")}
              />
            </div>
          </div>
