...
const KIND_META: Record<
  Kind,
  { label: string; icon: LucideIcon; description: string; rollback: string }
> = {
  members: {
    label: "Members",
    icon: Users,
    description:
      "Import member roster. Each row creates a new member or links an existing one to this group.",
    rollback:
      "Hard delete created members (kept if they have downstream records).",
  },
  contributions: {
    label: "Contributions",
    icon: CreditCard,
    description:
      "Import historical contribution rows. Members must already exist in this group.",
    rollback:
      "Soft-cancel: rows marked status='cancelled' to preserve audit trail.",
  },
  loans: {
    label: "Loans",
    icon: Landmark,
    description:
      "Import historical loans with status active/completed/defaulted/written_off. Repayment schedule is NOT generated.",
...
export default function DataImportPage() {
    ...
    async (file: File) => {
      if (!file.name.toLowerCase().endsWith(".csv")) {
        toast({
          ...
          title: "CSV only",
          description:
            "Excel (.xlsx) support is queued. Save as CSV and try again.",
        });
        return;
      }
      setPhase("uploading");
      try {
        ...
        const result = await api.upload<ImportJob>(
          `/import/preview?type=${kind}`,
          formData,
        );
        setJob(result);
        setPhase("preview");
  ...
  const commit = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${result.imported ?? 0} row(s) added`,
      });
    } catch (err) {
      setPhase("preview");
  ...
  const rollback = async () => {
    if (!job) return;
    try {
      const result = await api.post<ImportJob>(`/import/${job.id}/rollback`, {
      ...
      toast({
        ...
        description: `${undoneCount} row(s) undone`,
      });
    } catch (err) {
      toast({
        variant: "destructive",
        ...
        <CardContent>
          ...
          >
            ...
            <p className="mt-3 font-medium">
              Drag &amp; drop your {KIND_META[kind].label.toLowerCase()} CSV
            </p>
            <p className="text-sm text-muted-foreground">or click to browse</p>
            <p className="mt-3 text-xs text-muted-foreground">
              Maximum 5MB · up to 5000 rows · CSV only
            </p>
            <input
              ref={inputEl}
              type="file"
      ...
      <Card>
        ...
        <CardContent className="space-y-3 text-sm">
          <p className="text-muted-foreground">
            Use the canonical column names. Headers are case-insensitive and
            accept common aliases.
          </p>
          <Button asChild variant="outline" className="w-full">
            <a href={`/api/v1/import/template?type=${kind}`} download>
              <Download className="mr-2 h-4 w-4" /> Download{" "}
              {KIND_META[kind].label.toLowerCase()} template
            </a>
          </Button>
        </CardContent>
      </Card>
...
function PreviewView({
  ...
  const rows = (job.preview_rows ?? []) as
    | MemberRow[]
    | ContributionRow[]
    | LoanRow[];
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-3">
        ...
        <SummaryCard
          ...
          valueClass={hasErrors ? "text-red-600" : ""}
        />
      </div>
            ...
            <ul className="max-h-64 space-y-1 overflow-y-auto text-sm">
              ...
              {job.errors.length > 100 && (
                <li className="italic text-muted-foreground">
                  …and {job.errors.length - 100} more
                </li>
              )}
            </ul>
          </CardContent>
      ...
      {visible.length > 0 && (
        <Card>
          ...
          <CardContent className="overflow-x-auto">
            ...
            {hiddenCount > 0 && (
              <p className="mt-3 text-xs text-muted-foreground">
                …and {hiddenCount} more row(s). All will be imported on confirm.
              </p>
            )}
          </CardContent>
        </Card>
...
function PreviewTable({
  ...
  kind: Kind;
  rows: MemberRow[] | ContributionRow[] | LoanRow[];
}) {
  if (kind === "members") {
    ...
    return (
      <PaginatedTable
        data={singlePage(
          memberRows.map((r) => ({ ...r, id: String(r.row_num) })),
        )}
        isLoading={false}
        onPageChange={() => {}}
        ...
        columns={[
          {
            ...
            header: "Row",
            render: (r) => (
              <span className="font-mono text-xs">{r.row_num}</span>
            ),
          },
          {
            key: "phone",
      ...
      <PaginatedTable
        ...
        columns={[
          ...
          },
          {
            key: "member_phone",
            header: "Phone",
            render: (r) => <span className="font-mono">{r.member_phone}</span>,
          },
          {
            key: "amount",
            header: "Amount",
    ...
    <PaginatedTable
      ...
      columns={[
        ...
        },
        {
          ...
          header: "Rate",
          className: "text-right",
          render: (r) => <span className="font-mono">{r.interest_rate}%</span>,
        },
        {
          key: "term_months",
          header: "Term",
          className: "text-right",
          render: (r) => <span className="font-mono">{r.term_months} mo</span>,
        },
        {
          key: "disbursement_date",
          header: "Disbursed",
          render: (r) => r.disbursement_date,
        },
        {
          key: "status",
          header: "Status",
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          {isRolledBack ? (
            <>
              <XCircle className="h-5 w-5 text-muted-foreground" /> Import
              rolled back
            </>
          ) : isCommitted ? (
            <>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 md:grid-cols-3">
          <StatCard
            title="Imported"
            value={job.imported ?? job.created_member_ids.length}
          />
          ...
          <SummaryCard
            label="Errors"
            value={job.error_rows}
            valueClass={job.error_rows > 0 ? "text-amber-600" : ""}
          />
        </div>

        {isRolledBack && (
          <p className="text-sm">
            {job.kind === "contributions" ? "Cancelled" : "Removed"}{" "}
            <strong>{undoneCount}</strong> row(s).
            {(job.blocked?.length ?? 0) > 0 && (
              <>
                {" "}
                <strong>{job.blocked!.length}</strong> blocked from rollback
                (check details below).
              </>
            )}
          </p>
        ...
        {isRolledBack && (job.blocked?.length ?? 0) > 0 && (
          <details className="rounded border p-3">
            <summary className="cursor-pointer text-sm font-medium">
              View {job.blocked!.length} blocked row(s)
            </summary>
        ...
        {job.errors.length > 0 && (
          <details className="rounded border p-3">
            <summary className="cursor-pointer text-sm font-medium">
              View {job.errors.length} issue(s)
            </summary>
            <ul className="mt-2 max-h-64 space-y-1 overflow-y-auto text-sm">
              {job.errors.map((e, i) => (
                <li key={i} className="font-mono">
