...
export default function DataImportPage() {
    ...
    async (file: File) => {
      if (!file.name.toLowerCase().endsWith('.csv')) {
        toast({
          variant: 'destructive',
          title: 'CSV only',
          description: 'Excel (.xlsx) support is queued. Save as CSV and try again.',
        });
        return;
      }
      setPhase('uploading');
  ...
  return (
    <div className="space-y-6">
      ...
      {phase === 'preview' && job && <PreviewView job={job} onCommit={commit} onDiscard={discard} />}
          ...
          { key: 'member_phone', header: 'Phone', render: (r) => <span className="font-mono">{r.member_phone}</span> },
          {
            ...
            render: (r) => (
              <span className="font-mono">
                {new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(r.amount)}
              </span>
            ),
          },
    ...
    <PaginatedTable
      ...
      columns={[
        { key: 'row_num', header: 'Row', render: (r) => <span className="font-mono text-xs">{r.row_num}</span> },
        {
          ...
          render: (r) => (
            <span className="font-mono">
              {new Intl.NumberFormat('en-KE', { style: 'currency', currency: 'KES' }).format(r.principal_amount)}
            </span>
          ),
        },
        {
