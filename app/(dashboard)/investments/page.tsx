...
export default function InvestmentsPage() {
  ...
  const columns = [
      ...
      render: (row: InvestmentRow) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {typeLabels[row.investment_type] ?? row.investment_type}
          </p>
        </div>
      ),
    },
      ...
      render: (row: InvestmentRow) =>
        row.current_value ? (
          <span className="font-semibold text-sm text-green-600">
            {formatKES(row.current_value)}
          </span>
        ) : (
          <span
            className="text-sm text-muted-foreground"
            title="Not revalued yet — shown at cost"
          >
            {formatKES(row.principal_amount)}
          </span>
        ),
    ...
    },
    {
      ...
      render: (row: InvestmentRow) => (
        <StatusPill
          status={row.status}
          tone={row.status === "pending_approval" ? "pending" : undefined}
          size="sm"
        />
      ),
    },
    {
      ...
      header: "Start Date",
      render: (row: InvestmentRow) => (
        <span className="text-xs">{formatDate(row.start_date)}</span>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <TabsList>
          {["all", "pending_approval", "active", "matured", "liquidated"].map(
            (s) => (
        ...
        <DialogContent className="max-w-lg">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1">
              ...
              <Input
                {...form.register("name")}
                placeholder="e.g. Nairobi Land Plot — Ruai"
              />
