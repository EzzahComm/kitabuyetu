...
import { z } from 'zod';
import { ArrowLeft, CheckCircle2, Loader2, Send, XCircle, Wallet, Coins } from 'lucide-react';
...
const fmtPct = (rate: string | number) => (Number(rate) * 100).toFixed(rate.toString().length > 5 ? 2 : 0) + '%';
...
export default function DividendDetailPage() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      toast({ variant: 'destructive', title: 'Failed', description: e instanceof ApiError ? e.message : '' });
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StatCard
          ...
          description={decl.total_eligible_members ? `${decl.total_eligible_members} members` : undefined}
        />
        <StatCard
          title="Paid"
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
          ...
          <Field
            ...
            value={
              decl.share_class_ids.length === 0 ? 'All active classes' : `${decl.share_class_ids.length} class(es)`
            }
          />
            ...
            <PaginatedTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (r) => <span className="font-mono">{r.sharesHeld}</span>,
                },
                {
                  key: 'gross',
                  header: 'Gross',
                  className: 'text-right',
                  render: (r) => <span className="font-mono">{fmtMoney(r.grossAmount)}</span>,
                },
                {
                  key: 'tax',
                  header: 'Tax',
                  className: 'text-right',
                  render: (r) => <span className="font-mono">{fmtMoney(r.taxAmount)}</span>,
                },
                {
                  key: 'net',
                  header: 'Net',
                  className: 'text-right',
                  render: (r) => <span className="font-mono font-medium">{fmtMoney(r.netAmount)}</span>,
                },
              ]}
            />
          ) : (
            <PaginatedTable
              ...
              columns={[
                ...
                },
                {
                  ...
                  className: 'text-right',
                  render: (a) => (
                    <span className={`font-mono ${a.status === 'cancelled' ? 'opacity-60' : ''}`}>{a.shares_held}</span>
                  ),
                },
                {
                  key: 'gross',
                  ...
                  className: 'text-right',
                  render: (a) => (
                    <span className={`font-mono ${a.status === 'cancelled' ? 'opacity-60' : ''}`}>
                ...
                },
                {
                  ...
                  className: 'text-right',
                  render: (a) => (
                    <span className={`font-mono font-medium ${a.status === 'cancelled' ? 'opacity-60' : ''}`}>
      ...
      <DialogContent>
        ...
        <form
          onSubmit={handleSubmit(async (v) => {
            await onConfirm(v.paymentMethod, v.paymentReference?.trim() || undefined);
            reset();
          })}
          className="space-y-3"
        >
    ...
    >
      <DialogContent>
        ...
        <p className="text-sm text-muted-foreground">
          Same payment method/reference is recorded against each selected allocation. Already-paid or cancelled rows are
          skipped.
        </p>
        <form
          onSubmit={handleSubmit(async (v) => {
