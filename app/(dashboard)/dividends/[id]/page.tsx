...
export default function DividendDetailPage() {
  ...
  const showPreview =
    decl && (decl.status === "draft" || decl.status === "pending_approval");
  ...
  const allocsQ = useQuery<{ items: Allocation[] }>({
    ...
    queryFn: () =>
      api.get<{ items: Allocation[] }>(`/dividends/${id}/allocations`),
  ...
  const onApprove = async () => {
    ...
    try {
      ...
      toast({
        title: "Approved",
        description: "Allocations have been computed and persisted.",
      });
      await refresh();
    } catch (e) {
      toast({
  ...
  if (!decl) {
    return (
      <div>
        <p className="text-muted-foreground">Declaration not found.</p>
      </div>
    );
  }
  ...
  const pendingSelectableIds = allocItems
    .filter((a) => a.status === "pending")
    .map((a) => a.id);
  const allSelected =
    pendingSelectableIds.length > 0 &&
    pendingSelectableIds.every((id) => selected.has(id));
    ...
    <div className="space-y-6">
      <div className="flex items-start gap-3">
              ...
              {decl.status === "approved" && (
                <>
                  {canManageDividends && (
                    ...
                    >
                      <Wallet className="mr-2 h-4 w-4" /> Pay selected (
                      {selected.size})
                    </Button>
                  )}
                  {canApproveDividends && (
        ...
        >
          <StatusPill
            status={decl.status}
            tone={decl.status === "pending_approval" ? "pending" : undefined}
          />
        </PageHeader>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StatCard
          title="Allocated"
          value={fmtMoney(decl.total_allocated)}
          description={
            decl.total_eligible_members
              ? `${decl.total_eligible_members} members`
              : undefined
          }
        />
        <StatCard
          title="Paid"
          value={fmtMoney(decl.total_paid)}
          description={
            Number(decl.total_allocated) > 0
              ? `${Math.round((Number(decl.total_paid) / Number(decl.total_allocated)) * 100)}% of allocated`
              : undefined
          }
        />
        <StatCard
          title="Tax"
          value={fmtMoney(decl.total_tax)}
          description={`${fmtPct(decl.withholding_tax_rate)} rate`}
        />
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
          <Field
            label="Policy"
            value={
              decl.policy_type === "flat_per_member"
                ? "Flat per member"
                : decl.policy_type === "proportional_to_shares"
          ...
          <Field
            ...
            value={
              decl.share_class_ids.length === 0
                ? "All active classes"
                : `${decl.share_class_ids.length} class(es)`
            }
          />
          <Field
            label="Declared"
            value={new Date(decl.declared_at).toLocaleString()}
          />
          <Field
            label="Approved"
            value={
              decl.approved_at
                ? new Date(decl.approved_at).toLocaleString()
                : "—"
            }
          />
          <Field
            ...
            value={
              decl.snapshot_at
                ? new Date(decl.snapshot_at).toLocaleString()
                : "—"
            }
          />
          <Field
            label="Paid out"
            value={decl.paid_at ? new Date(decl.paid_at).toLocaleString() : "—"}
          />
          {decl.notes && <Field label="Notes" value={decl.notes} />}
          {decl.cancellation_reason && (
            <Field
              label="Cancellation reason"
              value={decl.cancellation_reason}
            />
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center justify-between gap-3">
            <span>
              {showPreview
                ? "Allocation preview (not yet committed)"
                : "Allocations"}
            </span>
            {showPreview && previewQ.data && (
              <span className="text-xs font-normal text-muted-foreground">
                Pool {fmtMoney(decl.pool_amount)} →{" "}
                {previewQ.data.totalEligibleMembers} member(s) · rounding
                remainder {fmtMoney(previewQ.data.roundingRemainder)}
              </span>
            )}
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {showPreview ? (
            <PaginatedTable
              data={singlePage(
                previewRows.map((r) => ({ ...r, id: r.memberId })),
              )}
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (r) => (
                    <span className="font-mono">{r.sharesHeld}</span>
                  ),
                },
                {
                  key: "gross",
                  ...
                  className: "text-right",
                  render: (r) => (
                    <span className="font-mono">{fmtMoney(r.grossAmount)}</span>
                  ),
                },
                {
                  key: "tax",
                  ...
                  className: "text-right",
                  render: (r) => (
                    <span className="font-mono">{fmtMoney(r.taxAmount)}</span>
                  ),
                },
                {
                  key: "net",
            ...
            <PaginatedTable
              ...
              columns={[
                  ...
                  ? [
                      {
                        ...
                        header: (
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked)
                                setSelected(new Set(pendingSelectableIds));
                  ...
                  : []),
                {
                  ...
                  render: (a) => (
                    <div
                      className={a.status === "cancelled" ? "opacity-60" : ""}
                    >
                      <p className="font-medium">
                        {a.member_first_name} {a.member_last_name}
                      </p>
                      <p className="font-mono text-xs text-muted-foreground">
                        {a.member_phone}
                      </p>
                ...
                },
                {
                  ...
                  render: (a) => (
                    <span
                      className={`font-mono ${a.status === "cancelled" ? "opacity-60" : ""}`}
                    >
                      {a.shares_held}
                    </span>
                  ),
                ...
                },
                {
                  ...
                  render: (a) => (
                    <span
                      className={`font-mono font-medium ${a.status === "cancelled" ? "opacity-60" : ""}`}
                    >
                      {fmtMoney(a.net_amount)}
                    </span>
                  ),
                },
                {
                  ...
                  render: (a) => (
                    <StatusPill
                      status={a.status}
                      tone={a.status === "reinvested" ? "info" : undefined}
                      className={a.status === "cancelled" ? "opacity-60" : ""}
                    />
                  ),
                },
                    ...
                    >
                      {a.payment_method ? (
                        <>
                          ...
                          {a.payment_reference && (
                            <p className="font-mono">{a.payment_reference}</p>
                          )}
                          {a.paid_at && (
                            <p>{new Date(a.paid_at).toLocaleDateString()}</p>
                          )}
                        </>
                      ) : (
                        "—"
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <XCircle className="h-5 w-5" /> Cancel declaration
          </DialogTitle>
        </DialogHeader>
      ...
      <DialogContent>
        ...
        <form
          onSubmit={handleSubmit(async (v) => {
            await onConfirm(
              v.paymentMethod,
              v.paymentReference?.trim() || undefined,
            );
            reset();
          })}
          className="space-y-3"
...
function BulkPayDialog({
  ...
  return (
    ...
    >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Bulk pay {count} allocation(s)</DialogTitle>
        </DialogHeader>
        <p className="text-sm text-muted-foreground">
          Same payment method/reference is recorded against each selected
          allocation. Already-paid or cancelled rows are skipped.
        </p>
        <form
          onSubmit={handleSubmit(async (v) => {
            await onConfirm(
        ...
        >
          ...
          <div className="space-y-1">
            ...
            <Input
              id="bulkRef"
              placeholder="Common ref applied to every paid row"
              {...register("paymentReference")}
            />
          </div>
          <DialogFooter>
