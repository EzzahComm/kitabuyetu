...
const ScoreHistoryChart = dynamic(
  () => import("./_charts").then((m) => m.ScoreHistoryChart),
  { ssr: false },
);
...
const COMPONENT_HINT: Record<string, string> = {
  contribution_consistency:
    "% of last 12 months with at least one completed contribution",
  loan_repayment:
    "% of repayments paid on or before due date · defaults cap at 30",
  ...
  dividend_participation:
    "Received at least one paid dividend in last 12 months",
  meeting_attendance:
    "% of meetings attended in last 12 months (excused excluded)",
  welfare_participation:
    "Contributed to welfare pool in last 12 months · 100/40 binary",
  leadership_role:
    "Officer role (admin/treasurer/secretary) = 100, member = 50",
};
...
export default function CreditScoreDetailPage() {
  ...
  const historyQ = useQuery<{ items: CreditScore[] }>({
    ...
    queryFn: () =>
      api.get<{ items: CreditScore[] }>(
        `/credit-scores/${id}/history?limit=24`,
      ),
  });
  ...
  const notFound =
    latestQ.error instanceof ApiError && latestQ.error.status === 404;
    ...
    <div className="space-y-6">
      <div className="flex items-start gap-3">
        ...
        <PageHeader
          ...
          description={
            !noScoreYet && latestQ.data ? latestQ.data.member_phone : undefined
          }
          actions={
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-10 text-center">
            ...
            <p className="max-w-md text-sm text-muted-foreground">
              Click <strong>Recompute now</strong> above to score this member
...
function ScoreDetail({
  ...
  const componentEntries = Object.entries(latest.components) as [
    string,
    ComponentScore,
  ][];
  ...
  return (
    <>
      <div className="grid gap-3 md:grid-cols-4">
        <StatCard
          title="Overall score"
          value={Number(latest.overall_score).toFixed(0)}
          description="/ 100"
        />
        <StatCard
          title="Financial"
          value={Number(latest.financial_score).toFixed(0)}
          description="/ 100"
        />
        <StatCard
          title="Social"
          value={Number(latest.social_score).toFixed(0)}
          description="placeholder (E6.2)"
        />
        <StatCard
