...
export default function CreditScoresPage() {
  ...
  const listQ = useQuery<PaginatedResult<CreditScore>>({
    ...
    queryFn: () =>
      api.get<PaginatedResult<CreditScore>>(
        `/credit-scores?page=${page}&limit=50`,
      ),
  });
  ...
  const [policyEdits, setPolicyEdits] = useState<
    Record<Tier, { min: string; loanMultiplier: string }>
  >({} as never);
  const setPolicy = useMutation({
    mutationFn: (thresholds: TierThreshold[]) =>
      api.put<TierPolicy>("/credit-scores/policy", { thresholds }),
    onSuccess: () => {
      ...
      toast({
        ...
        description:
          result.failed.length > 0
            ? `${result.failed.length} failed — check the audit log.`
            : undefined,
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <StatCard
          ...
          value={summaryQ.isLoading ? "—" : (summary?.averageOverall ?? "—")}
        />
        <StatCard
          ...
          value={
            summaryQ.isLoading
              ? "—"
              : `${summary?.scoredMembers ?? 0} / ${summary?.totalMembers ?? 0}`
          }
        />
        <StatCard
          ...
          value={
            summaryQ.isLoading
              ? "—"
              : `${(summary?.byTier.excellent ?? 0) + (summary?.byTier.good ?? 0)}`
          }
        ...
        <StatCard
          ...
          value={
            summaryQ.isLoading
              ? "—"
              : `${(summary?.byTier.poor ?? 0) + (summary?.byTier.high_risk ?? 0)}`
          }
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ...
            and advisory loan limit (savings × multiplier). Overriding here only
            affects this group.
          </p>
        </CardHeader>
              ...
              <tbody>
                {(policyQ.data?.thresholds ?? []).map((t) => {
                  const edit = policyEdits[t.tier] ?? {
                    min: String(t.min),
                    loanMultiplier: String(t.loanMultiplier),
                  };
                  return (
                      ...
                      <td className="px-4 py-2">
                        <Input
                          ...
                          onChange={(e) =>
                            setPolicyEdits((prev) => ({
                              ...prev,
                              [t.tier]: { ...edit, min: e.target.value },
                            }))
                          }
                        />
                      </td>
        ...
        {policyQ.data && (
          <div className="flex items-center justify-between px-4 pb-4">
            <Badge
              variant={policyQ.data.source === "group" ? "success" : "outline"}
              className="text-xs capitalize"
            >
              {policyQ.data.source === "group"
                ? "Your override"
                : `Inherited — ${policyQ.data.source}`}
            </Badge>
            <Button
              size="sm"
              variant="outline"
              ...
              disabled={
                Object.keys(policyEdits).length === 0 || setPolicy.isPending
              }
              onClick={() => {
                const merged = (policyQ.data?.thresholds ?? []).map((t) => {
                  ...
                  return edit
                    ? {
                        tier: t.tier,
                        min: parseFloat(edit.min),
                        loanMultiplier: parseFloat(edit.loanMultiplier),
                      }
                    : t;
                });
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-10 text-center">
            ...
            <p className="max-w-md text-sm text-muted-foreground">
              Click <strong>Recompute all</strong> to score every active member
              based on their contribution, loan, share, and dividend history.
            </p>
          </CardContent>
        </Card>
      )}
        ...
        <PaginatedTable<CreditScore>
          ...
          columns={[
            ...
            },
            {
              ...
              render: (s) => (
                <span className="font-mono">
                  {Number(s.financial_score).toFixed(0)}
                </span>
              ),
            },
            {
            ...
            },
            {
              ...
              render: (s) => (
                <span className="font-mono">
                  {fmtMoney(s.loan_eligibility_limit)}
                </span>
              ),
            },
            {
