...
export default function WhatsAppPage() {
  ...
  const [recipientMode, setRecipientMode] = useState<"member" | "phone">(
    "member",
  );
  ...
  const logQ = useQuery<PaginatedResult<WhatsAppMessage>>({
    ...
    queryFn: () =>
      api.get<PaginatedResult<WhatsAppMessage>>(
        `/whatsapp/messages?page=${logPage}&limit=50`,
      ),
  });
    ...
    <div className="space-y-6">
      ...
      {!configured && (
        <Alert>
          ...
          <AlertDescription>
            Set{" "}
            <code className="rounded bg-amber-100 px-1">WHATSAPP_PHONE_ID</code>{" "}
            and
            <code className="ml-1 rounded bg-amber-100 px-1">
              WHATSAPP_ACCESS_TOKEN
            </code>{" "}
            in your environment to enable live sends. Messages submitted here
            are logged with status{" "}
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (m) => (
                    <StatusPill
                      status={m.status}
                      tone={m.status === "read" ? "positive" : undefined}
                      size="sm"
                    />
                  ),
                },
