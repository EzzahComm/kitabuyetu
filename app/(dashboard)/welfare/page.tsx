...
import {
  ...
  useReviewWelfareRequest,
  useWelfarePool,
  useRecordWelfarePoolContribution,
...
const poolSchema = z.object({
  ...
  contributionType: z
    .enum(["regular", "emergency_levy", "special", "bereavement_levy"])
    .default("regular"),
...
export default function WelfarePage() {
  ...
  const onApprove = async () => {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      toast({
        variant: "destructive",
        title: "Enter an approved amount",
    ...
    try {
      ...
      toast({
        ...
        description: `Approved ${formatKES(amount)}`,
      });
      closeReview();
    } catch (e) {
      toast({
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Member",
      render: (row: WelfareRequestRow) => (
        <span className="text-sm">{row.member_name}</span>
      ),
    },
    {
      key: "amount",
        ...
        <div className="text-right">
          ...
          {row.amount_approved && (
            <p className="text-xs text-green-600">
              Approved: {formatKES(row.amount_approved)}
            </p>
          )}
        </div>
      ),
    },
    {
      ...
      render: (row: WelfareRequestRow) => (
        <span
          className={`text-xs capitalize ${priorityClass[row.priority] ?? ""}`}
        >
          {row.priority}
        </span>
      ),
    },
    {
      key: "status",
      header: "Status",
      render: (row: WelfareRequestRow) => <StatusPill status={row.status} />,
    },
    {
      key: "created_at",
      header: "Date",
    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="pt-5">
            ...
            <p className="text-2xl font-bold mt-1 text-green-600">
              {formatKES(summary?.balance ?? 0)}
            </p>
            <p className="text-xs text-muted-foreground mt-1">
              Collected: {formatKES(summary?.totalCollected ?? 0)}
            </p>
          </CardContent>
        </Card>
        <StatCard
          title="Total Disbursed"
          value={formatKES(summary?.totalDisbursed ?? 0)}
        />
        <Card>
          <CardContent className="pt-5">
            <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
      ...
      <Dialog open={openRequest} onOpenChange={setOpenRequest}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Submit Welfare Request</DialogTitle>
          </DialogHeader>
          <form
            onSubmit={reqForm.handleSubmit(onSubmitRequest)}
            className="space-y-4"
          >
            <div className="space-y-1">
              <Label>Request Type</Label>
              <select
                {...reqForm.register("requestType")}
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                placeholder="e.g. Hospital bill support for John Doe"
              />
              {reqForm.formState.errors.title && (
                <p className="text-xs text-destructive">
                  {reqForm.formState.errors.title.message as string}
                </p>
              )}
            </div>
            <div className="space-y-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  type="number"
                  step="0.01"
                  {...reqForm.register("amountRequested")}
                />
              </div>
              <div className="space-y-1">
                <Label>Priority</Label>
                <select
                  {...reqForm.register("priority")}
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                >
      ...
      <Dialog open={openPool} onOpenChange={setOpenPool}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Record Welfare Fund Contribution</DialogTitle>
          </DialogHeader>
          <form
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input
                  type="number"
                  step="0.01"
                  {...poolForm.register("amount")}
                />
              </div>
              <div className="space-y-1">
                <Label>Type</Label>
                <select
                  {...poolForm.register("contributionType")}
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                >
              ...
              <div className="space-y-1">
                ...
                <select
                  {...poolForm.register("paymentMethod")}
                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                >
            ...
            <DialogFooter>
              <Button
                type="button"
                variant="outline"
                onClick={() => setOpenPool(false)}
              >
                Cancel
              </Button>
              <Button type="submit" loading={poolForm.formState.isSubmitting}>
                Record
              </Button>
            </DialogFooter>
          </form>
      ...
      {reviewRow && (
        <Dialog
          open={!!reviewRow}
          onOpenChange={(o) => {
            if (!o && !reviewBusy) closeReview();
          }}
        >
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Review welfare request</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="rounded-lg border bg-muted/40 p-3 text-sm">
                ...
                <p className="text-xs text-muted-foreground capitalize">
                  {reviewRow.request_type?.replace("_", " ")} ·{" "}
                  {reviewRow.member_name}
                </p>
                <dl className="mt-2 flex items-center justify-between gap-4">
                  <dt className="text-muted-foreground">Requested</dt>
                  <dd className="font-mono font-semibold">
                    {formatKES(reviewRow.amount_requested)}
                  </dd>
                </dl>
              </div>
              ...
              {!canManage && (
                <p className="text-xs text-destructive">
                  Requires treasurer or chairperson to approve or reject.
                </p>
              )}
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Pre-filled with the full request. Lower it to approve a
                  partial award.
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Required to reject — recorded on the request.
                </p>
              </div>
            </div>
            ...
            <DialogFooter className="gap-2 sm:gap-2">
              ...
              <Button
                ...
                title={
                  !canManage ? "Requires treasurer or chairperson" : undefined
                }
              >
                Reject
              </Button>
