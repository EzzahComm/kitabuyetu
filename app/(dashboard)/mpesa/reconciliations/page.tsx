...
export default function ReconciliationsPage() {
  ...
  const runNow = async () => {
    ...
    try {
      ...
      const typeLabel =
        reconType === "paybill" ? "Paybill sweep" : "STK reconciliation";
      toast({
        title: `${typeLabel} complete`,
        description: `${res.transactionsChecked} checked, ${res.resolvedCount} resolved`,
      });
          ...
          actions={
            <>
              ...
              <Button onClick={runNow} loading={running}>
                <Play size={15} className="mr-2" /> Run now
              </Button>
            </>
          }
        />
        ...
        columns={[
          {
            ...
            render: (r) => (
              <StatusPill
                status={r.status}
                tone={r.status === "running" ? "pending" : undefined}
                size="sm"
              />
            ),
          },
          {
            ...
            header: "Checked",
            render: (r) => r.transactions_checked,
          },
          {
            key: "mismatches_found",
            header: "Mismatches",
            render: (r) => r.mismatches_found,
          },
          {
            key: "resolved_count",
            header: "Resolved",
            className: "font-medium",
            render: (r) => r.resolved_count,
          },
          {
            key: "started_at",
            header: "Started",
          ...
          },
          {
            ...
            header: "By",
            className: "text-muted-foreground",
            render: (r) => r.initiated_by_name ?? "Cron",
          },
        ]}
      />
    </div>
