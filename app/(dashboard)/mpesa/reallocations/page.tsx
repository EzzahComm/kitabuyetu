...
export default function ReallocationsPage() {
  ...
  const refresh = () =>
    qc.invalidateQueries({ queryKey: ["mpesa", "reallocations"] });
  ...
  const initiate = async () => {
    if (!contributionId || !toMemberId || reason.trim().length < 5) {
      toast({
        variant: "destructive",
        title:
          "Pick a contribution, a target member, and give a reason (5+ chars)",
      });
      return;
    }
    setBusy(true);
  ...
  const reject = async () => {
    ...
    setBusy(true);
    try {
      await api.post(`/mpesa/reallocations/${rejecting.id}`, {
        action: "reject",
                ...
                <CardContent className="py-4 flex items-center justify-between flex-wrap gap-3">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {row.from_member_name ?? "Unknown"} →{" "}
                      {row.to_member_name ?? "Unknown"}
                      {row.mpesa_receipt_number
                        ? ` · Receipt ${row.mpesa_receipt_number}`
                        : ""}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {row.reason} · by {row.initiated_by_name ?? "unknown"} ·{" "}
      ...
      <Dialog open={creating} onOpenChange={(o) => !o && setCreating(false)}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
              ...
              >
                ...
                {correctable.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.member_name} — KES {Number(c.amount).toLocaleString()} ·{" "}
                    {c.mpesa_receipt_number}
                  </option>
                ))}
              </select>
            ...
            <p className="text-xs text-muted-foreground">
              Corrections above your group&apos;s approval threshold need a
              second officer to approve.
            </p>
          </div>
          <DialogFooter>
      ...
      {approving && (
        <MoneyActionDialog
          ...
          details={[
            ...
            ...(approving.mpesa_receipt_number
              ? [{ label: "Receipt", value: approving.mpesa_receipt_number }]
              : []),
            {
              label: "Initiated by",
              value: approving.initiated_by_name ?? "—",
            },
