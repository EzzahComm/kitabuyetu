...
export default function MpesaPage() {
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery<
    PaginatedResult<MpesaTxn>
  >({
    queryKey: ["mpesa", "transactions", page, type, status, phone],
    queryFn: () =>
      api.get<PaginatedResult<MpesaTxn>>(
        `/mpesa/transactions?${qs.toString()}`,
      ),
  });
  ...
  const columns = [
    {
      ...
      render: (r: MpesaTxn) => (
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-[10px] ${r.direction === "inbound" ? "text-green-600" : "text-amber-600"}`}
          >
    ...
    },
    {
      ...
      header: "Amount",
      render: (r: MpesaTxn) => (
        <span className="font-semibold">{formatKES(Number(r.amount))}</span>
      ),
    },
    {
      key: "phone",
      header: "Phone",
      render: (r: MpesaTxn) => r.phone_number ?? "—",
    },
    {
      key: "receipt",
      header: "Receipt",
    ...
    },
    {
      ...
      render: (r: MpesaTxn) => (
        <div>
          <StatusPill
            status={r.status}
            tone={MPESA_STATUS_TONE[r.status]}
            size="sm"
          />
          {r.failure_reason && (
            <p className="text-[10px] text-destructive mt-0.5 max-w-[180px] truncate">
              {r.failure_reason}
            </p>
          )}
        </div>
    ...
    },
    {
      key: "date",
      header: "Date",
      render: (r: MpesaTxn) => formatDate(r.completed_at ?? r.created_at),
    },
  ];
