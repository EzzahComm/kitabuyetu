...
const schema = z
  .object({
    ...
    phone: z
      .string()
      .regex(/^(?:\+254|0)[17]\d{8}$/, "Valid Kenyan phone required"),
...
const OBJECTIVES: {
  value: NonNullable<FormValues["primaryObjective"]>;
  label: string;
}[] = [
...
const sectionTitle =
  "text-xs font-semibold uppercase tracking-wider text-brand-blue-500 pt-2";
...
function RegisterForm() {
  ...
  const product: SubscriptionProduct =
    searchParams.get("product") === "chama_reminder"
      ? "chama_reminder"
      : "kitabu_yetu";
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      const data = (await authApi.register({ ...body, product })) as Awaited<
        ReturnType<typeof authApi.register>
      > & {
        groupCode?: string;
        membershipNo?: string;
      ...
      toast({
        title:
          product === "chama_reminder"
            ? "Welcome to Chama Reminder!"
            : "Welcome to Kitabu Yetu!",
        description:
          `Your group is ${data.groupCode ?? "created"}.` +
          (data.membershipNo
            ? ` Your account number: ${formatMembershipNo(data.membershipNo)}.`
            : ""),
      });
      ...
      if (err instanceof ApiError && err.code === "DUPLICATE_PHONE") {
        const productLabel =
          product === "chama_reminder" ? "Chama Reminder" : "Kitabu Yetu";
        ...
        toast({
          ...
          description: (
            <>
              If your last attempt showed an error, your group may already have
              been created —{" "}
              <Link href="/login" className="underline">
                log in
              </Link>{" "}
              to check and finish verifying it. Already have a different group?
              Log in and use &ldquo;Create another group&rdquo; to add{" "}
              {productLabel}.
            </>
          ),
        });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. Bungoma West"
                {...register("subCountyText")}
              />
            </div>
            <div className="space-y-1.5">
              <Label>
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input
                placeholder="e.g. Sang'alo Plot 24"
                {...register("villageEstate")}
              />
            </div>
          </div>
          ...
          <p className={sectionTitle}>
            Meeting schedule{" "}
            <span className="lowercase font-normal text-muted-foreground normal-case">
              (optional)
            </span>
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <p className="text-xs text-muted-foreground">
                The person creating the group must hold one of the three
                mandatory officer positions.
              </p>
            </div>
          </div>
