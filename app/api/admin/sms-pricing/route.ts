...
export function POST(req: NextRequest) {
  return withPlatformRole(req, "super_admin", async (ctx) => {
    ...
    switch (body?.kind) {
      case "tier":
        return created(
          await smsPricingAdminService.createTier(
            ctx.userId,
            TierCreateSchema.parse(body),
          ),
        );
      case "package":
        return created(
          await smsPricingAdminService.createPackage(
            ctx.userId,
            PackageCreateSchema.parse(body),
          ),
        );
      case "provider_cost": {
      ...
      case "activate_tiers": {
        ...
        return ok(
          await smsPricingAdminService.setActiveTiers(
            ctx.userId,
            input.tierIds,
          ),
        );
...
export function PATCH(req: NextRequest) {
  return withPlatformRole(req, "super_admin", async (ctx) => {
    ...
    if (body?.kind === "package") {
      return ok(
        await smsPricingAdminService.updatePackage(
          ctx.userId,
          id,
          PackageUpdateSchema.parse(body),
        ),
      );
    }
    return ok(
      await smsPricingAdminService.updateTier(
        ctx.userId,
        id,
        TierUpdateSchema.parse(body),
      ),
    );
  });
}
