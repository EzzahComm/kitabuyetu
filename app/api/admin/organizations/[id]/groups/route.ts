...
import { assignGroupToOrganization, revokeGroupFromOrganization } from '@/lib/services/admin-organizations.service';
...
export function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  return withPlatformRole(req, 'super_admin', async (auth) => {
    ...
    const result = await assignGroupToOrganization(id, parsed.data.groupId, auth.userId, parsed.data.accessLevel);
    return ok(result);
  });
}
