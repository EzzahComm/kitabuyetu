...
export async function PATCH(
  ...
  return withOrganizationAccess(
    ...
    async (auth) => {
      ...
      return ok(
        await organizationFinanceService.updateProgramStatus(ctx, id, status),
      );
    },
  );
}
...
export async function POST(
  ...
  return withOrganizationAccess(req, "capital.product.manage", async (auth) => {
    ...
    if (body.action === "capitalize") {
      return ok(
        await organizationFinanceService.capitalizeProduct(ctx, id, body),
      );
    }
    return ok(
      await organizationFinanceService.decapitalizeProduct(ctx, id, body),
    );
  });
}
