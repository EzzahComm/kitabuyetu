...
export function PATCH(
  ...
  return withPlatformRole(req, "super_admin", async (auth) => {
    ...
    if (
      parsed.data.action === "allocate" &&
      (!parsed.data.groupId || !parsed.data.memberId)
    ) {
      return badRequest("groupId and memberId are required to allocate");
    }
    if (
      parsed.data.action === "activate_subscription" &&
      (!parsed.data.groupId || !parsed.data.planType || !parsed.data.product)
    ) {
      return badRequest(
        "groupId, planType and product are required to activate a subscription",
      );
    }
