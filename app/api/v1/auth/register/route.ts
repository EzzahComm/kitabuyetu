...
const REGISTRATION_FEE = parseInt(
  process.env.REGISTRATION_FEE_KES ?? "300",
  10,
);
  ...
  try {
    ...
    const { result, membershipNo } = await withAdminDb(async (client) => {
      const { rows } = await client.query<{
        register_group: RegisterGroupResult;
    ...
    try {
      ...
      logger.error(
        "[register] failed to persist refresh token (non-fatal)",
        redisErr,
      );
    }
    ...
    if (e?.code === "23505") {
      ...
      if (e.constraint?.includes("group_code")) {
        return errorResponse(
          "Group code collision — please retry",
