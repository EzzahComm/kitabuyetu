    ...
    try {
      ...
      const result = await withAdminDb(async (client) => {
        const { rows } = await client.query<{
          create_additional_group: CreateAdditionalGroupResult;
        }>(
      ...
      try {
        ...
        logger.error(
          "[create-group] failed to persist refresh token (non-fatal)",
          redisErr,
        );
      }
      ...
      if (e?.code === "P0002")
        return handleError(new NotFoundError("Active membership"));
