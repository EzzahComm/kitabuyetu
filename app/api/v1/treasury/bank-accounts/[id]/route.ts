...
export async function POST(
  ...
  return withPermission(req, "treasury.manage", async (auth) => {
    try {
      ...
      requirePermission(
        { role: auth.role, permissions: freshPermissions },
        "treasury.manage",
      );
      ...
      if (input.action === "activate")
        return ok(await groupBankAccountsService.activate(ctx, id));
      if (input.action === "reject")
        return ok(await groupBankAccountsService.reject(ctx, id, input.reason));
