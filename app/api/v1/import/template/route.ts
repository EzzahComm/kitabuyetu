...
export async function GET(req: NextRequest): Promise<Response> {
  return withAuth(req, async () => {
    const type = (req.nextUrl.searchParams.get("type") ??
      "members") as ImportKind;
    if (!IMPORT_KINDS.includes(type)) {
      return errorResponse(
        `Unsupported template type: ${type}. Supported: ${IMPORT_KINDS.join(", ")}.`,
        "VALIDATION_ERROR",
        422,
      );
    }
    ...
    const built =
      type === "members"
        ? buildMembersCsv(today)
        : type === "contributions"
          ? buildContributionsCsv(today)
...
function buildMembersCsv(today: string): { csv: string; filename: string } {
  ...
  return {
    csv:
      [
        headers.join(","),
        headers.map((h) => csvEscape(sample[h])).join(","),
      ].join("\n") + "\n",
    filename: "kitabuyetu-members-template.csv",
  };
}

function buildContributionsCsv(today: string): {
  csv: string;
  filename: string;
} {
