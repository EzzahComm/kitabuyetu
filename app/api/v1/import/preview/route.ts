...
export async function POST(req: NextRequest): Promise<Response> {
  return withPermission(req, "import.preview", async (auth) => {
    ...
    if (file.size > MAX_BYTES) {
      return errorResponse(
        `File too large: ${(file.size / 1024 / 1024).toFixed(1)}MB exceeds 5MB cap`,
        "VALIDATION_ERROR",
        413,
      );
    }
    ...
    const job =
      type === "members"
        ? await importService.previewMembers(ctx, buffer, filename)
        : type === "contributions"
          ? await importService.previewContributions(ctx, buffer, filename)
