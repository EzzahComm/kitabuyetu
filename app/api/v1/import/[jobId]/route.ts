...
export async function GET(req: NextRequest, { params }: RouteParams): Promise<Response> {
  ...
  return withAuth(req, async (auth) => {
    const job = await importService.getJob({ userId: auth.userId, groupId: auth.groupId, role: auth.role }, jobId);
    return ok(job);
  });
}
...
export async function DELETE(req: NextRequest, { params }: RouteParams): Promise<Response> {
  ...
  return withPermission(req, 'import.cancel', async (auth) => {
    await importService.cancelPreview({ userId: auth.userId, groupId: auth.groupId, role: auth.role }, jobId);
    return noContent();
  });
}
