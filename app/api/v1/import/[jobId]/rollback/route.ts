...
export async function POST(
  ...
  return withPermission(req, "import.rollback", async (auth) => {
    ...
    if (job.kind === "members")
      return ok(await importService.rollbackMembers(ctx, jobId, reason));
    if (job.kind === "contributions")
      return ok(await importService.rollbackContributions(ctx, jobId, reason));
    if (job.kind === "loans")
      return ok(await importService.rollbackLoans(ctx, jobId, reason));

    return errorResponse(
      `Unsupported import kind: ${job.kind}`,
      "VALIDATION_ERROR",
      422,
    );
  });
