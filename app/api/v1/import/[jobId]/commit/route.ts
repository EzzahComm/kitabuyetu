...
export async function POST(
  ...
  return withPermission(req, "import.commit", async (auth) => {
    ...
    if (job.kind === "members")
      return ok(await importService.commitMembers(ctx, jobId));
    if (job.kind === "contributions")
      return ok(await importService.commitContributions(ctx, jobId));
    if (job.kind === "loans")
      return ok(await importService.commitLoans(ctx, jobId));
