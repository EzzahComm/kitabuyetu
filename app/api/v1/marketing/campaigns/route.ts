...
import { createCampaign, listCampaigns } from '@/lib/services/marketing-campaigns.service';
...
export async function POST(request: NextRequest): Promise<Response> {
  return withPermission(request, 'crm.manage', async (auth) => {
    ...
    const { title, message, audience_id } = body;
    ...
    const ctx = { userId: auth.userId, groupId: auth.groupId, role: auth.role, organizationId: auth.organizationId };
    const campaign = await createCampaign(ctx, { title, message, audience_id });
