...
export async function GET(req: NextRequest): Promise<Response> {
  return withAuth(req, async (auth) => {
    ...
    const plans = Object.entries(PLAN_FEATURES[product]).map(
      ([plan, features]) => ({
        plan,
        product,
  ...
  return withPermission(req, "billing.manage", async (auth) => {
    ...
    return ok(
      await withTransaction(ctx, async (client) => {
        ...
        const activated = await billingService.activateSubscriptionForPayment(
          client,
          {
            groupId: auth.groupId,
            planType: input.planType,
        ...
        return (
          activated ??
          (await billingService.getSubscription(ctx, input.product))
        );
      }),
    );
  });
