    ...
    try {
      ...
      if (data.status !== "completed") {
        throw new ValidationError(
          `Only completed contributions have a receipt (this one is '${data.status}').`,
        );
      }
      ...
      const pdfBuffer = await renderToBuffer(
        ContributionReceipt({
          ...
          membershipNo: data.membership_no
            ? formatMembershipNo(data.membership_no)
            : null,
          amount: data.amount,
      ...
      return errorResponse(
        `Failed to generate receipt: ${(err as Error).message}`,
        "INTERNAL_ERROR",
        500,
      );
    }
