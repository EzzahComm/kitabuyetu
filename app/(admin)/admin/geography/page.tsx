  ...
  if (wards.length === 0) {
    return (
      <p className="p-3 text-xs text-muted-foreground">
        No groups recorded for this county yet.
      </p>
    );
  }
  return (
        ...
        <TableBody>
          {wards.map((w) => (
            <TableRow key={w.ward} className="hover:bg-transparent">
              ...
              <TableCell className="py-1.5 text-right text-sm">
                {Number(w.member_count).toLocaleString()}
              </TableCell>
  ...
  return (
    <TableHead className={className}>
      ...
      >
        ...
        <ArrowUpDown
          size={11}
          className={active ? "text-foreground" : "text-gray-300"}
        />
        {active && (
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <StatCard
          title="Counties with groups"
          value={`${totals.counties} / ${rows.length || 47}`}
          icon={MapPin}
        />
        <StatCard title="Total groups" value={totals.groups} />
        <StatCard
          title="Total member reach"
                  ...
                  return (
                    <Fragment key={county.county_id}>
                      <TableRow
                        ...
                        onClick={() =>
                          setExpanded(isOpen ? null : county.county_id)
                        }
                      >
