...
export default function SmsPricingPage() {
  ...
  const handleActivate = (tierIds: string[]) => {
    ...
    activate.mutate(tierIds, {
      ...
      onError: (e) =>
        toast({ variant: 'destructive', title: 'Could not switch price list', description: getErrorMessage(e) }),
    });
  };
  ...
  const handleSetRate = () => {
    ...
    setOrgRate.mutate(
      { organizationId: rateTarget.id, rate: Number(newRate) },
      {
        ...
        onError: (e) =>
          toast({ variant: 'destructive', title: 'Could not save rate', description: getErrorMessage(e) }),
      },
    );
  };
      ...
      <Tabs defaultValue="pricing">
        ...
        <TabsContent value="pricing" className="space-y-6 pt-4">
          ...
          <p className="-mt-4 text-xs text-muted-foreground">
            Recording a new cost closes the current one rather than overwriting it, so margin on past sales keeps using
            the cost that applied when they were sold.
          </p>
