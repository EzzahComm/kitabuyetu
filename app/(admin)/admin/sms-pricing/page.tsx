...
import {
  useSmsPricingConfig,
  useActivateSmsTiers,
  useSetSmsProviderCost,
  useSmsMargin,
  useAdminTopUpOrganizationSmsCredits,
  useSetOrganizationSmsRate,
...
export default function SmsPricingPage() {
  ...
  const handleSaveCost = () =>
    saveCost.mutate(Number(newCost), {
      onSuccess: () => {
        setNewCost("");
        toast({ title: "Provider cost recorded" });
      },
      onError: (e) =>
        toast({
          variant: "destructive",
  ...
  const handleActivate = (tierIds: string[]) => {
    if (!tierIds.length) {
      toast({
        ...
        description: isLoading
          ? "Still loading the price list — try again in a moment."
    ...
    activate.mutate(tierIds, {
      ...
      onError: (e) =>
        toast({
          variant: "destructive",
          title: "Could not switch price list",
          description: getErrorMessage(e),
        }),
    });
  };
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="pricing">
        ...
        <TabsContent value="pricing" className="space-y-6 pt-4">
              ...
              <div className="space-y-1">
                ...
                {data?.providerCost && (
                  <p className="text-xs text-muted-foreground">
                    In force since{" "}
                    {new Date(
                      data.providerCost.effective_from,
                    ).toLocaleDateString()}
                  </p>
                )}
              </div>
          ...
          <p className="-mt-4 text-xs text-muted-foreground">
            Recording a new cost closes the current one rather than overwriting
            it, so margin on past sales keeps using the cost that applied when
            they were sold.
          </p>
          ...
          <div className="space-y-3">
                              ...
                              {margin === null ? (
                                ...
                                >
                                  ...
                                  {loss && (
                                    <AlertTriangle
                                      size={13}
                                      className="ml-1 inline text-destructive"
                                    />
                                  )}
                                </span>
                              )}
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                onClick={() =>
                  handleActivate(tiersNamed((n) => n === "Standard"))
                }
              >
                Use the flat rate
              </Button>
              <Button
                ...
                disabled={isLoading || activate.isPending}
                onClick={() =>
                  handleActivate(tiersNamed((n) => n.startsWith("Volume")))
                }
              >
            ...
            <p className="text-xs text-muted-foreground">
              The whole set changes in one step. If the chosen bands overlap,
              nothing changes at all — a half-applied price list would leave
              some volumes unpriced.
            </p>
          </div>
          ...
          <div className="space-y-3">
            <SectionHeader
              title="Packages"
              subtitle="Fixed bundles customers can buy."
            />
            <Card>
              <CardContent className="overflow-x-auto p-0">
                <table className="w-full text-sm">
                        ...
                        <tr key={p.id} className="border-b last:border-0">
                          <td className="px-4 py-3 font-medium text-foreground">
                            ...
                            {p.is_recommended && (
                              <CheckCircle2
                                size={13}
                                className="ml-1 inline text-brand-600"
                              />
                            )}
                          </td>
                          <td className="px-4 py-3">
        ...
        <TabsContent value="revenue" className="space-y-6 pt-4">
            ...
            <Card>
              <CardContent className="py-4">
                ...
                <p className="text-xl font-semibold text-foreground">
                  {marginLoading
                    ? "—"
                    : formatKES(margin?.summary.revenue ?? 0)}
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="py-4">
                ...
                <p className="text-xl font-semibold text-foreground">
                  {marginLoading
                    ? "—"
                    : formatKES(margin?.summary.providerCost ?? 0)}
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="py-4">
                ...
                >
                  {marginLoading
                    ? "—"
                    : formatKES(margin?.summary.grossMargin ?? 0)}
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="py-4">
                ...
                <p className="text-xl font-semibold text-foreground">
                  {marginLoading ||
                  margin?.summary.marginPct === null ||
                  margin?.summary.marginPct === undefined
                    ? "—"
          ...
          {!marginLoading && (margin?.summary.creditsWithoutCost ?? 0) > 0 && (
            <p className="-mt-3 flex items-center gap-1.5 text-xs text-amber-600">
              ...
              {margin!.summary.creditsWithoutCost.toLocaleString()} sold credits
              have no recorded provider cost for their period — margin above is
              understated by whatever those cost.
            </p>
          )}
                    ...
                    {marginLoading ? (
                      ...
                      margin.topCustomers.map((g) => (
                        <tr key={g.groupId} className="border-b last:border-0">
                          ...
                          <td className="px-4 py-3">
                            {g.creditsSold.toLocaleString()}
                          </td>
                          <td className="px-4 py-3">{formatKES(g.revenue)}</td>
                          <td className="px-4 py-3">
                            {g.creditsConsumed.toLocaleString()}
                          </td>
                          <td
                            className={`px-4 py-3 ${g.grossMargin < 0 ? "text-destructive" : ""}`}
                          >
          ...
          <div className="space-y-3">
                    ...
                    {marginLoading ? (
                      ...
                      margin.byOrganization.map((o) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            {o.creditsConsumed.toLocaleString()}
                          </td>
                          <td className="px-4 py-3">
                            {o.currentBalance.toLocaleString()}
                          </td>
                          <td className="px-4 py-3">
                            {o.creditsPurchased.toLocaleString()}
                          </td>
            ...
            <p className="text-xs text-muted-foreground">
              Organizations have no real-time M-Pesa collection today — a top-up
              here (or one the organization&apos;s own coordinator records in
              the Funding Portal) is a manual entry, trusted the same way a
              group&apos;s general capital deposit already is. &quot;Set
              rate&quot; controls the negotiated per-SMS price a top-up&apos;s
              credits are computed at.
            </p>
          </div>
        </TabsContent>
      </Tabs>
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Top up {topUpTarget?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Credits are computed at this organization&apos;s current
              negotiated rate. Recorded as a manual top-up — no M-Pesa payment
              is collected here.
            </p>
          </div>
          <DialogFooter>
            ...
            <Button
              onClick={handleTopUp}
              loading={topUpOrg.isPending}
              disabled={
                !topUpAmount ||
                Number.isNaN(Number(topUpAmount)) ||
      ...
      >
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Set SMS rate for {rateTarget?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Applies to future top-ups only — past purchases keep the rate they
              were bought at.
            </p>
          </div>
          <DialogFooter>
