...
const SERVICE_GROUPS: {
  title: MonitoringDashboardPayload["services"][number]["group"];
...
export default function MonitoringPage() {
  const { data, isLoading, error } = useQuery<MonitoringDashboardPayload>({
    ...
    queryFn: () =>
      adminFetch<MonitoringDashboardPayload>(
        "/api/admin/dashboard?widget=monitoring_dashboard",
      ),
    staleTime: 60_000,
  ...
  const mpesaSuccess = mpesa.length
    ? mpesa.reduce((a, s) => a + s.success, 0) / mpesa.length
    : 0;
  ...
  const smsRate = smsUsage.sentToday
    ? (smsUsage.delivered / smsUsage.sentToday) * 100
    : 0;
    ...
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        actions={
          <Button
            ...
            size="sm"
            className={
              live ? "bg-blue-600 text-xs hover:bg-blue-700" : "text-xs"
            }
            onClick={() => setLive((l) => !l)}
          >
            {live ? (
              <>
                <Pause size={13} className="mr-1" /> Pause feed
              </>
            ) : (
              <>
                <Play size={13} className="mr-1" /> Resume feed
              </>
            )}
          </Button>
        }
      ...
      {degraded.length > 0 && (
        <Alert>
          ...
          <AlertDescription>
            {degraded.map((s) => s.name).join(", ")}. Monitoring upstream.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          ...
          description={
            isLoading
              ? "Loading…"
              : `${smsUsage.delivered.toLocaleString()} of ${smsUsage.sentToday.toLocaleString()}`
          }
          icon={MessageSquare}
          accent="purple"
        />
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <p className="text-xs text-muted-foreground">
            p95 latency and trailing success rate by service
          </p>
        </CardHeader>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <ChartCard
          title="Transaction value (today)"
          description="Hourly M-Pesa throughput"
          height={260}
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="mb-1 flex items-center justify-between text-xs">
                ...
                <span className="font-semibold text-gray-900">
                  {smsUsage.creditsRemaining.toLocaleString()} /{" "}
                  {smsUsage.creditsTotal.toLocaleString()}
                </span>
              </div>
              <Progress
                value={
                  smsUsage.creditsTotal > 0
                    ? (smsUsage.creditsRemaining / smsUsage.creditsTotal) * 100
                    : 0
                }
              />
            </div>
            <div className="grid grid-cols-3 gap-2 text-center">
              {[
                {
                  label: "Delivered",
                  value: smsUsage.delivered,
                  cls: "text-green-600",
                },
                {
                  label: "Pending",
                  value: smsUsage.pending,
                  cls: "text-amber-600",
                },
                {
                  label: "Failed",
                  value: smsUsage.failed,
                  cls: "text-red-600",
                },
              ].map((x) => (
                <div key={x.label} className="rounded-lg bg-muted/50 p-2">
                  <p className={`text-lg font-bold ${x.cls}`}>
                ...
                {isLoading ? (
                  ...
                  feed.map((tx, i) => {
                    ...
                    return (
                      <tr
                        key={tx.id}
                        className={`border-t transition-colors hover:bg-muted/30 ${i === 0 && live ? "animate-in fade-in slide-in-from-top-1" : ""}`}
                      >
                        <td className="px-4 py-2.5">
                          <span
                            className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[11px] font-semibold ${ts.cls}`}
                          >
                        ...
                        <td className="px-4 py-2.5 text-right">
                          <MoneyDisplay
                            amount={tx.amount}
                            size="sm"
                            color={tx.status === "failed" ? "red" : "default"}
                          />
                        </td>
                        <td className="px-4 py-2.5">
                          <StatusPill
                            status={tx.status}
                            tone={txnStatusTone[tx.status]}
                            label={tx.status}
                            size="sm"
                          />
                        </td>
      ...
      <div className="flex items-start gap-2 rounded-lg border border-dashed bg-muted/40 p-3 text-xs text-muted-foreground">
        ...
        <span>
          The monitoring page now uses the live admin dashboard payload for
          service health, SMS usage, and recent M-Pesa activity.{" "}
