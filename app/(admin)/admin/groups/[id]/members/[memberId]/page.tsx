    ...
    <div className="space-y-5">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          {
            label: profile.group_name ?? "Group",
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {profile.platform_role && profile.platform_role !== "member" && (
            <StatusPill
              ...
              tone="info"
              size="sm"
              label={profile.platform_role.replace("_", " ")}
            />
          )}
        </div>
      </PageHeader>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <StatCard
          title="Savings"
          value={formatKES(snapshot?.savings ?? 0)}
          icon={Wallet}
          iconClass="bg-green-50"
        />
        <StatCard
          title="Shares"
          value={formatKES(snapshot?.shares ?? 0)}
          icon={Coins}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <StatusPill
                    ...
                    label={
                      TIER_LABEL[creditScore.reliability_tier as CreditTier]
                    }
                  />
                </div>
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {Number(creditScore.financial_score).toFixed(0)}
                    </p>
                  </div>
                  <div>
                    <p className="text-gray-400">Social</p>
                    <p className="font-medium text-gray-900">
                      {Number(creditScore.social_score).toFixed(0)}
                    </p>
                  </div>
                </div>
                <div className="pt-2 border-t border-gray-100 flex justify-between text-xs">
                  ...
                  <span className="font-semibold text-gray-900">
                    {formatKES(creditScore.loan_eligibility_limit)}
                  </span>
                </div>
                <p className="text-[11px] text-gray-400">
                  Computed {formatDate(creditScore.computed_at)}
                </p>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground text-center py-6">
                No credit score computed yet
              </p>
            )}
          </CardContent>
        </Card>
          ...
          {recentActivity.length === 0 ? (
            ...
            <div className="space-y-1">
              {recentActivity.map((row) => (
                <div
                  key={row.id}
                  className="flex items-center justify-between gap-3 py-1.5 border-b border-gray-50 last:border-0 text-xs"
                >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEdits({ ...edits, firstName: e.target.value })
                  }
                />
              </div>
              <div className="space-y-1">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEdits({ ...edits, lastName: e.target.value })
                  }
                />
              </div>
            </div>
            ...
            <div className="rounded-md border bg-muted/40 px-3 py-2 space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                The phone number ({profile.phone}) is the login identity and
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (edits.firstName !== (profile.first_name ?? ""))
                  body.firstName = edits.firstName;
                if (edits.lastName !== (profile.last_name ?? ""))
                  body.lastName = edits.lastName;
                if (edits.email !== (profile.email ?? ""))
                  body.email = edits.email || null;
