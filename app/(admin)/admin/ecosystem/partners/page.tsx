...
function PartnerCard({ partner }: { partner: Partner }) {
  ...
  const [form, setForm] = useState({
    name: partner.name, type: partner.type, description: partner.description ?? '',
    website_url: partner.website_url ?? '', contact_email: partner.contact_email ?? '', contact_phone: partner.contact_phone ?? '',
  });
    ...
    <Card>
      <CardContent className="flex items-start justify-between gap-3 p-5">
        <div className="min-w-0 flex-1">
          ...
          {partner.description && <p className="mt-1 line-clamp-2 text-sm text-muted-foreground">{partner.description}</p>}
        </div>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <div className="space-y-1.5">
                  ...
                  <Input id={`name-${partner.id}`} value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor={`type-${partner.id}`}>Type *</Label>
                  <Select value={form.type} onValueChange={(v) => setForm((f) => ({ ...f, type: v as Partner['type'] }))}>
                    <SelectTrigger id={`type-${partner.id}`}><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {(['donor', 'lender', 'insurer', 'trainer', 'service_provider', 'investor'] as Partner['type'][]).map((t) => (
                        <SelectItem key={t} value={t}>{t.replace('_', ' ')}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea id={`description-${partner.id}`} rows={3} value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor={`website-${partner.id}`}>Website</Label>
                  <Input id={`website-${partner.id}`} value={form.website_url} onChange={(e) => setForm((f) => ({ ...f, website_url: e.target.value }))} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label htmlFor={`email-${partner.id}`}>Contact email</Label>
                    <Input id={`email-${partner.id}`} type="email" value={form.contact_email} onChange={(e) => setForm((f) => ({ ...f, contact_email: e.target.value }))} />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor={`phone-${partner.id}`}>Contact phone</Label>
                    <Input id={`phone-${partner.id}`} value={form.contact_phone} onChange={(e) => setForm((f) => ({ ...f, contact_phone: e.target.value }))} />
                  </div>
                </div>
              </div>
              <DialogFooter>
