...
const PLATFORM_ROLE_BADGE: Record<
  string,
  ...
  const handleAssignRole = async () => {
    if (!roleUser?.group_id) {
      toast({
        ...
        title: "No group",
        description:
          "This member has no active group membership to assign a role in.",
      });
      return;
    }
    if (!selRoleId) return;
    try {
      ...
      const roleName =
        res?.newRole?.name ??
        assignableRoles.find((r) => r.id === selRoleId)?.name ??
        "role";
      toast({
  ...
  return (
    <div className="space-y-5">
        ...
        <CardContent className="pt-4">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            >
              ...
              <option value="organization_coordinator">
      ...
      <PaginatedTable<AdminUserRow>
        ...
        onRowClick={(u) => {
          if (u.group_id)
            router.push(`/admin/groups/${u.group_id}/members/${u.id}`);
          else
            toast({
          ...
          },
          {
            ...
            render: (u) =>
              u.platform_role && u.platform_role !== "member" ? (
                <Badge
                  variant={PLATFORM_ROLE_BADGE[u.platform_role] ?? "secondary"}
                  className="text-xs"
                >
              ...
              <div onClick={(e) => e.stopPropagation()}>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    >
                      <ShieldCheck size={13} className="mr-2" /> Change Platform
                      Role
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
            ...
            <div className="rounded-lg border border-gray-100 bg-gray-50 p-3 text-sm space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono text-xs text-gray-700">
                  {roleUser.member_code ?? "—"}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">Organization</span>
                <span className="text-gray-700">
                  {roleUser.organization_name ?? "—"}
                </span>
              </div>
              <div className="flex justify-between">
