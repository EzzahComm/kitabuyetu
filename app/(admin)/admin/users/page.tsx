...
import { useAdminUsers, useUpdateUserRole, useAssignableRoles, useAssignGroupRole } from '@/hooks/use-admin';
  ...
  const handleAssignRole = async () => {
    if (!roleUser?.group_id) {
      toast({
        variant: 'destructive',
        title: 'No group',
        description: 'This member has no active group membership to assign a role in.',
      });
      return;
    }
    if (!selRoleId) return;
    try {
      ...
      toast({
        ...
        description: `${roleUser.first_name} ${roleUser.last_name} in ${roleUser.group_name ?? 'group'}`,
      });
      setRoleUser(null);
      setSelRoleId('');
    } catch (e) {
      ...
      <PaginatedTable<AdminUserRow>
        ...
        onRowClick={(u) => {
          if (u.group_id) router.push(`/admin/groups/${u.group_id}/members/${u.id}`);
            ...
            toast({
              title: 'No active group membership',
              description: 'This user has no group to show a detail page for.',
            });
        }}
        columns={[
          {
          ...
          },
          {
            key: 'status',
            header: 'Status',
            render: (u) => <StatusPill status={u.status === 'active' ? 'active' : 'inactive'} size="sm" />,
          },
          {
            key: 'joined',
            header: 'Joined',
            hideBelow: 'md' as const,
            render: (u) => (
              <span className="text-xs text-muted-foreground">{formatDate(u.joined_at ?? u.created_at)}</span>
            ),
          },
          {
            key: 'actions',
              ...
              <div onClick={(e) => e.stopPropagation()}>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem disabled={!u.group_id} onClick={() => openAssignRole(u)}>
      ...
      >
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleRoleUpdate} loading={updateRole.isPending}>
              Update Role
            </Button>
          </DialogFooter>
        </DialogContent>
          ...
          <div className="space-y-1">
            ...
            {selRoleId &&
              (() => {
                ...
                return r?.description ? <p className="text-xs text-muted-foreground">{r.description}</p> : null;
              })()}
          </div>
