...
import {
  Search, Filter, Building2, MoreHorizontal,
...
export default function OrganizationsPage() {
  ...
  const { data, isLoading, isFetching } = useAdminOrganizations({ page, limit: 25, search, status, plan });
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      toast({ title: `Organization ${confirm.action}d successfully` });
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            {total.toLocaleString()} total · manage lifecycle, KYC, subscriptions
          </p>
        </div>
      </div>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 capitalize">{org.group_type?.replace('_', ' ')}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${PLAN_BADGE[org.plan] ?? 'bg-gray-100 text-gray-600'}`}>
                      {org.plan}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <Badge variant={STATUS_BADGE[org.onboarding_status] ?? 'secondary'} className="text-xs capitalize">
                      {org.onboarding_status?.replace('_', ' ')}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-right font-medium">{org.member_count}</td>
                  <td className="px-4 py-3 text-right text-green-600 font-medium">
                    {formatKES(org.total_contributions)}
                  </td>
                  <td className="px-4 py-3 text-right text-blue-600 font-medium">
                    {formatKES(org.active_loans)}
                  </td>
                  <td className="px-4 py-3">
                    <RiskBadge score={org.risk_score ?? 0} />
                  </td>
