...
const PLAN_TYPES: {
  value: "starter" | "growth" | "premium" | "premium_plus";
  label: string;
  fee: string;
}[] = [
  {
  ...
  const submitOnboard = async () => {
    ...
    if (planType === "premium_plus" && !(parseFloat(custom.monthlyFee) >= 0)) {
      toast({
        ...
        description: "Every term is negotiated per contract.",
      });
      return;
    }
    try {
        ...
        custom:
          planType === "premium_plus"
            ? {
                ...
                maxLinkedGroups: custom.maxLinkedGroups
                  ? parseInt(custom.maxLinkedGroups, 10)
                  : null,
                maxStaff: custom.maxStaff
                  ? parseInt(custom.maxStaff, 10)
                  : null,
                maxFundingPrograms: custom.maxFundingPrograms
                  ? parseInt(custom.maxFundingPrograms, 10)
                  : null,
                smsAllowanceIncluded: custom.smsAllowanceIncluded
                  ? parseFloat(custom.smsAllowanceIncluded)
                  : undefined,
      ...
      toast({
        title: `${form.name.trim()} onboarded on the ${PLAN_TYPES.find((p) => p.value === planType)?.label} plan`,
      });
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button
              size="sm"
              variant="outline"
              onClick={() => router.push("/admin/organizations/compare")}
            >
      ...
      <PaginatedTable<AdminOrgRow>
        ...
        columns={[
          ...
          },
          {
            ...
            render: (org) => (
              <span
                className={`text-xs font-semibold px-2 py-0.5 rounded-full ${TYPE_BADGE[org.type] ?? "bg-gray-100 text-gray-600"}`}
              >
          ...
          },
          {
            ...
            render: (org) => (
              <span className="font-medium">{org.group_count}</span>
            ),
          },
          {
            key: "memberReach",
              ...
              <div onClick={(e) => e.stopPropagation()}>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    >
                      <ArrowUpRight size={13} className="mr-2" /> View & assign
                      groups
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                    {org.is_active ? (
      ...
      >
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            ...
            {planType === "premium_plus" && (
              <div className="grid gap-3 sm:grid-cols-2 rounded-md border bg-muted/20 p-3">
                <p className="sm:col-span-2 text-xs text-muted-foreground">
                  Premium+ is negotiated per contract — every term below is
                  entered by hand. Blank limits mean unlimited.
                </p>
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    min={0}
                    value={custom.monthlyFee}
                    onChange={(e) =>
                      setCustom({ ...custom, monthlyFee: e.target.value })
                    }
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCustom({
                        ...custom,
                        supportTier: e.target
                          .value as typeof custom.supportTier,
                      })
                    }
                    className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                  >
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    min={1}
                    value={custom.maxLinkedGroups}
                    onChange={(e) =>
                      setCustom({ ...custom, maxLinkedGroups: e.target.value })
                    }
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    min={1}
                    value={custom.maxStaff}
                    onChange={(e) =>
                      setCustom({ ...custom, maxStaff: e.target.value })
                    }
                    placeholder="Unlimited"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    min={1}
                    value={custom.maxFundingPrograms}
                    onChange={(e) =>
                      setCustom({
                        ...custom,
                        maxFundingPrograms: e.target.value,
                      })
                    }
                    placeholder="Unlimited"
                  />
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="number"
                    min={0}
                    value={custom.smsAllowanceIncluded}
                    onChange={(e) =>
                      setCustom({
                        ...custom,
                        smsAllowanceIncluded: e.target.value,
                      })
                    }
                    placeholder="0"
                  />
          ...
          <DialogFooter>
            ...
            <Button onClick={submitOnboard} loading={createOrg.isPending}>
              Onboard
            </Button>
          </DialogFooter>
        </DialogContent>
        ...
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {confirm?.action === "deactivate" &&
              " Its coordinator loses portal access until reactivated."}
          </p>
          <DialogFooter>
