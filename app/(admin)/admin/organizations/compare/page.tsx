...
function healthBand(score: number) {
  if (score >= 80)
    return { label: "Strong", className: "text-green-600 bg-green-50" };
  if (score >= 50)
    return { label: "Watch", className: "text-amber-600 bg-amber-50" };
          ...
          {isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Onboard an organization to see it here.
              </p>
            </div>
          ) : (
            <Table>
              ...
              <TableBody>
                {rows.map((org) => {
                  const score =
                    org.avg_health_score !== null
                      ? Number(org.avg_health_score)
                      : null;
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell className="text-right">
                        {band ? (
                          <span
                            className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${band.className}`}
                          >
                            {band.label} {score}
                          </span>
                        ) : (
