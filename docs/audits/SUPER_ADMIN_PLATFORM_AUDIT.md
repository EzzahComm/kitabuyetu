...
- `app/(dashboard)/dashboard/page.tsx` (the _group officer's own_ landing page, a different portal entirely): cash/M-Pesa balance, welfare fund, external funding, member count, recent contributions — no shares, no loan portfolio total, no attendance.
...
**NOT BUILT in any form.** The ⌘K command palette (`components/admin/command-palette.tsx`) is a static list of ~15 hardcoded navigation shortcuts with client-side fuzzy-matching against their own labels — it never queries a table. The topbar's search box is a non-functional placeholder `<span>` that just opens the same static palette. Three _separate_, page-scoped searches exist today (organization name, group name, member name/email/phone) with no unified cross-entity endpoint behind any of them.

### 2.9 Platform Portfolio

**PARTIAL.** `getPlatformAnalytics()` (`admin.service.ts:886-939`) computes a genuine top-10-groups-by-contributions leaderboard and platform-wide loan/welfare aggregates — but there is no full, sortable, exportable table across _all_ organizations with growth/health/risk columns, and no organization-vs-organization comparison exists anywhere (see §2.11).

**Naming bug found in passing**: `admin/analytics/page.tsx` labels its 12-month trend chart "Organization Growth," but the underlying query (`admin.service.ts:895`) counts `groups`, not `organizations`. Same mislabeling on "Top Organizations by Contributions," which is actually top _groups_.
...
**Status (2026-07-29): shipped, in 3 commits on `feat/admin-unified-search`, not yet merged.**

- ~~One unified cross-entity search endpoint (organization/group/member by name, phone, membership number), wired into the ⌘K palette and topbar search box that already have the UI shell built and waiting~~ — done. New `searchPlatform()` (`lib/services/admin-search.service.ts`) runs parallel ILIKE queries over organizations/groups/members (joined through `group_members` for the `member_code` membership number) and feeds the existing palette shell's now-widened `useGroups(query)` signature.
- ~~An Organization Comparison view — feasible cheaply once Phase 2's health scores exist, since the financial rollups are already there~~ — done. `compareOrganizations()` (`lib/services/admin-organizations.service.ts`) reuses `listOrganizations`'s own group_count/member_reach/wallet SQL shape and adds each org's average governance health score, averaged across the groups it oversees via `organization_group_access`. Rendered at `/admin/organizations/compare`, reachable via a "Compare" button on the Organizations list.
- ~~County/ward-level aggregation using the geographic hierarchy that already exists — start with a sortable table before an interactive map~~ — done, as a sortable table, no map. `getCountyAggregation()`/`getWardAggregation()` (`lib/services/admin-geography.service.ts`) roll groups up by their `county_id` FK — the consistently-written jurisdiction column; `groups.ward_id`/`sub_county_id` are unpopulated dead schema, so ward-level grouping uses the free-text `groups.ward` column instead, scoped per county. Rendered at the new `/admin/geography` page (added to the sidebar), with click-to-expand rows lazily fetching each county's ward breakdown.
