...
**How to read this:** _Urgency_ = cost of leaving it as-is. _Complexity_ = work to close it. Two axes because they don't move together here — the most urgent item (the KES 15,000 payment) is also the _least_ complex (one factual answer), while the least urgent item (Chama Reminder Phases 5-6) is the most complex.
...
You asked for this explicitly _after_ the interest-rate correction, so members are never texted numbers about to change. That correction (migration 148, 10%/month flat) shipped and is live 2026-08-16 — this is now unblocked. Needs building (mirror `sendSubscriptionConfirmation`'s pattern) and your confirmation of channel wording (all 4 were paid in cash, so the message should say so, plus first-instalment amount + due date).
