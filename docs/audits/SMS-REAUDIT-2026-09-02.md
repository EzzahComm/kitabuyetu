...
and is correct, and still cannot tell whether its _inputs can ever satisfy it_. Only
...
T2-5 named the officer list as the item that _"unblocks the human workflow"_ and, with
...
its own `logger.error` _"the paging signal"_ while nothing consumes `logger.error`.
...
T3-5's closure test reads _"a DSAR for one member is answerable **from the UI**"_.
...
description claimed _"both closure tests verbatim"_ — **that claim was wrong**: the
integration test proves the _service_ answers a DSAR, not that the product does.
...
`syncCampaignCompletion()` is correct and works for campaigns completing _after_ it
...
But T3-1's closure metric — _"the count of rows stuck 'sent' >7 days trends to 0"_ — is now
**unmeetable by design**: those rows can never be polled again. The pathway proposed marking
messages _terminally-unknown_ past ~48h; what shipped instead filters them out. Same budget
...
| Cooldown has ever _fired_             | It can (F-evidence above), but reminder volume is low (37 rows lifetime) and the burst scenario needs several scanners due at once. Not observed firing.                                                    |
...
1. **PR #130 claimed both T3-5 closure tests were met.** One of them says _"from the UI"_
...
3. **The first migration-164 dry run reported success while its behavioural proof had
   ...
   fixture existed. _Absence of an exception is not evidence._
