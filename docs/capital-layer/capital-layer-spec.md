...
- **Phase 2** — allocation lifecycle on `organization_disbursements`; eligibility; schedule; `group_funding_sources`. *Accept when:* the reference scenario runs — EZZAHCOMM allocates KES 1,000,000 to The Fionas; org shows 9,000,000 available / 1,000,000 allocated; group shows the funding source; self-approval is rejected; an ineligible group is refused without an override and accepted with a logged one.
