...
describe("loan repayment frequency", () => {
  ...
  async function makeLoan(opts: {
    ...
    const [totals] = await rawQuery<{ total_repayable: string }>(
      `SELECT total_repayable FROM loans WHERE id = $1`,
      [loan.id],
    );
    ...
    return {
      id: loan.id,
      totalRepayable: Number(totals.total_repayable),
      installments,
    };
  }
  ...
  describe("invariant 2: cadence changes the split, not the price", () => {
    ...
    it.each<[Freq, number]>([
      ...
      "flat 10%% p.a. over 12 months costs 143,000 whether %s (%i instalments)",
      async (freq, n) => {
        const loan = await makeLoan({
          principal: 130_000,
          rate: 10,
    ...
    it("sums the instalments to exactly the total, with drift in the last one", async () => {
      ...
      const sum = loan.installments.reduce(
        (a, r) => a + Number(r.total_due),
        0,
      );
      expect(sum).toBeCloseTo(143_000, 2);
      expect(Number(loan.installments.at(-1)!.closing_balance)).toBeCloseTo(
        0,
        2,
      );
    });
  ...
  describe("due dates step by the cadence", () => {
    it("spaces weekly instalments 7 days apart", async () => {
      ...
      expect(
        new Date(loan.installments[0].due_date).toISOString().slice(0, 10),
      ).toBe("2026-02-07");
    });
    ...
    it("sticks to the 28th once a month-end date clamps through February", async () => {
      ...
      const dates = loan.installments.map((r) =>
