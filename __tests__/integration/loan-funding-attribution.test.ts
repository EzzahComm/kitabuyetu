...
describe("loan funding attribution", () => {
  ...
  beforeAll(async () => {
    ...
    await organizationFinanceService.deposit(orgCtx, {
      amount: 1_500_000,
      source: "Cash",
    });
    ...
    internalSourceId = sources.find(
      (s) => s.sourceType === "internal_savings",
    )!.id;
    allocationSourceId = sources.find(
  ...
  it("supports a blended loan drawn from both savings and organization capital", async () => {
    ...
    expect(
      splits.find((s) => s.sourceType === "organization_allocation")!.amount,
    ).toBe(60_000);
    expect(
      splits.find((s) => s.sourceType === "internal_savings")!.amount,
    ).toBe(40_000);
  });
  ...
  describe("the database invariant (the real enforcement)", () => {
    it("refuses to leave a disbursed loan partly attributed", async () => {
      ...
      await expect(
        rawQuery(`DELETE FROM loan_funding_splits WHERE loan_id = $1`, [
          loanId,
        ]),
      ).rejects.toThrow();
    });
    ...
    it("allows an approved (not yet disbursed) loan to have no splits at all", async () => {
      ...
      const rows = await rawQuery<{ n: string }>(
        `SELECT count(*) AS n FROM loan_funding_splits WHERE loan_id = $1`,
        [loanId],
      );
