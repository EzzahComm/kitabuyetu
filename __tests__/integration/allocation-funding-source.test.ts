...
describe("allocation → group funding source", () => {
  ...
  beforeAll(async () => {
    await resetDatabase();
    ({ organizationId: orgId, coordinatorId: coordId } =
  ...
  it("creates an organization_allocation funding source when a cash allocation settles", async () => {
    ...
    const fund = await organizationFinanceService.createProgram(
      orgCtx(coordId),
      {
        name: "Seed Capital",
    ...
    await organizationFinanceService.deposit(orgCtx(coordId), {
      amount: 1_500_000,
      source: "Cash",
    });
    ...
    if (disb.status === "pending_approval") {
      await organizationFinanceService.approveDisbursement(
        orgCtx(approverId),
        disb.id,
      );
    }
    ...
    const allocationSource = sources.find(
      (s) => s.sourceType === "organization_allocation",
    );
  ...
  it("leaves the group's internal savings source intact alongside it", async () => {
    ...
    expect(
      sources.some((s) => s.sourceType === "organization_allocation"),
    ).toBe(true);
  ...
  it("is idempotent — re-settling does not duplicate the funding source", async () => {
    ...
    await organizationFinanceService
      .approveDisbursement(orgCtx(approverId), disb.id)
      .catch(() => {});
  ...
  it("snapshots repayable terms onto the allocation, not a live join to the product", async () => {
    ...
    const facility = await organizationFinanceService.createProgram(
      orgCtx(coordId),
      {
    ...
    await rawQuery(
      `UPDATE funding_programs SET interest_rate_annual = 30 WHERE id = $1`,
      [facility.id],
    );
  ...
  describe("processing fee (migration 125) — deducted from what's disbursed", () => {
    it("nets the fee out of wallet cash while the group owes the full gross principal", async () => {
      ...
      const before = await organizationFinanceService.getWallet(
        orgCtx(coordId),
      );
      ...
      const walletDelta =
        ...
        parseFloat(after.available_balance);
      ...
      expect(parseFloat(groupCash.debit)).toBeCloseTo(
        parseFloat(row.net_disbursed_amount),
        2,
      );
    ...
    it("a grant (non-repayable) can still carry a processing fee", async () => {
      ...
      const grant = await organizationFinanceService.createProgram(
        orgCtx(coordId),
        {
