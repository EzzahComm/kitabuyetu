...
describe("Organization subscription plans", () => {
  ...
  describe("assignment", () => {
    it("snapshots the static Growth definition onto the row", async () => {
      const sub = await assignOrganizationPlan(
        organizationId,
        "growth",
        coordinatorId,
      );
    ...
    it('rejects premium_plus with no custom terms — "custom, including pricing" was explicit', async () => {
      await expect(
        assignOrganizationPlan(organizationId, "premium_plus", coordinatorId),
    ...
    it("a later assignment cancels the prior row rather than mutating it — the snapshot is never retroactively altered", async () => {
      const first = await assignOrganizationPlan(
        organizationId,
        "starter",
        coordinatorId,
      );
    ...
    it("writes an audit_logs row with old and new plan values", async () => {
      ...
      const [audit] = await rawQuery<{
        old_values: { plan_type: string };
        new_values: { plan_type: string };
      }>(
  ...
  describe("linked-group cap", () => {
    it("blocks at the cap and allows re-granting an already-active link", async () => {
      ...
      await expect(
        assignGroupToOrganization(organizationId, a.groupId, coordinatorId),
      ).resolves.toBeDefined();

      await expect(
        assignGroupToOrganization(organizationId, b.groupId, coordinatorId),
    ...
    it("a null cap (e.g. Premium) never blocks", async () => {
      ...
      for (const g of groups) {
        await expect(
          assignGroupToOrganization(organizationId, g.groupId, coordinatorId),
        ).resolves.toBeDefined();
      }
    });
    ...
    it("no plan assigned at all defaults to Starter caps (5), never unlimited", async () => {
      ...
      await rawQuery(
        `UPDATE organization_subscriptions SET status = 'cancelled' WHERE organization_id = $1`,
        [organizationId],
      );

      const groups = await Promise.all(
        Array.from({ length: 5 }, () => createTestGroup()),
      );
      for (const g of groups) {
      ...
      await expect(
        assignGroupToOrganization(organizationId, sixth.groupId, coordinatorId),
  ...
  describe("funding program cap", () => {
    ...
    it("blocks creating an active program past the cap", async () => {
      ...
      await organizationFinanceService.createProgram(
        ctxFor(coordinatorId, organizationId),
        grantInput,
      );
      await expect(
        organizationFinanceService.createProgram(
          ctxFor(coordinatorId, organizationId),
          { ...grantInput, name: "Second Grant" },
        ),
  ...
  describe("feature gates", () => {
    it("Starter cannot reach advanced reports; Growth can", async () => {
      await assignOrganizationPlan(organizationId, "starter", coordinatorId);
      await expect(
        organizationFinanceService.programBudgetReport(
          ctxFor(coordinatorId, organizationId),
        ),
    ...
    it("only Premium+ can set white-label branding — Premium itself cannot", async () => {
      await assignOrganizationPlan(organizationId, "premium", coordinatorId);
      await expect(
        organizationService.setBranding(ctxFor(coordinatorId, organizationId), {
          primaryColor: "#123456",
        }),
