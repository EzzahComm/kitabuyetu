...
describe("financial products", () => {
  ...
  beforeAll(async () => {
    await resetDatabase();
    ({ organizationId: orgAId, coordinatorId: coordAId } =
      await createTestOrganization());
    ({ organizationId: orgBId, coordinatorId: coordBId } =
      await createTestOrganization());
  });
  ...
  const seedCapitalInput = {
    ...
    tenorMonths: 12,
    repaymentWaterfall: {
      order: ["penalty", "interest", "principal"] as Array<
  ...
  describe("§1 reference scenario — EZZAHCOMM Seed Capital", () => {
    it("reports available 10,000,000 / allocated 0 for a product capitalized to 10,000,000", async () => {
      const product = await organizationFinanceService.createProgram(
        ctxA(),
        seedCapitalInput,
      );

      const [balances] = await organizationFinanceService.productBalances(
        ctxA(),
        product.id,
      );
  ...
  describe("capitalize / decapitalize", () => {
    ...
    it("lowers capital and records a decapitalization entry", async () => {
      ...
      await organizationFinanceService.decapitalizeProduct(ctxA(), product.id, {
        amount: 500_000,
      });
    ...
    it("does NOT move wallet cash — capitalization is a spending authority, not a cash event", async () => {
      ...
      expect(parseFloat(after.available_balance)).toBe(
        parseFloat(before.available_balance),
      );
      expect(parseFloat(after.total_deposited)).toBe(
        parseFloat(before.total_deposited),
      );
    });
    ...
    it("refuses to withdraw more than the uncommitted capital", async () => {
      ...
      await expect(
        organizationFinanceService.decapitalizeProduct(ctxA(), product.id, {
          amount: 150_000,
        }),
    ...
    it("works for an organization that has never opened a wallet", async () => {
      ...
      await expect(
        organizationFinanceService.capitalizeProduct(freshCtx, product.id, {
          amount: 9_000_000,
        }),
    ...
    it("rejects a non-positive adjustment", async () => {
      ...
      await expect(
        organizationFinanceService.capitalizeProduct(ctxA(), product.id, {
          amount: 0,
        }),
  ...
  describe("DB CHECK constraints (the real enforcement)", () => {
    ...
    it("rejects a repayable product with no tenor or waterfall", async () => {
      await expect(
        insertRaw(
          "is_repayable, interest_method, repayment_frequency",
          "true, $2, $3",
          ["flat", "monthly"],
        ),
      ).rejects.toThrow();
    ...
    it("rejects an unknown member_visibility", async () => {
      await expect(
        insertRaw("member_visibility", "$2", ["public"]),
      ).rejects.toThrow();
    });

    it("rejects a shared loss bearer with no ratio", async () => {
      await expect(
        insertRaw("loss_bearer", "$2", ["shared"]),
      ).rejects.toThrow();
    });
  ...
  describe("cross-organization isolation", () => {
    it("organization B cannot see or capitalize organization A's products", async () => {
      ...
      const bBalances =
        await organizationFinanceService.productBalances(ctxB());
      expect(bBalances.some((p) => p.programId === product.id)).toBe(false);

      await expect(
        organizationFinanceService.capitalizeProduct(ctxB(), product.id, {
          amount: 1_000,
        }),
      ).rejects.toThrow();
    });
