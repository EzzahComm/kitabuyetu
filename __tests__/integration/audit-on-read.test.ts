...
const readBudgetReport = (userId: string, organizationId: string) =>
  programsGet(
    buildRequest("/api/admin/organization/programs?report=budget", {
...
const readPlainList = (userId: string, organizationId: string) =>
  programsGet(
    buildRequest("/api/admin/organization/programs", {
      headers: headersFor(userId, organizationId),
    }),
  );
...
const readAuditLog = (userId: string, organizationId: string) =>
  auditLogsGet(
    buildRequest("/api/admin/organization/audit-logs", {
      headers: headersFor(userId, organizationId),
    }),
  );
...
describe("R11 — audit on significant organization reads", () => {
  ...
  it("records a report view AND the coordinator can actually read it back", async () => {
    ...
    expect((await readBudgetReport(coordinatorId, organizationId)).status).toBe(
      200,
    );
    ...
    const row = data.items.find(
      (r) => r.action === "organization.report.budget.view",
    );
  ...
  it("collapses repeat views inside the dedupe window into one row", async () => {
    ...
    const { data } = (await (
      await readAuditLog(coordinatorId, organizationId)
    ).json()) as { data: Paged };
  ...
  it("does NOT audit the plain program list — that is what the UI polls", async () => {
    ...
    expect((await readPlainList(coordinatorId, organizationId)).status).toBe(
      200,
    );
    ...
    expect(
      data.items.filter((r) => r.action.startsWith("organization.report.")),
    ).toHaveLength(0);
  });
  ...
  it("does not leak one organization's audit rows to another coordinator", async () => {
    ...
    const { data } = (await (
      await readAuditLog(b.coordinatorId, b.organizationId)
    ).json()) as { data: Paged };
    expect(
      data.items.filter((r) => r.actorId === a.coordinatorId),
    ).toHaveLength(0);
  });
});
