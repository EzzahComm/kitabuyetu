...
describe("SMS credit reservation (migration 123)", () => {
  ...
  describe("settle", () => {
    ...
    it("never drives a balance negative, so a post-send settle cannot fail", async () => {
      ...
      await expect(
        settleReservation([logId], "consume"),
  ...
  describe("bundled monthly allowance (migration 124)", () => {
    ...
    async function reservedLogsFor(
      ...
      const ids: string[] = [];
      for (let i = 0; i < res.fromAllowanceCount; i++)
        ids.push(await reservedLog(groupId, 1, {}, 1));
      for (let i = 0; i < res.fromPaidCount; i++)
        ids.push(await reservedLog(groupId, 1, {}, 0));
      return ids;
    }
