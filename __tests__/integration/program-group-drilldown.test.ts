...
describe("Funding-program group drill-down", () => {
  ...
  const call = (id: string, headers: Record<string, string>) =>
    programGroupsGet(
      buildRequest(`/api/admin/organization/programs/${id}/groups`, {
        headers,
      }),
  ...
  beforeAll(async () => {
    await resetDatabase();
    ({ organizationId: orgA, coordinatorId: coordA } =
      await createTestOrganization());
    ({ organizationId: orgB, coordinatorId: coordB } =
      await createTestOrganization());
  ...
  it("does NOT inflate disbursed by member count (fan-out regression guard)", async () => {
    ...
    const { data } = (await res.json()) as {
      data: { groups: GroupLine[]; incomplete: string[] };
    };
  ...
  it("reconciles with the program header — per-group disbursed sums to disbursed_total", async () => {
    ...
    const { data } = (await res.json()) as {
      data: {
        program: { disbursed_total: string };
        groups: GroupLine[];
        incomplete: string[];
      };
    };

    const summed = data.groups.reduce(
      (acc, g) => acc + parseFloat(g.disbursed),
      0,
    );
