...
describe("migration 169 — organization coordinator reads under app_tenant", () => {
  ...
  beforeAll(async () => {
    ...
    ({ groupId: linkedGroupId, officerId: linkedOfficerId } =
      await createTestGroup("chairperson", { subscribed: true }));
    ({ groupId: unlinkedGroupId, officerId: unlinkedOfficerId } =
      await createTestGroup("chairperson", { subscribed: true }));
    ({ groupId: revokedGroupId } = await createTestGroup("chairperson", {
      subscribed: true,
    }));
    ...
    await rawQuery(
      `UPDATE organization_group_access SET is_active = false WHERE group_id = $1`,
      [revokedGroupId],
    );
  ...
  it("is actually connected as app_tenant (no BYPASSRLS) — not the admin pool", async () => {
    const [role] = await withDb(
      coordinatorCtx(coordinatorId, orgId),
      async (c) => {
  ...
  it("sees group_members of a linked group, and no others, on a WHERE-less SELECT", async () => {
    ...
    expect(new Set(rows.map((r) => r.group_id))).toEqual(
      new Set([linkedGroupId]),
    );
  });
  ...
  it("sees group_members of an inactive membership too (portfolio health counts them)", async () => {
    ...
    try {
      ...
      await rawQuery(
        `UPDATE group_members SET is_active = true WHERE group_id = $1`,
        [linkedGroupId],
      );
    }
  });
  ...
  it("cannot see another organization's group_members", async () => {
    const rows = await withDb(
      coordinatorCtx(otherCoordinatorId, otherOrgId),
      async (c) => {
