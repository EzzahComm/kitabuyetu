...
describe("SMS credit ledger", () => {
  ...
  it("writes NO entry for a release, because it moves no money", async () => {
    ...
    await rawQuery(
      `SELECT settle_sms_credit_reservation($1::uuid[], 'release')`,
      [[logId]],
    );
  ...
  it("is append-only", async () => {
    ...
    await expect(
      rawQuery(
        `UPDATE sms_credit_ledger SET amount = 999 WHERE group_id = $1`,
        [groupId],
      ),
  ...
  it("seeds an opening balance so a pre-existing balance reconciles", async () => {
    ...
    await rawQuery(
      `UPDATE billing_accounts SET sms_credits = 55 WHERE group_id = $1`,
      [groupId],
    );
  ...
  it("records a system-driven top-up, where there is no human actor", async () => {
    ...
    await billingService.addSmsCredits(
      { userId: "system", groupId, role: "chairperson" },
      90,
    );
