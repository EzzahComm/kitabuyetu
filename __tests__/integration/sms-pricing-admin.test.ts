...
async function tierIdsNamed(names: string[]): Promise<string[]> {
  const rows = await rawQuery<{ id: string }>(
    `SELECT id FROM sms_pricing_tiers WHERE name = ANY($1::text[]) ORDER BY display_order`,
    [names],
  );
...
describe("super-admin SMS pricing controls (§12)", () => {
  ...
  describe("creating and editing", () => {
    ...
    it("can set a tier back to open-ended, which COALESCE alone cannot express", async () => {
      ...
      const updated = await smsPricingAdminService.updateTier(
        actorId,
        tier.id,
        { maxCredits: null },
      );
    ...
    it("keeps at most one recommended package", async () => {
      ...
      await smsPricingAdminService.updatePackage(actorId, rows[0].id, {
        isActive: true,
        isRecommended: true,
      });
      await smsPricingAdminService.updatePackage(actorId, rows[1].id, {
        isActive: true,
        isRecommended: true,
      });
  ...
  describe("provider cost", () => {
    it("closes the old window instead of overwriting it, so history holds", async () => {
      ...
      await smsPricingAdminService.setProviderCost(
        actorId,
        0.42,
    ...
    it("refuses a negative cost", async () => {
      await expect(
        smsPricingAdminService.setProviderCost(actorId, -1),
  ...
  describe("auditability (§12)", () => {
    it("records who changed what, for every kind of change", async () => {
      ...
      await smsPricingAdminService.updateTier(actorId, tier.id, {
        unitPrice: 0.22,
      });
      ...
      expect(
        await auditCount("sms_pricing.provider_cost_changed"),
      ).toBeGreaterThan(0);
