...
} from "@/lib/services/mpesa-c2b.service";
...
describe("C2B confirmation with a hashed MSISDN", () => {
  ...
  it("does not throw, and files an unroutable payment to mpesa_unrouted instead of discarding it", async () => {
    ...
    await expect(
      handleC2BConfirmation(
        c2bBody(receipt, "NOSUCHACCT", 250, HASHED_MSISDN),
        "196.201.214.200",
        { skipIpCheck: true },
      ),
  ...
  it(
    ...
    async () => {
      ...
      await expect(
        handleC2BConfirmation(
          c2bBody(receipt, "SUBSCRIPT", 150, HASHED_MSISDN),
          "196.201.214.200",
          { skipIpCheck: true },
        ),
      ).resolves.not.toThrow();

      const unrouted = await rawQuery(
        `SELECT id FROM mpesa_unrouted WHERE receipt = $1`,
        [receipt],
      );
