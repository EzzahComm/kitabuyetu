...
const call = (userId: string, organizationId: string) =>
  healthGet(
    buildRequest("/api/admin/organization/health", {
...
describe("Portfolio health indicators", () => {
  ...
  it("reports an empty denominator as null, never as 0%", async () => {
    ...
    const { data } = (await (
      await call(coordinatorId, organizationId)
    ).json()) as { data: Body };
  ...
  it("does not count another organization's arrears", async () => {
    ...
    const { data } = (await (
      await call(b.coordinatorId, b.organizationId)
    ).json()) as { data: Body };
