 ...
 */
import {
  GET as smsTemplatesGet,
  POST as smsTemplatesPost,
} from "@/app/api/v1/sms/templates/route";
import {
  GET as smsSchedulesGet,
  POST as smsSchedulesPost,
} from "@/app/api/v1/sms/schedules/route";
...
describe("SMS/Email messaging permission gates", () => {
  ...
  let memberPerms: string[],
    secretaryPerms: string[],
    treasurerPerms: string[],
    chairpersonPerms: string[];

  beforeAll(async () => {
    await resetDatabase();
    const { groupId: gId, officerId: founderId } =
  ...
  it("messaging.templates.view: member is denied, secretary can list SMS templates", async () => {
    const denied = await smsTemplatesGet(
      buildRequest("/api/v1/sms/templates", {
        headers: authHeaders({
          userId: memberId,
          groupId,
          role: "member",
    ...
    const allowed = await smsTemplatesGet(
      buildRequest("/api/v1/sms/templates", {
        headers: authHeaders({
          userId: memberId,
          groupId,
  ...
  it("messaging.templates.manage: secretary is denied, chairperson can create an SMS template", async () => {
    const body = {
      templateKey: "my_test_tpl",
      name: "Test",
      body: "Hello {{name}}",
      category: "custom",
    };
    const denied = await smsTemplatesPost(
      buildRequest("/api/v1/sms/templates", {
        method: "POST",
        body,
        headers: authHeaders({
    ...
    const allowed = await smsTemplatesPost(
      buildRequest("/api/v1/sms/templates", {
        method: "POST",
        body,
        headers: authHeaders({
  ...
  it("messaging.schedules.view/manage: member denied both, secretary can view, chairperson can create", async () => {
    const deniedView = await smsSchedulesGet(
      buildRequest("/api/v1/sms/schedules", {
        headers: authHeaders({
          userId: memberId,
          groupId,
          role: "member",
    ...
    const allowedView = await smsSchedulesGet(
      buildRequest("/api/v1/sms/schedules", {
        headers: authHeaders({
          userId: memberId,
          groupId,
    ...
    const deniedManage = await smsSchedulesPost(
      buildRequest("/api/v1/sms/schedules", {
        method: "POST",
        body: {
  ...
  it("messaging.manage: secretary cannot cancel an SMS campaign (fake id still proves the gate, not a 404-vs-403 ambiguity)", async () => {
    const res = await smsCampaignDelete(
      buildRequest(
        "/api/v1/sms/campaign?id=00000000-0000-0000-0000-000000000000",
        {
          method: "DELETE",
          headers: authHeaders({
            userId: memberId,
  ...
  it("messaging.templates.view (email): the previously-ungated GET /email/templates/[id] now denies a plain member", async () => {
    const res = await emailTemplateGet(
      buildRequest(
        "/api/v1/email/templates/00000000-0000-0000-0000-000000000000",
        {
          headers: authHeaders({
            userId: memberId,
            groupId,
  ...
  it("messaging.send (email): the previously-ungated GET /email/campaigns now denies a plain member, allows secretary", async () => {
    const denied = await emailCampaignsGet(
      buildRequest("/api/v1/email/campaigns", {
        headers: authHeaders({
          userId: memberId,
          groupId,
          role: "member",
    ...
    const allowed = await emailCampaignsGet(
      buildRequest("/api/v1/email/campaigns", {
        headers: authHeaders({
          userId: memberId,
          groupId,
  ...
  it("withAnyPermission composition: POST /email/schedules allows treasurer (treasury.manage) and chairperson (messaging.manage), denies secretary", async () => {
    ...
    const deniedSecretary = await emailSchedulesPost(
      buildRequest("/api/v1/email/schedules", {
        method: "POST",
        body,
        headers: authHeaders({
          userId: memberId,
    ...
    const allowedTreasurer = await emailSchedulesPost(
      buildRequest("/api/v1/email/schedules", {
        method: "POST",
        body,
        headers: authHeaders({
