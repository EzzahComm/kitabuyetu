...
describe("permissions claim tracks live role via auth_version (Batch 1 plumbing)", () => {
  ...
  it("demotes treasurer -> member mid-session: auth_version bumps for real, and the next refresh drops treasury.manage", async () => {
    ...
    expect((initialClaims as { permissions?: string[] }).permissions).toContain(
      "treasury.manage",
    );

    const [{ auth_version: authVersionBefore }] = await rawQuery<{
      auth_version: number;
    }>(
    ...
    const [{ auth_version: authVersionAfter }] = await rawQuery<{
      auth_version: number;
    }>(
      `SELECT auth_version FROM group_members WHERE group_id = $1 AND member_id = $2`,
      [groupId, memberId],
    ...
    const refreshedClaims = verifyAccessToken(
      refreshBody.data.accessToken as string,
    );

    expect(refreshedClaims).toMatchObject({ role: "member" });
    const refreshedPermissions =
      (refreshedClaims as { permissions?: string[] }).permissions ?? [];
