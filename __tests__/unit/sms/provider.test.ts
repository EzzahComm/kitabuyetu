...
describe("sms provider abstraction", () => {
  ...
  describe("routing to the adapter", () => {
    it("sendSingleSms calls through to textsms.service and returns its result", async () => {
      ...
      const res = await provider.sendSingleSms({
        mobile: "254700000001",
        message: "hi",
      });
    ...
    it("sendBulkSmsChunked calls through and returns its result", async () => {
      mockSendBulkSmsChunked.mockResolvedValueOnce({
        responses: [],
        sent: 0,
        failed: 0,
      });
    ...
    it("getDeliveryReport and getProviderBalance call through", async () => {
      mockGetDeliveryReport.mockResolvedValueOnce({
        messageId: "m1",
        status: "DeliveredToTerminal",
      });
      mockGetProviderBalance.mockResolvedValueOnce({
        balance: 100,
        currency: "KES",
      });
      ...
      await expect(provider.getProviderBalance()).resolves.toEqual({
        balance: 100,
        currency: "KES",
      });
    });
    ...
    it("throws ServiceUnavailableError without calling any adapter", async () => {
      await expect(
        provider.sendSingleSms(
          { mobile: "254700000001", message: "hi" },
