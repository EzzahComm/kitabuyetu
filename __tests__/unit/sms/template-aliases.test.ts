...
describe("template variable aliases", () => {
  ...
  it("leaves a genuinely unknown variable in place for stripUnresolved to handle", () => {
    expect(renderTemplate("{{not_a_variable}}", vars)).toBe(
      "{{not_a_variable}}",
    );
  });

  it("does not resolve an alias whose canonical is absent", () => {
    expect(renderTemplate("{{short_member_id}}", { first_name: "Mary" })).toBe(
      "{{short_member_id}}",
    );
  });
...
describe("platformPaybill", () => {
  ...
  it("prefers the working shortcode when set", () => {
    ...
    jest.resetModules();
    const mod =
      require("@/lib/sms/templates") as typeof import("@/lib/sms/templates");
...
describe("unresolvedVars agrees with renderTemplate", () => {
  ...
  it("reports a name nothing supplies, under either spelling", () => {
    expect(
      t.unresolvedVars("Receipt: {{receipt}}", { first_name: "Mary" }),
    ).toEqual(["receipt"]);
    expect(
      t.unresolvedVars("Due: {{amount_due}}", { first_name: "Mary" }),
    ).toEqual(["amount_due"]);
  });
  ...
  it("treats null and undefined as unresolved, exactly as the renderer does", () => {
    ...
    expect(t.renderTemplate(body, { balance: null })).toBe(
      "Balance {{balance}}",
    );
  });
