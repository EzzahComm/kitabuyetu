...
describe("client payload contracts", () => {
  describe("accounting: POST /accounting/journals", () => {
    ...
    it("rejects a single-line entry, which the row-remove button could produce", () => {
      expect(
        CreateJournalSchema.safeParse({ ...payload, lines: [payload.lines[0]] })
          .success,
      ).toBe(false);
    });
    ...
    it("rejects an unbalanced entry", () => {
      ...
      expect(CreateJournalSchema.safeParse({ ...payload, lines }).success).toBe(
        false,
      );
    });
  });
  ...
  describe("welfare: POST /welfare/[id] review", () => {
    it("accepts an explicit positive approved amount", () => {
      expect(
        ReviewWelfareRequestSchema.safeParse({
          action: "approve",
          amountApproved: 5000,
        }).success,
      ).toBe(true);
    ...
    it("accepts approve with the amount omitted (server falls back to amount_requested)", () => {
      expect(
        ReviewWelfareRequestSchema.safeParse({ action: "approve" }).success,
      ).toBe(true);
    });
  ...
  describe("loans: the loan detail/list page payloads", () => {
    ...
    it("rejects a reject with no reason — what the Reject button used to send", () => {
      ...
      expect(
        RejectLoanSchema.safeParse({ reason: "Insufficient savings history" })
          .success,
      ).toBe(true);
    });
  ...
  describe("accounting: posting-template overrides", () => {
    ...
    it.each(["loan_disbursement", "loan_repayment"])(
      "accepts the %s event",
      (event) => {
        expect(
          SetPostingTemplateSchema.safeParse({ event, lines }).success,
        ).toBe(true);
      },
    );
    ...
    it("still rejects an unknown event", () => {
      expect(
        SetPostingTemplateSchema.safeParse({ event: "not_an_event", lines })
          .success,
      ).toBe(false);
    });
  });
  ...
  describe("sms: POST /sms/bulk", () => {
    ...
    it("accepts a server-resolved audience, which is what the tab now sends", () => {
      expect(
        BulkSmsSchema.safeParse({ recipientType: "all_members", message })
          .success,
      ).toBe(true);
      expect(
        BulkSmsSchema.safeParse({ recipientType: "active_members", message })
          .success,
      ).toBe(true);
    });
    ...
    it('accepts an explicit list, which is what "Custom Phones" sends', () => {
      expect(
        BulkSmsSchema.safeParse({ phones: ["254712345678"], message }).success,
      ).toBe(true);
    });
    ...
    it("rejects an audience it cannot resolve", () => {
      ...
      expect(
        BulkSmsSchema.safeParse({ recipientType: "selected", message }).success,
      ).toBe(false);
    });
    ...
    it("still rejects a member-count key it does not understand as a recipient source", () => {
      ...
      expect(BulkSmsSchema.safeParse({ pageSize: 500, message }).success).toBe(
        false,
      );
    });
  });
