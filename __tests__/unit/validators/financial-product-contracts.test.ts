...
describe("CreateProgramSchema — non-repayable products (grants)", () => {
  ...
  it.each([
    ...
    expect(CreateProgramSchema.safeParse({ ...grant, ...extra }).success).toBe(
      false,
    );
  });
});
...
describe("CreateProgramSchema — repayable products", () => {
  ...
  it("rejects the source spec's 'declining_balance' — the codebase spells it 'reducing_balance'", () => {
    ...
    expect(
      CreateProgramSchema.safeParse({ ...seedCapital, interestMethod: "flat" })
        .success,
    ).toBe(true);
  });
  ...
  it("treats interestRateAnnual as a percentage, not a 0-1 ratio", () => {
    ...
    expect(
      CreateProgramSchema.safeParse({ ...seedCapital, interestRateAnnual: 36 })
        .success,
    ).toBe(true);
  });
});
...
describe("CreateProgramSchema — reserved but unimplemented options", () => {
  it.each([
    ...
    [
      "lossBearer organization",
      { lossBearer: "organization" as const, sharedLossRatio: undefined },
    ],
    [
      "lossBearer shared",
      { lossBearer: "shared" as const, sharedLossRatio: 0.5 },
    ],
    [
      "memberVisibility identified",
      { memberVisibility: "identified" as const },
    ],
    ["memberVisibility aggregate", { memberVisibility: "aggregate" as const }],
  ])(
    "rejects %s with an explicit message rather than half-working",
    (_label, extra) => {
      expect(
        CreateProgramSchema.safeParse({ ...grant, ...extra }).success,
      ).toBe(false);
    },
  );
...
describe("CreateProgramSchema — shared ratios", () => {
  it("requires revenueShareRatio when revenueOwner is 'shared'", () => {
    expect(
      CreateProgramSchema.safeParse({ ...grant, revenueOwner: "shared" })
        .success,
    ).toBe(false);
    expect(
      CreateProgramSchema.safeParse({
...
describe("RepaymentWaterfallSchema", () => {
  ...
  it("rejects a repeated component", () => {
    expect(
      RepaymentWaterfallSchema.safeParse({ order: ["interest", "interest"] })
        .success,
    ).toBe(false);
  });

  it("rejects an empty order", () => {
    expect(RepaymentWaterfallSchema.safeParse({ order: [] }).success).toBe(
      false,
    );
  });
...
describe("CapitalAdjustmentSchema / ProgramActionSchema", () => {
  it("requires a positive amount", () => {
    expect(
      CapitalAdjustmentSchema.safeParse({ amount: 10_000_000 }).success,
    ).toBe(true);
    expect(CapitalAdjustmentSchema.safeParse({ amount: 0 }).success).toBe(
      false,
    );
    expect(CapitalAdjustmentSchema.safeParse({ amount: -5 }).success).toBe(
      false,
    );
  });
  ...
  it.each(["capitalize", "decapitalize"])("accepts the %s action", (action) => {
    expect(
      ProgramActionSchema.safeParse({ action, amount: 1_000_000 }).success,
    ).toBe(true);
  });

  it("rejects an unknown action", () => {
    expect(
      ProgramActionSchema.safeParse({ action: "liquidate", amount: 1 }).success,
    ).toBe(false);
  });
});
