...
describe("SendSmsSchema recipient bounds (G9)", () => {
  ...
  it("accepts a single phone", () => {
    expect(SendSmsSchema.safeParse({ ...base, phone: VALID }).success).toBe(
      true,
    );
  });
  ...
  it("accepts a small array", () => {
    expect(
      SendSmsSchema.safeParse({ ...base, phone: [VALID, VALID_2] }).success,
    ).toBe(true);
  });

  it("accepts exactly the cap", () => {
    const phones = Array.from({ length: 10 }, () => VALID);
    expect(SendSmsSchema.safeParse({ ...base, phone: phones }).success).toBe(
      true,
    );
  });
  ...
  it("still rejects a malformed number inside an otherwise valid array", () => {
    expect(
      SendSmsSchema.safeParse({ ...base, phone: [VALID, "nonsense"] }).success,
    ).toBe(false);
  });
});
...
describe("rawRecipients audience validation (G10)", () => {
  ...
  it("does not force a PATCH to resend the audience", () => {
    ...
    expect(ScheduleUpdateSchema.safeParse({ name: "renamed" }).success).toBe(
      true,
    );
  });
...
describe("daily send limit is settable (G25)", () => {
  ...
  it('rejects null — the column is NOT NULL, so "no cap" is not storable', () => {
    ...
    expect(
      SmsGroupSettingsUpdateSchema.safeParse({ dailySendLimit: null }).success,
    ).toBe(false);
  });

  it("omits the key entirely when not supplied, so an unrelated toggle leaves the cap alone", () => {
    const r = SmsGroupSettingsUpdateSchema.safeParse({
      autoSendBirthday: true,
    });
    expect(r.success).toBe(true);
    if (r.success) {
      expect(
        Object.prototype.hasOwnProperty.call(r.data, "dailySendLimit"),
      ).toBe(false);
    }
  });
  ...
  it("rejects zero and negatives — use null for unlimited, not 0", () => {
    expect(
      SmsGroupSettingsUpdateSchema.safeParse({ dailySendLimit: 0 }).success,
    ).toBe(false);
    expect(
      SmsGroupSettingsUpdateSchema.safeParse({ dailySendLimit: -5 }).success,
    ).toBe(false);
  });

  it("rejects a non-integer cap", () => {
    expect(
      SmsGroupSettingsUpdateSchema.safeParse({ dailySendLimit: 2.5 }).success,
    ).toBe(false);
  });
});
