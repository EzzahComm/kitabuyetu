...
function req(headers: Record<string, string>): NextRequest {
  return new NextRequest(new URL("http://localhost/api/v1/loans/x"), {
    headers,
  });
}
...
describe("getAuthContext", () => {
  ...
  it("returns the auth context when required headers are present", () => {
    const auth = getAuthContext(
      req({ "x-user-id": "u1", "x-group-id": "g1", "x-role": "treasurer" }),
    );
  ...
  it("throws ForbiddenError when a tenant token (no x-aud: backoffice) hits a backoffice route", () => {
    expect(() =>
      getBackofficeContext(
        req({ "x-user-id": "u1", "x-platform-role": "super_admin" }),
      ),
    ).toThrow(ForbiddenError);
  });
