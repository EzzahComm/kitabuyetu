...
beforeEach(() => {
  ...
  (withTransaction as jest.Mock).mockImplementation((_ctx, fn) =>
    fn(mockClient),
  );
  (resolvePolicy as jest.Mock).mockReset();
  (resolvePolicyDetailed as jest.Mock).mockReset();
  (setPolicy as jest.Mock)
    .mockReset()
    .mockResolvedValue({ id: "p-1", version: 2 });
...
describe("buildTemplateLines", () => {
  it("maps a single-amount template to debit/credit lines", () => {
    const lines = buildTemplateLines(DEFAULT_TEMPLATES.share_purchase, {
      amount: 500,
    });
  ...
  it("drops zero-valued lines (dividend declaration without withholding posts two lines)", () => {
    const lines = buildTemplateLines(DEFAULT_TEMPLATES.dividend_declaration, {
      gross: 1000,
      net: 1000,
      tax: 0,
    });
  ...
  it("inverts every side for reversals", () => {
    const lines = buildTemplateLines(
      DEFAULT_TEMPLATES.share_purchase,
      { amount: 500 },
      { invert: true },
    );
    expect(lines).toEqual([
  ...
  it("throws when a referenced amount role was not supplied", () => {
    expect(() =>
      buildTemplateLines(DEFAULT_TEMPLATES.dividend_declaration, {
        gross: 1000,
        net: 1000,
      }),
  ...
  it("throws on a negative amount", () => {
    expect(() =>
      buildTemplateLines(DEFAULT_TEMPLATES.share_purchase, { amount: -5 }),
    ).toThrow(ValidationError);
  });
});
...
describe("postTemplatedJournal", () => {
  it("resolves the group template and posts the built lines in the same transaction", async () => {
    (resolvePolicy as jest.Mock).mockResolvedValueOnce(
...
describe("postLoanDisbursementJournal", () => {
  it("posts a 2-line entry (no charge)", async () => {
    (resolvePolicy as jest.Mock).mockResolvedValueOnce(
      DEFAULT_TEMPLATES.loan_disbursement,
    );
    mockQuery
      .mockResolvedValueOnce({
        rows: [{ member_id: "mem-1", group_membership_id: "gm-1" }],
      }) // member lookup
  ...
  it("folds the fee in when a charge is passed and the charge-role account exists", async () => {
    ...
    mockQuery
      .mockResolvedValueOnce({
        rows: [{ account_code: "5001" }, { account_code: "1001" }],
      }) // charge-account check
      .mockResolvedValueOnce({
        rows: [{ member_id: null, group_membership_id: null }],
      })
  ...
  it("falls back to principal-only when a charge exists but its account is missing from the chart", async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: [{ member_id: "mem-3", group_membership_id: null }],
      })
      .mockResolvedValueOnce({ rows: [] });
...
describe("postLoanRepaymentJournal", () => {
  it("posts a 4-line entry when there is an interest portion (principal + interest cash lines, both to 1001)", async () => {
    (resolvePolicy as jest.Mock).mockResolvedValueOnce(
      DEFAULT_TEMPLATES.loan_repayment,
    );
    mockQuery
      .mockResolvedValueOnce({
        rows: [{ member_id: "mem-1", group_membership_id: "gm-1" }],
...
describe("postingTemplatesService.setGroupOverride", () => {
  ...
  it("accepts an account remap that keeps the structure, after checking the group COA", async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [{ account_code: "1002" }, { account_code: "3001" }],
    });
    await postingTemplatesService.setGroupOverride(
      ctx,
      "share_purchase",
      remapped,
    );
  ...
  it("rejects an amount-role change (structure is locked)", async () => {
    ...
    await expect(
      postingTemplatesService.setGroupOverride(
        ctx,
        "share_purchase",
        roleChange,
      ),
  ...
  it("rejects an unbalanced side change (all lines on one side)", async () => {
    ...
    await expect(
      postingTemplatesService.setGroupOverride(ctx, "share_purchase", oneSided),
    ).rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it("rejects a line-count change", async () => {
    ...
    await expect(
      postingTemplatesService.setGroupOverride(ctx, "share_purchase", extra),
    ).rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it("rejects codes missing from the group chart of accounts", async () => {
    ...
    await expect(
      postingTemplatesService.setGroupOverride(ctx, "share_purchase", remapped),
    ).rejects.toBeInstanceOf(ValidationError);
...
describe("postingTemplatesService.setPlatformDefault", () => {
  ...
  it("accepts standard codes and writes at platform scope", async () => {
    ...
    await postingTemplatesService.setPlatformDefault(
      "admin-1",
      mockClient as never,
      "share_purchase",
      remap,
...
describe("postSettlementSweepJournal", () => {
  beforeEach(() => {
    (resolvePolicy as jest.Mock).mockResolvedValue(
      DEFAULT_TEMPLATES.settlement_sweep,
    );
  });
...
describe("postVendorPaymentJournal", () => {
  beforeEach(() => {
    (resolvePolicy as jest.Mock).mockResolvedValue(
      DEFAULT_TEMPLATES.vendor_payment,
    );
  });
  ...
  it("overrides the expense line with the row's own expense_account_code", async () => {
    ...
    expect(lines).toEqual(
      expect.arrayContaining([{ accountCode: "5001", debit: 10 }]),
    );
  });
