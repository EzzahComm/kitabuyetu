...
import {
  listAutomationRules, getAutomationRule, createAutomationRule, updateAutomationRule,
  listRuleExecutions, listFrequencyCaps, upsertFrequencyCap,
...
describe('createAutomationRule — validation', () => {
  it('rejects a blank name before any query runs', async () => {
    await expect(createAutomationRule(groupCtx, 'sms', { ...validSmsInput, name: '  ' }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects an unknown channel', async () => {
    await expect(createAutomationRule(groupCtx, 'whatsapp', validSmsInput))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects a malformed recipient_spec', async () => {
    await expect(createAutomationRule(groupCtx, 'sms', { ...validSmsInput, recipient_spec: { type: 'literally_anything' } }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects event_phone recipient_spec for the email channel (SMS-only spec)', async () => {
    await expect(createAutomationRule(groupCtx, 'email', { ...validSmsInput, recipient_spec: { type: 'event_phone', field: 'phone' } }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects a roles spec with an empty roles array', async () => {
    await expect(createAutomationRule(groupCtx, 'sms', { ...validSmsInput, recipient_spec: { type: 'roles', roles: [] } }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects conditions that are not {} or a single {field, op} leaf', async () => {
    await expect(createAutomationRule(groupCtx, 'sms', { ...validSmsInput, conditions: { all: [{ field: 'a', op: 'eq' }] } }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects delay_seconds out of range', async () => {
    await expect(createAutomationRule(groupCtx, 'sms', { ...validSmsInput, delay_seconds: -1 }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects a caller with neither a group nor an organization', async () => {
    await expect(createAutomationRule(noScopeCtx, 'sms', validSmsInput))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });
  ...
  it('rejects a template_key with no matching active template for the scope', async () => {
    ...
    await expect(createAutomationRule(groupCtx, 'sms', validSmsInput))
      .rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it('creates an organization-scoped email rule for a groupless organization context', async () => {
    ...
    const result = await createAutomationRule(orgCtx, 'email', {
      name: 'Newsletter', event_type: 'contribution.recorded', template_key: 'newsletter',
      recipient_spec: { type: 'active_members' },
    });
...
describe('updateAutomationRule — ownership + validation', () => {
  it('throws NotFoundError when the rule does not belong to the caller\'s own group', async () => {
    ...
    await expect(updateAutomationRule(groupCtx, 'sms', 'other-groups-rule', { is_active: false }))
      .rejects.toBeInstanceOf(NotFoundError);
  });

  it('rejects an update with an out-of-range max_retries before any query runs', async () => {
    await expect(updateAutomationRule(groupCtx, 'sms', 'rule-1', { max_retries: 11 }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });
  ...
  it('re-validates template_key against the templates table when it changes', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [] }); // assertTemplateExists finds nothing
    await expect(updateAutomationRule(groupCtx, 'email', 'rule-1', { template_key: 'ghost_template' }))
      .rejects.toBeInstanceOf(ValidationError);
  });
...
describe('frequency caps', () => {
  it('rejects max_per_day out of range before any query runs', async () => {
    await expect(upsertFrequencyCap(groupCtx, 'rule-1', { category: 'marketing', max_per_day: 0 }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });

  it('rejects a caller with no group (caps are group-scoped)', async () => {
    await expect(upsertFrequencyCap(orgCtx, 'rule-1', { category: 'marketing', max_per_day: 1 }))
      .rejects.toBeInstanceOf(ValidationError);
    expect(mockQuery).not.toHaveBeenCalled();
  });
  ...
  it('throws NotFoundError when the rule is not an email_trigger_rules row owned by this group', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [] });
    await expect(upsertFrequencyCap(groupCtx, 'sms-rule-1', { category: 'marketing', max_per_day: 1 }))
      .rejects.toBeInstanceOf(NotFoundError);
  });
