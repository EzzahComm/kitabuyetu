...
describe('disbursementsService.initiateDisbursement', () => {
  it('rejects when available balance is insufficient (C1)', async () => {
    ...
    await expect(disbursementsService.initiateDisbursement(ctx, input)).rejects.toBeInstanceOf(ValidationError);
  ...
  it('dispatches immediately when under the group threshold', async () => {
    ...
    mockQuery.mockResolvedValueOnce({
      rows: [{ organization_id: null, group_id: 'grp-1', value: { threshold: 20000 } }],
    }); // group threshold
    mockQuery.mockResolvedValueOnce({ rows: [] }); // reserve UPDATE
    mockQuery.mockResolvedValueOnce({
      // INSERT disbursement_requests
      rows: [
        {
          id: 'disb-2',
          group_id: 'grp-1',
  ...
  it('parks pending_approval above threshold and never dispatches (C3)', async () => {
    ...
    mockQuery.mockResolvedValueOnce({
      rows: [{ organization_id: null, group_id: 'grp-1', value: { threshold: 2000 } }],
    }); // low threshold
    mockQuery.mockResolvedValueOnce({ rows: [] }); // reserve UPDATE
    mockQuery.mockResolvedValueOnce({
      rows: [
  ...
  it('gates loan-linked disbursement on loan status = approved (F11)', async () => {
    ...
    await expect(disbursementsService.initiateDisbursement(ctx, { ...input, loanId: 'loan-1' })).rejects.toBeInstanceOf(
      ValidationError,
    );
    expect(initiateB2C).not.toHaveBeenCalled();
  });
...
describe('disbursementsService.approve', () => {
  it('refuses approval by the initiator (maker-checker, C3)', async () => {
    ...
    await expect(disbursementsService.approve(ctx, 'disb-1')).rejects.toBeInstanceOf(ForbiddenError);
    expect(initiateB2C).not.toHaveBeenCalled();
  });

  it('throws NotFoundError when no pending row matches', async () => {
    mockQuery.mockResolvedValueOnce({ rows: [] });
    await expect(disbursementsService.approve(ctx, 'missing')).rejects.toBeInstanceOf(NotFoundError);
  });
});
