...
describe("fiscalPeriodsService.close", () => {
  it("upserts a closed period row and writes an audit log", async () => {
    ...
    const result = await fiscalPeriodsService.close(ctx, {
    ...
    expect(insertCall[0]).toContain(
      "ON CONFLICT (group_id, period_start) DO UPDATE",
    );
...
describe("fiscalPeriodsService.reopen", () => {
  it("throws NotFoundError when the period does not exist", async () => {
    ...
    await expect(
      fiscalPeriodsService.reopen(ctx, "fp-1", {
        reason: "Correcting a posting error",
      }),
        ...
        rows: [
          {
            id: "fp-1",
            status: "open",
            reopen_reason: "Correcting a posting error",
          },
        ],
      })
      .mockResolvedValueOnce({ rows: [] }); // audit log

    const result = await fiscalPeriodsService.reopen(ctx, "fp-1", {
      reason: "Correcting a posting error",
    });
