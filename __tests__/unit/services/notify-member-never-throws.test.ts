...
jest.mock("@/lib/db", () => ({
  pool: {
    query: jest
      .fn()
      .mockResolvedValue({ rows: [{ id: "log-1" }], rowCount: 1 }),
  },
...
describe("notifyMember never throws", () => {
  ...
  it("suppresses rather than fails when credits are exhausted", async () => {
    ...
    mockPoolQuery.mockImplementation((sql: string) => {
      if (String(sql).includes("reserve_sms_credits")) {
        const err = Object.assign(new Error("insufficient SMS credits"), {
          code: "22003",
        });
  ...
  it("does not reserve anything when WhatsApp delivers", async () => {
    ...
    const reserved = mockPoolQuery.mock.calls.some((c) =>
      String(c[0]).includes("reserve_sms_credits"),
    );
