...
describe("evaluateCondition", () => {
  ...
  it("compares NUMERIC strings numerically, not lexically", () => {
    ...
    expect(
      evaluateCondition({ field: "amount", op: "gt", value: 900 }, payload),
    ).toBe(true);
    expect(
      evaluateCondition({ field: "amount", op: "gte", value: 1500 }, payload),
    ).toBe(true);
    expect(
      evaluateCondition({ field: "amount", op: "lt", value: 1500 }, payload),
    ).toBe(false);
  });

  it("equates 1500.00 with 1500 across string/number boundaries", () => {
    expect(
      evaluateCondition({ field: "amount", op: "eq", value: 1500 }, payload),
    ).toBe(true);
    expect(
      evaluateCondition({ field: "amount", op: "neq", value: 1500 }, payload),
    ).toBe(false);
  });

  it("does not coerce a non-numeric string into 0", () => {
    expect(
      evaluateCondition({ field: "status", op: "eq", value: 0 }, payload),
    ).toBe(false);
    expect(
      evaluateCondition({ field: "status", op: "gt", value: -1 }, payload),
    ).toBe(false);
  });

  it("distinguishes a zero value from an absent one", () => {
    expect(
      evaluateCondition({ field: "penalty", op: "eq", value: 0 }, payload),
    ).toBe(true);
    expect(evaluateCondition({ field: "penalty", op: "exists" }, payload)).toBe(
      true,
    );
  });

  it("treats null and missing fields as non-existent", () => {
    expect(evaluateCondition({ field: "phone", op: "exists" }, payload)).toBe(
      false,
    );
    expect(evaluateCondition({ field: "nope", op: "exists" }, payload)).toBe(
      false,
    );
    expect(
      evaluateCondition(
        { field: "phone", op: "exists", value: false },
        payload,
      ),
    ).toBe(true);
  });
  ...
  it("never matches a comparison against a null field", () => {
    expect(
      evaluateCondition({ field: "phone", op: "eq", value: "x" }, payload),
    ).toBe(false);
    expect(
      evaluateCondition({ field: "phone", op: "neq", value: "x" }, payload),
    ).toBe(false);
  });
  ...
  it("supports in / nin / contains", () => {
    ...
    expect(
      evaluateCondition(
        { field: "receipt", op: "contains", value: "qk12" },
        payload,
      ),
    ).toBe(true);
  });
  ...
  it("returns false for malformed conditions rather than throwing", () => {
    ...
    expect(
      evaluateCondition({ field: "amount", op: "regex", value: ".*" }, payload),
    ).toBe(false);
    expect(
      evaluateCondition([{ field: "amount", op: "eq", value: 1 }], payload),
    ).toBe(false);
  });
