...
import {
  ...
  type TierThreshold,
} from "@/lib/services/loan-policy.service";
...
describe("getEffectiveTierThresholds", () => {
  it("resolves via the generic engine with the domain-specific fallback", async () => {
    ...
    const result = await getEffectiveTierThresholds(mockClient as never, {
      groupId: "g1",
    });
...
describe("loanPolicyService.getGroupPolicy", () => {
  it("returns the resolved ladder with provenance", async () => {
    (resolvePolicyDetailed as jest.Mock).mockResolvedValueOnce({
      value: VALID_LADDER,
      source: "platform",
    });
...
describe("loanPolicyService.setGroupOverride", () => {
  it("rejects a ladder missing a tier", async () => {
    ...
    await expect(
      loanPolicyService.setGroupOverride(ctx, incomplete),
    ).rejects.toBeInstanceOf(ValidationError);
    expect(setPolicy).not.toHaveBeenCalled();
  });

  it("rejects a negative loanMultiplier", async () => {
    const bad = VALID_LADDER.map((t) =>
      t.tier === "poor" ? { ...t, loanMultiplier: -1 } : t,
    );
    await expect(
      loanPolicyService.setGroupOverride(ctx, bad),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects non-descending tier minimums", async () => {
    const bad = VALID_LADDER.map((t) =>
      t.tier === "fair" ? { ...t, min: 90 } : t,
    ); // fair(90) > excellent(85)
    await expect(
      loanPolicyService.setGroupOverride(ctx, bad),
    ).rejects.toBeInstanceOf(ValidationError);
  });
...
describe("loanPolicyService.setPlatformDefault", () => {
  it("validates and writes at platform scope (both org/group null)", async () => {
    await loanPolicyService.setPlatformDefault(
      "admin-1",
      mockClient as never,
      VALID_LADDER,
    );
...
describe("loanPolicyService.setGroupTermsOverride — term options", () => {
  ...
  it("rejects an option longer than maxTermMonths", async () => {
    ...
    await expect(
      loanPolicyService.setGroupTermsOverride(ctx as never, {
        ...VALID_TERMS,
        termOptions: [1, 3, 18],
      }),
    ).rejects.toBeInstanceOf(ValidationError);
