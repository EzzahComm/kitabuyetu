...
import {
  synthesise,
  type ComponentScore,
} from "@/lib/services/credit-scores.service";
    ...
    out[key] = {
      ...
      raw:
        key === "contribution_consistency"
          ? { total_completed_amount: totalSavings }
          : {},
    };
  }
  return out;
...
describe("synthesise", () => {
  ...
  it("honors a custom loanMultiplier at the same tier", () => {
    const richerLadder = DEFAULT_LADDER.map((t) =>
      t.tier === "fair" ? { ...t, loanMultiplier: 4 } : t,
    );
