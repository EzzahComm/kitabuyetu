...
describe("loansService.apply", () => {
  const membershipRow = {
    rows: [{ id: "gm-1", member_code: "KY000000000001" }],
  };
  ...
  it("throws ValidationError when the applicant has no active membership", async () => {
    ...
    await expect(loansService.apply(ctx, applyInput)).rejects.toBeInstanceOf(
      ValidationError,
    );
...
describe("loansService.approve", () => {
  it("throws NotFoundError when loan does not exist", async () => {
    ...
    await expect(
      loansService.approve(adminCtx, "non-existent", {}),
    ).rejects.toBeInstanceOf(NotFoundError);
  });

  it("throws ValidationError when loan is already approved (idempotency guard)", async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: "loan-1", status: "approved", group_id: "grp-1" }],
    });

    await expect(
      loansService.approve(adminCtx, "loan-1", {}),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("throws ValidationError when loan is already disbursed", async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: "loan-1", status: "disbursed", group_id: "grp-1" }],
    });
...
describe("loansService.disburse", () => {
  ...
  it("throws ValidationError when disbursing a pending (not yet approved) loan", async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: "loan-1", status: "pending", group_id: "grp-1" }],
    });

    await expect(
      loansService.disburse(adminCtx, "loan-1", disburseInput),
    ).rejects.toBeInstanceOf(ValidationError);
  });
...
describe("loansService.reject", () => {
  it("can reject a pending loan", async () => {
    ...
    const result = await loansService.reject(adminCtx, "loan-1", {
...
describe("loansService.markDefaulted", () => {
  it("throws ValidationError when the loan is not active", async () => {
    ...
    await expect(
      loansService.markDefaulted(adminCtx, "loan-1", {
        reason: "Missed 3 payments",
      }),
    ).rejects.toBeInstanceOf(ValidationError);
  });
  ...
  it("marks an active loan defaulted", async () => {
    mockQuery
      .mockResolvedValueOnce({
        rows: [{ id: "loan-1", status: "active", group_id: "grp-1" }],
      })
      .mockResolvedValueOnce({
        rows: [{ id: "loan-1", status: "defaulted", defaulted_by: "admin-1" }],
      })
      .mockResolvedValueOnce({ rows: [] }); // audit log

    const result = await loansService.markDefaulted(adminCtx, "loan-1", {
      reason: "Missed 3 payments",
    });
...
describe("loansService.writeOff", () => {
  ...
  it("allows a different officer to write off, posting via the loan_writeoff template", async () => {
    ...
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: [{ id: "loan-1", status: "written_off" }],
      })
      .mockResolvedValueOnce({ rows: [] }); // audit log

    const result = await loansService.writeOff(writerCtx, "loan-1", {
      reason: "Confirmed uncollectible",
    });
  ...
  it("skips posting when the outstanding balance is zero", async () => {
    ...
    await loansService.writeOff(writerCtx, "loan-1", {
