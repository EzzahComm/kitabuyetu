...
describe("getCashFlowStatement", () => {
  it("classifies counter-accounts into IAS 7 sections (lending = operating)", async () => {
    ...
    const cf = await accountingService.getCashFlowStatement(
      ctx,
      "2026-01-01",
      "2026-12-31",
    );

    expect(cf.operating.map((l) => l.accountCode)).toEqual([
      "4001",
      "5002",
      "1101",
      "2102",
...
describe("getEquityChanges", () => {
  it("computes closing = opening + increases − decreases per account", async () => {
    ...
    const sce = await accountingService.getEquityChanges(
      ctx,
      "2026-01-01",
      "2026-12-31",
    );
