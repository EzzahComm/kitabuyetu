...
function mockRows(
  ...
  mockQuery.mockImplementation((sql: string) => {
    if (sql.includes("pg_class"))
      return Promise.resolve({ rows: [{ relkind }] });
    if (sql.includes("MIN(entry_date)"))
      return Promise.resolve({ rows: [{ min: minEntryDate }] });
    if (sql.includes("journal_lines_default"))
      return Promise.resolve({ rows: [{ count: defaultPartitionCount }] });
...
describe("ensureJournalLinesPartitions", () => {
  it("creates monthly partitions from the earliest entry_date through 3 months ahead, each with its constraint trigger", async () => {
    ...
    expect(result.created).toEqual([
      "journal_lines_y2026m05",
      "journal_lines_y2026m06",
      "journal_lines_y2026m07",
      "journal_lines_y2026m08",
      "journal_lines_y2026m09",
      "journal_lines_y2026m10",
    ]);
    ...
    const triggerCalls = mockQuery.mock.calls.filter(([sql]) =>
  ...
  it("no-ops with a warning when journal_lines is not partitioned (094/095 unapplied)", async () => {
    ...
    const ddl = mockQuery.mock.calls.filter(
      ([sql]) =>
