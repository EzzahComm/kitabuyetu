...
describe("organizationFinanceService.deposit", () => {
  it("rejects a non-positive amount before touching the database", async () => {
    await expect(
      organizationFinanceService.deposit(ctx, { amount: 0 }),
  ...
  it("posts DR 1001 / CR 4001 to the organization ledger after recording the deposit", async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: "wallet-1", available_balance: "10000.00" }],
    }); // wallet lock
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: "wallet-1", available_balance: "15000.00" }],
    }); // wallet UPDATE
    ...
    const result = await organizationFinanceService.deposit(ctx, {
      amount: 5000,
      source: "World Bank",
    });
