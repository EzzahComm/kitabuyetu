...
function acceptedResponse(body: unknown) {
  const list = (body as { smslist: { mobile: string; clientsmsid: number }[] })
    .smslist;
  return {
    data: {
...
describe("sendBulkSmsChunked partial failure", () => {
  ...
  it("keeps the accepted chunk when a later chunk fails", async () => {
    mockPost
      .mockImplementationOnce(
        (_u, body) => Promise.resolve(acceptedResponse(body)) as never,
      )
      .mockImplementationOnce(
        () => Promise.reject(new Error("provider 500")) as never,
      );
  ...
  it("carries the provider error text onto the failed rows", async () => {
    mockPost
      ...
      .mockImplementationOnce(
        () => Promise.reject(new Error("connection reset")) as never,
      );

    const res = await sendBulkSmsChunked(items(150));
    expect(
      res.responses.find((r) => !r.success)?.responseDescription,
  ...
  it("still throws when EVERY chunk fails, so outage handling runs", async () => {
    ...
    await expect(sendBulkSmsChunked(items(150))).rejects.toThrow(
      /provider down/,
    );
  });
