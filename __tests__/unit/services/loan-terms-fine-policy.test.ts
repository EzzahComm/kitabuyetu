...
describe("getEffectiveLoanTerms", () => {
  it("resolves via the generic engine under the loan/terms key", async () => {
    ...
    const result = await getEffectiveLoanTerms(mockClient as never, {
      groupId: "g1",
    });
...
describe("loanPolicyService.setGroupTermsOverride", () => {
  it("rejects an interest rate above 100", async () => {
    await expect(
      loanPolicyService.setGroupTermsOverride(ctx, {
        ...VALID_TERMS,
        interestRate: 101,
      }),
  ...
  it("rejects an unknown interest method", async () => {
    await expect(
      loanPolicyService.setGroupTermsOverride(ctx, {
        ...VALID_TERMS,
        interestMethod: "compound" as never,
      }),
    ).rejects.toBeInstanceOf(ValidationError);
  });
...
describe("finePolicyService.setGroupOverride", () => {
  it("rejects an empty schedule", async () => {
    await expect(
      finePolicyService.setGroupOverride(ctx, {}),
    ).rejects.toBeInstanceOf(ValidationError);
    expect(setPolicy).not.toHaveBeenCalled();
  });
  ...
  it("rejects a negative amount", async () => {
    await expect(
      finePolicyService.setGroupOverride(ctx, { absence: -5 }),
    ).rejects.toBeInstanceOf(ValidationError);
  });

  it("rejects a blank category name", async () => {
    await expect(
      finePolicyService.setGroupOverride(ctx, { "  ": 50 }),
    ).rejects.toBeInstanceOf(ValidationError);
  });
