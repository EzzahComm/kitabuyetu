...
describe("meetingsService.updateResolution", () => {
  it("scopes the existence check by resolution, meeting, AND group", async () => {
    ...
    mockQuery.mockResolvedValueOnce({
      rows: [{ ...existingRow, implemented: true }],
    });

    await meetingsService.updateResolution(ctx, "m1", "r1", {
      implemented: true,
    });
  ...
  it("throws NotFoundError when the resolution does not belong to this meeting/tenant", async () => {
    ...
    await expect(
      meetingsService.updateResolution(ctx, "m1", "not-mine", {
        implemented: true,
      }),
  ...
  it("sets implemented_at=now() when marking implemented", async () => {
    ...
    const updateSql = (mockQuery.mock.calls[1][0] as string).replace(
      /\s+/g,
      " ",
    );
  ...
  it("clears implemented_at when marking back to outstanding", async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [
        { ...existingRow, implemented: true, implemented_at: "2026-08-01" },
      ],
    });
    mockQuery.mockResolvedValueOnce({ rows: [existingRow] });
  ...
  it("derives implemented_at server-side — the client cannot supply it directly", () => {
    ...
    const parsed = UpdateResolutionSchema.safeParse({
      implemented: true,
  ...
  it("scopes the UPDATE itself by id AND group, not id alone", async () => {
    ...
    expect((updateSql as string).replace(/\s+/g, " ")).toMatch(
...
describe("UpdateResolutionSchema", () => {
  ...
  it("rejects a status outside the four real outcomes", () => {
    expect(
      UpdateResolutionSchema.safeParse({ status: "approved" }).success,
    ).toBe(false);
  });

  it("requires implementationDeadline as YYYY-MM-DD, or accepts null to clear it", () => {
    expect(
      UpdateResolutionSchema.safeParse({ implementationDeadline: "2026-09-01" })
        .success,
    ).toBe(true);
    expect(
      UpdateResolutionSchema.safeParse({ implementationDeadline: null })
        .success,
    ).toBe(true);
    expect(
      UpdateResolutionSchema.safeParse({ implementationDeadline: "09/01/2026" })
        .success,
    ).toBe(false);
  });
});
