...
describe("parseRecipientSpec", () => {
  it("accepts event-sourced phone and member specs", () => {
    expect(parseRecipientSpec({ type: "event_phone", field: "phone" })).toEqual(
      { type: "event_phone", field: "phone" },
    );
    expect(
      parseRecipientSpec({ type: "event_member", field: "member_id" }),
  ...
  it('rejects "group_admin" — renamed to chairperson in migration 050', () => {
    ...
    expect(
      parseRecipientSpec({ type: "roles", roles: ["group_admin"] }),
    ).toBeNull();
  });

  it("rejects the whole spec when any role is unknown, rather than narrowing the audience", () => {
    expect(
      parseRecipientSpec({ type: "roles", roles: ["treasurer", "bogus"] }),
    ).toBeNull();
    expect(parseRecipientSpec({ type: "roles", roles: [] })).toBeNull();
  });

  it("accepts membership specs and rejects unknown types", () => {
    expect(parseRecipientSpec({ type: "all_members" })).toEqual({
      type: "all_members",
    });
    expect(parseRecipientSpec({ type: "active_members" })).toEqual({
      type: "active_members",
    });
