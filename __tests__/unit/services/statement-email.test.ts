...
beforeEach(() => {
  mockQuery.mockReset();
  (withAdminDb as jest.Mock).mockImplementation((fn) =>
    fn({ query: mockQuery }),
  );
  (sendReactEmail as jest.Mock)
    .mockReset()
      ...
      rows: [
        {
          ...
          loan_balance: "0",
          shares: "0",
          contributed_this_period: "0",
        },
      ],
    });
    mockQuery.mockResolvedValueOnce({ rows: [] });
    (sendReactEmail as jest.Mock).mockRejectedValueOnce(
