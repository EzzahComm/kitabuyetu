...
} from "@/lib/services/sms-scheduler.service";
  ...
  withAdminDb: (fn: (client: FakeClient) => unknown) => {
    const client: FakeClient = {
      query: jest.fn((sql: string) => {
        if (/FROM\s+sms_schedules s/i.test(sql))
          return Promise.resolve({ rows: [SCHEDULE] });
        if (/WITH claimed/i.test(sql))
          return Promise.resolve({ rows: claimResult });
        if (/FROM\s+sms_campaigns c/i.test(sql))
          return Promise.resolve({ rows: campaignRows });
...
describe("processDueSmsSchedules", () => {
  it("enqueues exactly one send for a claimed occurrence", async () => {
    ...
    expect(enqueueJob).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        dedup_key: "sms_bulk_send:schedule:sched-1:2026-07-11T08:00:00.000Z",
      }),
      expect.anything(),
    );
  });
  ...
  it("pre-renders {{group_name}} but leaves per-recipient placeholders for the dispatch job", async () => {
    ...
    expect(enqueueJob).toHaveBeenCalledWith(
      "sms_bulk_send",
      expect.objectContaining({
        message: "Hi {{first_name}}, Umoja Chama meets tomorrow.",
      }),
      expect.anything(),
      expect.anything(),
    );
