...
describe("isFeatureEnabled", () => {
  it("fails open for a key with no row (gating shipped modules can never brick them)", async () => {
    ...
    await expect(
      isFeatureEnabled(mockClient, "nonexistent", { groupId: "g1" }),
    ).resolves.toBe(true);
  });
  ...
  it("is off for everyone when enabled=false, regardless of targeting", async () => {
    ...
    await expect(
      isFeatureEnabled(mockClient, "welfare_module", { groupId: "g1" }),
    ).resolves.toBe(false);
  });
  ...
  describe("applies_to='plan'", () => {
    it("is on when the group plan meets min_plan", async () => {
      mockQuery
        .mockResolvedValueOnce(
          flagRow({ applies_to: "plan", conditions: { min_plan: "growth" } }),
        )
        .mockResolvedValueOnce({ rows: [{ plan_type: "enterprise" }] });
      await expect(
        isFeatureEnabled(mockClient, "bulk_sms", { groupId: "g1" }),
      ).resolves.toBe(true);
    });
    ...
    it("is off when min_plan names an unknown plan", async () => {
      mockQuery.mockResolvedValueOnce(
        flagRow({ applies_to: "plan", conditions: { min_plan: "platinum" } }),
      );
      await expect(
        isFeatureEnabled(mockClient, "bulk_sms", { groupId: "g1" }),
      ).resolves.toBe(false);
  ...
  describe("applies_to='group' / 'member'", () => {
    it("is on only for listed groups", async () => {
      mockQuery.mockResolvedValueOnce(
        flagRow({
          applies_to: "group",
          conditions: { group_ids: ["g1", "g2"] },
        }),
      );
      await expect(
        isFeatureEnabled(mockClient, "new_dashboard", { groupId: "g1" }),
      ).resolves.toBe(true);
      ...
      await expect(
        isFeatureEnabled(mockClient, "new_dashboard", { groupId: "g3" }),
      ).resolves.toBe(false);
    });

    it("is off for group targeting with no group_ids at all", async () => {
      mockQuery.mockResolvedValueOnce(
        flagRow({ applies_to: "group", conditions: {} }),
      );
      await expect(
        isFeatureEnabled(mockClient, "new_dashboard", { groupId: "g1" }),
      ).resolves.toBe(false);
    ...
    it("is on only for listed members", async () => {
      mockQuery.mockResolvedValueOnce(
        flagRow({ applies_to: "member", conditions: { member_ids: ["m1"] } }),
      );
      await expect(
        isFeatureEnabled(mockClient, "beta_thing", { memberId: "m1" }),
      ).resolves.toBe(true);
      ...
      await expect(
        isFeatureEnabled(mockClient, "beta_thing", { memberId: "m2" }),
      ).resolves.toBe(false);
    });
  });
  ...
  describe("rollout_pct", () => {
    ...
    it("is deterministic — the same subject always resolves the same way", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        results.push(
          await isFeatureEnabled(mockClient, "welfare_module", {
    ...
    it("needs a stable subject — no group or member means off for partial rollouts", async () => {
      mockQuery.mockResolvedValueOnce(flagRow({ rollout_pct: 50 }));
      await expect(
        isFeatureEnabled(mockClient, "welfare_module", {}),
      ).resolves.toBe(false);
    });
  });
});
