...
describe("allocateSplit — empty / degenerate input", () => {
  ...
  it("sends everything to the default account when no rules", () => {
    ...
    expect(out).toEqual([
      { account_code: "4001", amount: 1500, amount_cents: 150000 },
    ]);
  });
});
...
describe("allocateSplit — percentage rules", () => {
  ...
  it("dumps leftover to default when percentages sum to less than 100", () => {
    ...
    expect(out.find((a) => a.account_code === "4001")?.amount_cents).toBe(
      30000,
    );
  });
});
...
describe("allocateSplit — fixed-amount rules", () => {
  it("applies fixed lines in priority order", () => {
    ...
    expect(out.find((a) => a.account_code === "WELFARE")?.amount_cents).toBe(
      50000,
    );
    expect(out.find((a) => a.account_code === "SAVINGS")?.amount_cents).toBe(
      125000,
    );
    expect(out.find((a) => a.account_code === "INVEST")?.amount_cents).toBe(
      125000,
    );
  });
  ...
  it("partially allocates a fixed line when amount is smaller", () => {
    ...
    expect(out).toEqual([
      { account_code: "WELFARE", amount: 300, amount_cents: 30000 },
    ]);
  ...
  it("stops applying once amount is exhausted by fixed lines", () => {
    ...
    expect(out.find((a) => a.account_code === "EMERGENCY")?.amount_cents).toBe(
      20000,
    );
    ...
    const fixtures: Array<{ amount: number; rules: SplitRule[] }> = [
      {
        amount: 100,
        rules: [
          rule({ account_code: "A", percentage: 33.33 }),
          rule({ account_code: "B", percentage: 33.33 }),
          rule({ account_code: "C", percentage: 33.34 }),
        ],
      },
      {
        amount: 7777,
        rules: [
          rule({ account_code: "A", percentage: 25 }),
          rule({ account_code: "B", percentage: 25 }),
          rule({ account_code: "C", percentage: 50 }),
        ],
      },
      {
        amount: 12345,
        rules: [
          rule({ account_code: "A", fixed_amount: 500 }),
          rule({ account_code: "B", percentage: 60 }),
          rule({ account_code: "C", percentage: 40 }),
        ],
      },
