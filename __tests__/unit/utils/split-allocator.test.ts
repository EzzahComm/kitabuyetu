...
describe('allocateSplit — percentage rules', () => {
  it('splits 50/50 evenly when divisible', () => {
    const out = allocateSplit(
      1000,
      [rule({ account_code: 'A', percentage: 50 }), rule({ account_code: 'B', percentage: 50 })],
      '4001',
    );
    expect(out).toEqual([
  ...
  it('scales down when percentages exceed 100', () => {
    ...
    const out = allocateSplit(
      1000,
      [rule({ account_code: 'A', percentage: 60 }), rule({ account_code: 'B', percentage: 60 })],
      '4001',
    );
    expect(sumCents(out)).toBe(100000);
  ...
  it('dumps leftover to default when percentages sum to less than 100', () => {
    const out = allocateSplit(1000, [rule({ account_code: 'A', percentage: 70 })], '4001');
    expect(sumCents(out)).toBe(100000);
...
describe('allocateSplit — fixed-amount rules', () => {
  ...
  it('partially allocates a fixed line when amount is smaller', () => {
    const out = allocateSplit(300, [rule({ account_code: 'WELFARE', fixed_amount: 500 })], '4001');
