...
for (const p of PREFIXES)
  for (const s of SEQS) samples.push(composeMembershipNo(p, s));
    ...
    for (const no of samples) {
      for (const pos of [0, 1]) {
        ...
        for (let c = 65; c <= 90; c++) {
          ...
          const mutated =
            no.slice(0, pos) + String.fromCharCode(c) + no.slice(pos + 1);
          expect(isValidMembershipNo(mutated)).toBe(false);
        }
      }
    }
...
describe("normalisation & display", () => {
  it("accepts human input variants", () => {
    ...
    expect(
      isValidMembershipNo(`${no.slice(0, 2)} ${no.slice(2, 7)} ${no[7]}`),
    ).toBe(true);
