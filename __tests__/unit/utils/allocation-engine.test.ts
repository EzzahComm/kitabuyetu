...
describe('resolveProduct — decision table', () => {
  ...
  it('A2 beats a suffix (suffix never overrides an exact match)', () => {
    const r = resolveProduct({
      ...base,
      suffix: 'W',
      openRequests: [req({ id: 'r-exact', product: 'loan_repayment', amount: 1000 })],
    });
  ...
  it('A3 beats non-exact open requests', () => {
    const r = resolveProduct({
      ...base,
      suffix: 'W',
      openRequests: [req({ id: 'r-1', product: 'loan_repayment', amount: 750 })],
    });
  ...
  it('A4: exactly one open request, amount differs → variance tagged', () => {
    const r = resolveProduct({
      ...base,
      amount: 400,
      openRequests: [req({ id: 'r-1', product: 'loan_repayment', amount: 1000 })],
    });
