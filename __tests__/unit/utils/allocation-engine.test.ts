...
const base = {
  suffix: null as null,
  openRequests: [] as OpenPaymentRequest[],
  memberDefault: null,
  groupDefault: "savings" as const,
  amount: 1000,
};
...
describe("resolveProduct — decision table", () => {
  ...
  it("A3: valid suffix decides when no exact match", () => {
    expect(resolveProduct({ ...base, suffix: "L" })).toMatchObject({
      product: "loan_repayment",
      requestId: null,
      tier: "A3",
    });
    expect(resolveProduct({ ...base, suffix: "W" })).toMatchObject({
      product: "welfare",
      tier: "A3",
    });
    expect(resolveProduct({ ...base, suffix: "S" })).toMatchObject({
      product: "share",
      tier: "A3",
    });
  });
  ...
  it("A8: group default as the final tier", () => {
    expect(resolveProduct(base)).toMatchObject({
      product: "savings",
      requestId: null,
      tier: "A8",
    });
    expect(resolveProduct({ ...base, groupDefault: "welfare" })).toMatchObject({
      product: "welfare",
      tier: "A8",
    });
  });
...
describe("parseAccountRef — suffix handling (A1/A3 inputs)", () => {
  it("parses valid suffixes in every human format", () => {
    for (const raw of [
      ...
      expect(parseAccountRef(raw)).toEqual({
        account: "BG102534",
        suffix: "W",
        invalidSuffix: false,
      });
  ...
  it("A1: unknown trailing letter is flagged invalid, never guessed", () => {
    expect(parseAccountRef("BG102534-X")).toEqual({
      account: "BG102534",
      suffix: null,
      invalidSuffix: true,
    });
  ...
  it("bare numbers and non-membership refs pass through untouched", () => {
    expect(parseAccountRef("BG 10253 4")).toEqual({
      account: "BG102534",
      suffix: null,
      invalidSuffix: false,
    });
    expect(parseAccountRef("KYT-CONTR-KY1234567")).toMatchObject({
      suffix: null,
      invalidSuffix: false,
    });
    expect(parseAccountRef("INV-2026-000123")).toMatchObject({
      suffix: null,
      invalidSuffix: false,
    });
  });
