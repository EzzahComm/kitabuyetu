 ...
 */
async function releaseCashReservation(
  db: PoolClient,
  groupId: string,
  amount: string,
): Promise<void> {
  ...
  const watchdogNotify = await withAdminDb(
    async (db): Promise<WatchdogNotifyInfo | undefined> => {
      ...
      if (!parsed.success) {
        ...
        return {
          rowId: row.id,
          eventData: { status: "failed", failureReason: parsed.desc },
        };
      }
      ...
      if (!jeId) {
        ...
        logger.error(
          "[settlements] sweep completed but GL posting was skipped",
          {
            settlementId: row.id,
            groupId: row.group_id,
          },
      ...
      return {
        rowId: row.id,
        eventData: { status: "completed", receipt: parsed.receipt },
      };
    },
  );
...
export async function handleVendorPaymentResult(
  ...
  const watchdogNotify = await withAdminDb(
    async (db): Promise<WatchdogNotifyInfo | undefined> => {
      ...
      if (!jeId) {
        logger.error(
          "[vendor-payments] payment completed but GL posting was skipped",
          {
            vendorPaymentId: row.id,
            groupId: row.group_id,
            expenseAccountCode: row.expense_account_code,
