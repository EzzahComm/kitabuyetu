...
export async function getEffectiveSavingsLimits(
  ...
  return resolvePolicy<SavingsLimits>(
    client,
    DOMAIN,
    POLICY_KEY,
    scope,
...
function validateSavingsLimits(limits: SavingsLimits): void {
  ...
  if (
    ...
    !(limits.maxContribution > limits.minContribution)
  ) {
    throw new ValidationError(
      "maxContribution must be greater than minContribution, or null for no maximum",
    );
  }
  if (
    !(Number.isInteger(limits.gracePeriodDays) && limits.gracePeriodDays >= 0)
  ) {
    throw new ValidationError(
      "gracePeriodDays must be a whole number, zero or positive",
    );
  }
}
...
export const savingsPolicyService = {
  ...
  async setPlatformLimitsDefault(
    userId: string,
    client: PoolClient,
    limits: SavingsLimits,
  ): Promise<void> {
