...
async function loadThresholds(
  client: PoolClient,
  groupId: string,
): Promise<Map<string, ThresholdRow>> {
...
const RAG_SCORE: Record<Rag, number | null> = {
  green: 100,
  amber: 55,
  red: 15,
  na: null,
...
async function getTotalAssetBalanceAsOf(
  client: PoolClient,
  groupId: string,
  asOf: string,
): Promise<number> {
...
async function getGrossLoansNow(
  client: PoolClient,
  groupId: string,
): Promise<number> {
  const { rows } = await client.query<{ total: string }>(
...
async function getRiskWeights(
  client: PoolClient,
): Promise<Map<string, { riskWeight: number; provisionRate: number }>> {
  const { rows } = await client.query<{
    asset_class: string;
    risk_weight: string;
  ...
  return new Map(
    rows.map((r) => [
      r.asset_class,
      {
        riskWeight: parseFloat(r.risk_weight),
...
export async function computeGroupGovernanceSnapshot(
  ...
  const growth = (curr: number, prior: number): number | null =>
    prior > 0 ? ((curr - prior) / prior) * 100 : null;
  ...
  const metrics: ComputedMetric[] = [
    ...
    {
      code: "par30",
      value: pct(buckets.par30, grossLoans),
      numerator: buckets.par30,
      denominator: grossLoans,
    },
    {
      code: "npl",
    ...
    {
      code: "concentration",
      value: pct(buckets.concentrationTop10, grossLoans),
    },
    ...
    {
      code: "roe",
      value: pct(pnl12m.income - pnl12m.expenses, institutionalCapital),
    },
    { code: "savings_growth", value: growth(savings, priorSavings) },
    {
      code: "membership_growth",
      value: growth(activeMembers, priorActiveMembers),
    },
    ...
    {
      code: "savings_protection",
      value: pct(protectionFund, memberSavingsAccount),
    },
  ];
  ...
  const priorByCode = new Map(
    priorRows.map((r) => [
      r.metric_code,
      r.value !== null ? parseFloat(r.value) : null,
    ]),
  );
  ...
  for (const m of metrics) {
    ...
    if ((HEALTH_SCORE_METRICS as readonly string[]).includes(m.code))
      ragForHealthScore.push(rag);
  ...
  const scored = ragForHealthScore
    .map((r) => RAG_SCORE[r])
    ...
    await client.query(
      ...
      [
        ...
        JSON.stringify(
          Object.fromEntries(
            metrics
              .filter((m) => HEALTH_SCORE_METRICS.includes(m.code as never))
              .map((m) => [m.code, m.value]),
          ),
        ),
      ],
    );
...
export async function computeGovernanceForAllGroups(asOf: string): Promise<{
  ...
  for (const group of groups) {
    try {
      const result = await withAdminDb((client) =>
        computeGroupGovernanceSnapshot(client, group.id, asOf, riskWeights),
      );
      succeeded++;
...
export async function listGovernanceAlerts(params: {
  ...
  return withAdminDb(async (db: PoolClient) => {
    ...
    const [data, count] = await Promise.all([
      ...
      db.query(
        `SELECT COUNT(*) AS total FROM public.governance_alerts a ${where}`,
        vals,
      ),
    ]);
