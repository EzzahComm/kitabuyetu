...
export const sharesService = {
      ...
      try {
        ...
        if (e?.code === "23505" && e?.constraint === "uq_share_class_code") {
          throw new ConflictError(
            `A share class with code '${input.code.toUpperCase()}' already exists in this group`,
          );
        }
        throw err;
      }
    ...
    return withTransaction(ctx, async (client) => {
      ...
      for (const [field, column] of Object.entries(fieldMap)) {
        ...
        if (v !== undefined) {
          ...
          const value =
            field === "code" && typeof v === "string" ? v.toUpperCase() : v;
      ...
      try {
        ...
        if (e?.code === "23505" && e?.constraint === "uq_share_class_code") {
          throw new ConflictError(
            "Another share class in this group already uses that code",
          );
        }
        throw err;
      }
  ...
  async createTransaction(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      if (!cls[0].is_active)
        throw new ConflictError(
          "Share class is inactive — re-activate it before posting transactions",
        );
      if (input.type === "transfer" && !cls[0].transfer_allowed) {
        throw new ConflictError(
          `Transfers are disabled for share class '${cls[0].code}'`,
        );
      }
      ...
      const isOutflow =
        input.type === "redemption" || input.type === "transfer";
      if (isOutflow && cls[0].lock_period_days > 0) {
        const lastAcq = await getLastAcquisitionAt(
          client,
          ctx.groupId,
          input.memberId,
          input.shareClassId,
        ...
        if (lastAcq) {
          const unlockAt = new Date(
            lastAcq.getTime() + cls[0].lock_period_days * 24 * 60 * 60 * 1000,
          );
          if (new Date() < unlockAt) {
            const daysLeft = Math.ceil(
              (unlockAt.getTime() - Date.now()) / (24 * 60 * 60 * 1000),
            );
            throw new ConflictError(
      ...
      const unitPrice: number =
        input.unitPrice ?? Number(cls[0].current_value ?? cls[0].par_value);
      ...
      if (input.type === "redemption" || input.type === "transfer") {
        const senderBalance = await getMemberBalance(
          client,
          ctx.groupId,
          input.memberId,
          input.shareClassId,
      ...
      if (cls[0].max_per_member !== null) {
        ...
        if (input.type === "transfer") {
          checks.push({
            memberId: input.counterpartyMemberId!,
            addAfter: qtyAbs,
          });
        }
        ...
        for (const c of checks) {
          const current = await getMemberBalance(
            client,
            ctx.groupId,
            c.memberId,
            input.shareClassId,
      ...
      if (input.type === "transfer") {
        ...
        const serial =
          ...
          input.type === "purchase" ||
          (input.type === "adjustment" && dbQuantity > 0)
        ...
        if (
          (input.type === "purchase" || input.type === "redemption") &&
          totalAmount > 0
        ) {
  ...
  async listTransactions(ctx: TenantContext, params: ShareTxnQueryInput) {
    return withDb(ctx, async (client) => {
      ...
      if (params.shareClassId)
        addCond(`t.share_class_id = $$`, params.shareClassId);
      if (params.from) addCond(`t.posted_at     >= $$`, params.from);
      if (params.to)
        addCond(`t.posted_at     <= ($$ ::date + INTERVAL '1 day')`, params.to);

      const where = conds.join(" AND ");

      const [{ rows: cnt }, { rows: items }] = await Promise.all([
        client.query<{ count: string }>(
          `SELECT COUNT(*) AS count FROM share_transactions t WHERE ${where}`,
          vals,
        ),
  ...
  async reverseTransaction(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      if (orig.status !== "posted")
        throw new ConflictError(
          `Transaction is in status '${orig.status}' — only posted transactions can be reversed`,
        );
      if (orig.reverses_transaction_id)
        throw new ConflictError(
          `Transaction is itself a reversal — cannot be reversed again`,
        );
      ...
      if (
        (orig.type === "purchase" || orig.type === "redemption") &&
        Number(orig.total_amount) > 0
      ) {
        ...
        await postTemplatedJournal(
          ...
          { amount },
          { reference: reverseRow.id, memberId: orig.member_id, invert: true },
        );
      }
  ...
  async listHoldings(ctx: TenantContext, params: HoldingsQueryInput) {
    return withDb(ctx, async (client) => {
      ...
      const [{ rows: cnt }, { rows: items }] = await Promise.all([
        client.query<{ count: string }>(
          `SELECT COUNT(*) AS count FROM share_holdings h WHERE ${where}`,
          vals,
        ),
...
async function assertGroupMembership(
  ...
  if (!rows[0])
    throw new ValidationError(
      `${field} (${memberId}) is not a member of this group`,
    );
}
...
async function allocateSerial(
  ...
  const { rows } = await client.query<{
    allocate_share_certificate_serial: string;
  }>(`SELECT allocate_share_certificate_serial($1, $2)`, [groupId, classCode]);
