...
export const reallocationsService = {
   ...
   */
  async initiate(ctx: TenantContext, data: { contributionId: string; toMemberId: string; reason: string }) {
    return withTransaction(ctx, async (client) => {
      // Target must hold an active membership in this group (§5/§6a).
      const { membershipId: toMembershipId } = await assertActiveMembership(client, ctx.groupId, data.toMemberId);
      ...
      if (!payment || payment.allocation_status !== 'allocated') {
        throw new ConflictError(`Payment is not in an allocatable state (${payment?.allocation_status ?? 'missing'})`);
      }
 ...
 */
async function executeReallocation(client: PoolClient, realloc: ReallocationRow, actorId: string): Promise<void> {
