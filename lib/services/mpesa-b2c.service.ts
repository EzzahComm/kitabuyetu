  ...
  await withAdminDb(async (db) => {
    ...
    const sourceAccount =
      params.commandId === "BusinessPayment"
        ? (process.env.MPESA_LOAN_DISBURSEMENT_SHORTCODE ??
          process.env.MPESA_UTILITY_SHORTCODE ??
          null)
...
export async function handleB2CResult(
  body: B2CResultBody,
  callerIp: string,
): Promise<void> {
  ...
  const watchdogNotify = await withAdminDb(
    async (db): Promise<WatchdogNotifyInfo | undefined> => {
      ...
      if (r.ResultCode !== 0) {
        ...
        if (b2c?.disbursement_request_id) {
          await releaseDisbursementReservation(
            db,
            b2c.disbursement_request_id,
            {
              status: "failed",
          ...
          return {
            rowId: b2c.disbursement_request_id,
            eventData: { status: "failed", failureReason: r.ResultDesc },
          };
        }
        return undefined;
      }
      ...
      if (b2c) {
        ...
        if (b2c.loan_id && receipt) {
          ...
          if (memberRows[0]) {
            const { emitBusinessEvent } =
              await import("@/lib/sms/trigger-engine");
        ...
        if (b2c.disbursement_request_id) {
          ...
          return {
            rowId: b2c.disbursement_request_id,
            eventData: { status: "completed", mpesaReceiptNumber: receipt },
          };
        }
      }
      return undefined;
  ...
  if (watchdogNotify) {
    await notifyDisbursementCallback(
      "disbursement",
      watchdogNotify.rowId,
      watchdogNotify.eventData,
    );
  }
}
...
async function applyLoanDisbursement(
  ...
  if (!loanRows[0]) {
    logger.warn(
      "[mpesa] B2C result loan not flipped — wrong status or already disbursed",
      {
        loanId: args.loanId,
      },
    );
