...
import type {
  ...
  UpdateDividendDeclarationInput,
  DividendQueryInput,
  PayAllocationInput,
  BulkPayAllocationsInput,
  DividendStatus,
  DividendPolicyType,
...
export const dividendsService = {
    ...
    return withDb(ctx, async (client) => {
      ...
      const [{ rows: cnt }, { rows: items }] = await Promise.all([
        client.query<{ count: string }>(
          `SELECT COUNT(*) AS count FROM dividend_declarations WHERE ${where}`,
          vals,
        ),
  ...
  async get(
    ...
    return withDb(ctx, async (client) => {
      ...
      if (!rows[0])
        throw new NotFoundError("Dividend declaration", declarationId);
      return rows[0];
    });
  },
    ...
    return withTransaction(ctx, async (client) => {
      const { rows } = await client.query<DividendDeclaration>(
        ...
        [
          ...
          input.poolAmount,
          input.policyType,
          JSON.stringify(input.policyConfig ?? {}),
          input.shareClassIds,
  ...
  async submitForApproval(
    ...
    return withTransaction(ctx, async (client) => {
      const decl = await fetchDeclarationForUpdate(
        client,
        ctx.groupId,
        declarationId,
      );
      if (decl.status !== "draft") {
        throw new ConflictError(
          `Cannot submit a declaration in status '${decl.status}'`,
        );
      }
      const { rows } = await client.query<DividendDeclaration>(
  ...
  async approve(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      if (decl.status !== "pending_approval" && decl.status !== "draft") {
        throw new ConflictError(
          `Cannot approve a declaration in status '${decl.status}'`,
        );
      }
      ...
      await client.query(
        `DELETE FROM dividend_allocations WHERE declaration_id = $1`,
        [declarationId],
      );
      ...
      if (totalGross > 0) {
        ...
        await postTemplatedJournal(
          ...
          {
            gross: totalGross,
            net: Number(computed.totalNet),
            tax: Number(computed.totalTax),
          },
  ...
  async cancel(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      if (decl.status === "paid" || decl.status === "cancelled") {
        throw new ConflictError(
          `Cannot cancel a declaration in status '${decl.status}'`,
        );
      }
      ...
      await writeAuditLog(client, ctx, "dividend.cancel", declarationId, {
        reason,
      });
      return rows[0];
    });
  ...
  async payAllocation(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      if (decl.status !== "approved") {
        throw new ConflictError(
          `Cannot record payments — declaration is in status '${decl.status}'`,
        );
      }
      ...
      if (a[0].status !== "pending")
        throw new ConflictError(
          `Allocation is in status '${a[0].status}' — only pending allocations can be paid`,
        );
      ...
      await writeAuditLog(
        client,
        ctx,
        "dividend.allocation.pay",
        allocationId,
        {
          declaration_id: declarationId,
          method: input.paymentMethod,
          amount: a[0].net_amount,
        },
      );
  ...
  async bulkPayAllocations(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      for (const id of input.allocationIds) {
        ...
        if (a[0].status !== "pending") {
          skipped.push({ id, reason: `Status is ${a[0].status}` });
          continue;
        }
  ...
  async cancelAllocation(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      if (decl.status !== "approved") {
        throw new ConflictError(
          `Cannot cancel allocations — declaration is in status '${decl.status}'`,
        );
      }
      ...
      if (!updated[0])
        throw new ConflictError(
          "Allocation not found or not in pending status",
        );
...
async function fetchDeclarationForUpdate(
  client: PoolClient,
  groupId: string,
  id: string,
): Promise<DividendDeclaration> {
...
async function computeAllocations(
  ...
  const totalShares = holdings.reduce(
    (s, h) => s + parseInt(h.total_shares, 10),
    0,
  );
