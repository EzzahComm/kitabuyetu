...
const specificity = (r: {
  organization_id: string | null;
  group_id: string | null;
}): number => (r.group_id ? 2 : r.organization_id ? 1 : 0);
...
export async function resolvePolicyDetailed<T>(
  ...
  const winner = rows.reduce((best, r) =>
    specificity(r) > specificity(best) ? r : best,
  );
  const source: PolicySource = winner.group_id
    ? "group"
...
export async function resolvePolicy<T>(
  ...
  return (
    await resolvePolicyDetailed(client, domain, policyKey, scope, fallback)
  ).value;
}
