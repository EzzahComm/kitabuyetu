...
export async function getMarketingAnalytics(ctx: TenantContext, days = 30): Promise<MarketingAnalytics> {
  ...
  return withDb(ctx, async (db) => {
    ...
    const smsSent = Number(sms.sent),
      smsRecipients = Number(sms.recipients);
    const email = emailRow.rows[0];
    const emailSent = Number(email.sent),
      emailRecipients = Number(email.recipients);
    ...
    return {
      ...
      sms: {
        campaigns: Number(sms.campaigns),
      ...
      email: {
        campaigns: Number(email.campaigns),
        recipients: emailRecipients,
        sent: emailSent,
        failed: Number(email.failed),
        deliveryRate: rate(emailSent, emailRecipients),
        opened: Number(email.opened),
        openRate: rate(Number(email.opened), emailSent),
      },
      automation,
      automationVolume,
      crm: {
        totalContacts,
        optedInContacts,
        optInRate: rate(optedInContacts, totalContacts),
        opportunitiesByStage,
        activitiesInPeriod: Number(activityRow.rows[0].count),
      },
    };
  });
}
