...
import { periodToInterval, type AnalyticsPeriod } from '@/lib/validators/analytics.schema';
import { rowsToCsv, csvFilename } from '@/lib/utils/csv';

export const EXPORT_KINDS = ['members', 'contributions', 'loans', 'share_holdings', 'credit_scores'] as const;
...
export const analyticsService = {
  async getExecutiveSummary(ctx: TenantContext, period: AnalyticsPeriod): Promise<ExecutiveSummary> {
    ...
    return cached(keys.cache('executive-summary', `${ctx.groupId}:${period}`), 60, () =>
  ...
  async exportCsv(ctx: TenantContext, kind: ExportKind): Promise<{ csv: string; filename: string }> {
    return withDb(ctx, async (client) => {
      switch (kind) {
        case 'members':
          return exportMembers(client, ctx.groupId);
        case 'contributions':
          return exportContributions(client, ctx.groupId);
        case 'loans':
          return exportLoans(client, ctx.groupId);
        case 'share_holdings':
          return exportShareHoldings(client, ctx.groupId);
        case 'credit_scores':
          return exportCreditScores(client, ctx.groupId);
      }
    });
  },
  ...
  async getRiskAnalysis(ctx: TenantContext): Promise<RiskAnalysis> {
    return cached(keys.cache('risk-analysis', ctx.groupId), 60, () =>
      withDb(ctx, async (client) => {
        const groupId = ctx.groupId;
        const [overdueLoans, defaultedLoans, riskMembers, idleMembers, staleWelfare] = await Promise.all([
          client.query<{
            loan_id: string;
            member_id: string;
            first_name: string;
