...
export const analyticsService = {
  async getExecutiveSummary(
    ...
    return cached(
      keys.cache("executive-summary", `${ctx.groupId}:${period}`),
      60,
      () =>
      ...
      withDb(ctx, async (client) => {
        ...
        const out: RiskAnalysis = {
          ...
          highRiskMembers: riskMembers.rows.map((r) => ({
            ...
            reliabilityTier:
              r.reliability_tier as RiskAnalysis["highRiskMembers"][0]["reliabilityTier"],
          })),
...
async function exportContributions(
  ...
  return {
    ...
    filename: csvFilename("contributions"),
  };
}
...
async function exportShareHoldings(
  client: PoolClient,
  groupId: string,
): Promise<{ csv: string; filename: string }> {
  const { rows } = await client.query<Record<string, string | null>>(
  ...
  return {
    csv: rowsToCsv(headers, rows),
    filename: csvFilename("share-holdings"),
  };
}

async function exportCreditScores(
  client: PoolClient,
  groupId: string,
): Promise<{ csv: string; filename: string }> {
  ...
  return {
    csv: rowsToCsv(headers, rows),
    filename: csvFilename("credit-scores"),
  };
}
