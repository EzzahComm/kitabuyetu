...
export async function replayUnprocessedCallbacks(): Promise<{
  ...
  const rows = await withAdminDb((db) =>
    db
      .query<{ id: string; callback_type: string; caller_ip: string | null; body: unknown }>(
...
export async function handleReversalResult(body: Record<string, unknown>, callerIp: string): Promise<void> {
  ...
  await withAdminDb(async (db) => {
    ...
    if (success && receipt) {
      ...
      await db.query("UPDATE payments SET status='reversed' WHERE mpesa_receipt_number=$1", [receipt]);
...
export async function handleBalanceResult(body: Record<string, unknown>, callerIp: string): Promise<void> {
  assertSafaricomIp(callerIp);
  ...
  await withAdminDb(async (db) => {
    ...
    await db.query(
      ...
      [newStatus, JSON.stringify(body), r.OriginatorConversationID ?? '', r.ConversationID ?? ''],
    );
