  ...
  for (const line of template.lines) {
    ...
    if (value === undefined) {
      throw new ValidationError(
        `Posting template references amount '${line.amount}' which was not supplied`,
      );
    }
    if (!(value >= 0)) {
      throw new ValidationError(
        `Posting amount '${line.amount}' must be zero or positive`,
      );
    }
    if (value === 0) continue;
...
export async function resolvePostingTemplate(
  ...
  return resolvePolicy<PostingTemplate>(
    client,
    DOMAIN,
    keyFor(event),
    scope,
...
export async function postLoanDisbursementJournal(
  ...
  const template = await resolvePostingTemplate(client, "loan_disbursement", {
  ...
  await client.query(`UPDATE loans SET journal_entry_id = $1 WHERE id = $2`, [
    jeId,
    args.loanId,
  ]);
...
export async function postLoanRepaymentJournal(
  ...
  const template = await resolvePostingTemplate(client, "loan_repayment", {
    groupId: args.groupId,
  });
  const lines = buildTemplateLines(template, {
    principal: args.principalPortion,
    interest: args.interestPortion,
  });
  if (lines.length === 0) return null;
...
export async function postSettlementSweepJournal(
  ...
  const template = await resolvePostingTemplate(client, "settlement_sweep", {
    groupId: args.groupId,
  });
  ...
  await client.query(
    `UPDATE settlement_requests SET journal_entry_id = $1 WHERE id = $2`,
    [jeId, args.settlementId],
  );
  return jeId;
}
...
export async function postVendorPaymentJournal(
  ...
  const fee = args.fee ?? 0;
  const template = await resolvePostingTemplate(client, "vendor_payment", {
    groupId: args.groupId,
  });
  const overriddenLines = template.lines.map((l) =>
    l.amount === "amount" && l.side === "debit"
      ? { ...l, accountCode: args.expenseAccountCode }
      : l,
  );
  ...
  const lines = buildTemplateLines(
    ...
    { amount: args.amount, fee: postFee ? fee : 0 },
  );
  if (lines.length === 0) return null;
 ...
 */
function validateOverrideStructure(
  event: PostingEvent,
  lines: TemplateLine[],
): void {
  ...
  for (const l of lines) {
    if (!/^\d{4}$/.test(l.accountCode)) {
      throw new ValidationError(
        `'${l.accountCode}' is not a valid 4-digit account code`,
      );
    }
  }
}

async function assertCodesExistInGroupCoa(
  client: PoolClient,
  groupId: string,
  lines: TemplateLine[],
): Promise<void> {
  ...
  if (missing.length > 0) {
    throw new ValidationError(
      `Account code(s) not in your chart of accounts: ${missing.join(", ")}`,
    );
  }
}
