...
export async function sendDailyMpesaReconReports(): Promise<{
  groups: number;
  emailed: number;
}> {
  ...
  for (const group of groups) {
    ...
    for (const email of officers) {
      const res = await sendFinancialReport({
        ...
        logger.error("[mpesa-reports] email send failed", {
          groupId: group.id,
          err: String(err),
        });
...
function buildReportHtml(d: {
  ...
  if (d.unroutedCount > 0)
    alerts.push(`${d.unroutedCount} unrouted receipt(s) awaiting allocation`);
