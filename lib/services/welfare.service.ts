...
export type CreateWelfareRequestInput = z.infer<
  typeof CreateWelfareRequestSchema
>;
export type ReviewWelfareRequestInput = z.infer<
  typeof ReviewWelfareRequestSchema
>;
...
export const welfareService = {
  async listRequests(ctx: TenantContext, params: WelfareQueryInput) {
    return withDb(ctx, async (client) => {
      ...
      if (params.memberId) {
        conditions.push(`wr.member_id = $${i++}`);
  ...
  async reviewRequest(
    ctx: TenantContext,
    id: string,
    data: ReviewWelfareRequestInput,
  ) {
    return withTransaction(ctx, async (client) => {
      const {
        rows: [req],
      ...
      if (req.status !== "pending" && req.status !== "under_review") {
        throw new ValidationError(
          `Cannot review a request with status '${req.status}'`,
        );
      }
      ...
      if (data.action === "approve") {
        const { rows } = await client.query(
          ...
          [
            data.amountApproved ?? req.amount_requested,
            ctx.userId,
            data.notes ?? null,
            id,
            ctx.groupId,
  ...
  async disburse(ctx: TenantContext, id: string, data: DisburseWelfareInput) {
    return withTransaction(ctx, async (client) => {
      ...
      if (req.status !== "approved") {
        throw new ValidationError(
          `Cannot disburse a request with status '${req.status}'`,
        );
      }
      const { rows } = await client.query(
        `UPDATE welfare_requests SET
  ...
  async getPoolSummary(ctx: TenantContext) {
    return withDb(ctx, async (client) => {
      ...
      return {
        ...
        balance:
          Number(pool.total_collected) - Number(disbursed.total_disbursed),
  ...
  async recordPoolContribution(
    ctx: TenantContext,
    data: RecordWelfarePoolInput,
  ) {
    return withTransaction(ctx, async (client) => {
...
export type CreateWelfareRequestPayload = z.input<
  typeof CreateWelfareRequestSchema
>;
export type ReviewWelfareRequestPayload = z.input<
  typeof ReviewWelfareRequestSchema
>;
