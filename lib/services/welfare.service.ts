...
export const welfareService = {
  async listRequests(ctx: TenantContext, params: WelfareQueryInput) {
    return withDb(ctx, async (client) => {
      ...
      const {
        rows: [{ count }],
      } = await client.query(`SELECT COUNT(*) FROM welfare_requests wr WHERE ${where}`, args);
      return {
        items,
        total: Number(count),
  ...
  async reviewRequest(ctx: TenantContext, id: string, data: ReviewWelfareRequestInput) {
    return withTransaction(ctx, async (client) => {
      const {
        rows: [req],
      } = await client.query('SELECT * FROM welfare_requests WHERE id=$1 AND group_id=$2', [id, ctx.groupId]);
  ...
  async listPoolContributions(ctx: TenantContext, params: { page: number; limit: number }) {
    return withDb(ctx, async (client) => {
      ...
      const {
        rows: [{ count }],
      } = await client.query('SELECT COUNT(*) FROM welfare_pool_contributions WHERE group_id=$1', [ctx.groupId]);
