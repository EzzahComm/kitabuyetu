...
export async function getAccessToken(): Promise<string> {
  ...
  const creds = Buffer.from(`${CONSUMER_KEY}:${CONSUMER_SECRET}`).toString(
    "base64",
  );
  ...
  await redis.set(REDIS_TOKEN_KEY, JSON.stringify(entry), {
    px: ttlMs - 60_000,
  });
...
export function assertSafaricomIp(ip: string): void {
  ...
  if (!isSafaricomIp(ip)) {
    logger.warn(
      "[daraja] callback IP not in Safaricom allow-list — processing anyway",
      { ip },
    );
  }
}
...
export async function registerC2BUrls(
  version: C2BApiVersion = "v2",
): Promise<C2BRegistrationResult> {
  const c = await makeClient();
...
export async function initiateB2C(
  ...
  const { data } = await withRetry(() =>
    c.post<{
      ...
      QueueTimeOutURL: withCallbackToken(
        `${CALLBACK_BASE}/api/v1/mpesa/b2c?type=timeout`,
      ),
      ResultURL: withCallbackToken(
        `${CALLBACK_BASE}/api/v1/mpesa/b2c?type=result`,
      ),
...
export async function buyAirtime(
  ...
  const origId = originatorId();
  const endpoint =
    process.env.MPESA_AIRTIME_ENDPOINT ?? "/mpesa/airtime/v1/purchase";
...
export async function initiateB2B(input: B2BInput): Promise<B2BResponse> {
  ...
  const payload: Record<string, unknown> = {
    ...
    QueueTimeOutURL: withCallbackToken(
      `${CALLBACK_BASE}/api/v1/mpesa/b2b?type=timeout`,
    ),
    ResultURL: withCallbackToken(
      `${CALLBACK_BASE}/api/v1/mpesa/b2b?type=result`,
    ),
  };
...
export async function updateSingleInvoice(
  invoice: BillManagerInvoice,
): Promise<void> {
  const c = await makeClient();
  await withRetry(() =>
...
export async function updateBulkInvoices(
  invoices: BillManagerInvoice[],
): Promise<void> {
  const c = await makeClient();
  await withRetry(() =>
...
export async function cancelSingleInvoice(
  externalReference: string,
): Promise<void> {
  const c = await makeClient();
  await withRetry(() =>
