...
export const vendorPaymentsService = {
  async initiate(
    ctx: TenantContext,
    input: InitiateVendorPaymentInput,
    ...
    if (input.channel === "b2c" && !input.payeePhone) {
      throw new ValidationError(
        "A payee phone number is required for a B2C vendor payment",
      );
    }
    if (
      input.channel === "b2b" &&
      (!input.payeeShortcode || !input.payeeAccount)
    ) {
      throw new ValidationError(
        "A payee shortcode and account are required for a B2B vendor payment",
      );
    }
    ...
    return withTransaction(ctx, async (db) => {
      ...
      if (!acctRows[0]) {
        throw new ValidationError(
          "Group has no active Cash/M-Pesa account (1001) to pay from",
        );
      }
      const available =
  ...
  async approve(ctx: TenantContext, id: string): Promise<VendorPaymentRow> {
    const row = await withTransaction(ctx, async (db) => {
      ...
      await recordApproval(db, ctx, {
        ...
        subjectId: id,
        initiatedBy: rows[0].requested_by ?? "",
        decision: "approved",
      });
...
export async function findStuckVendorPayments(): Promise<{
  ...
  return withAdminDb(async (db) => {
    ...
    if (samples.length > 0) {
      logger.error(
        "[vendor-payments] stuck payouts — no result callback received",
        {
...
async function dispatchVendorPayment(id: string): Promise<void> {
  ...
  try {
    ...
    logger.error(
      "[vendor-payments] dispatch failed before Daraja accepted the request",
      {
        vendorPaymentId: id,
        err: String(err),
      },
