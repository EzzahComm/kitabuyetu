...
export async function getPlatformAnalytics() {
  return cached(keys.cache('platform-analytics', 'platform'), 120, () => withAdminDb(async (db: PoolClient) => {
    const [growth, topGroups, loanHealth, welfareStats] = await Promise.all([
      ...
      `),
      db.query(`
        -- LATERAL per child table, not a flat multi-table LEFT JOIN: the flat
        -- join fanned every contribution row out across every loan row (and
        -- vice versa) before the SUMs ran — the same bug class PR #105 fixed
        -- elsewhere (getGroupById, listGroups). Proven live: THE FIONA'S
        -- showed contributions 20x and loan_book 30x true values, and the
        -- inflated numbers even swapped the #1/#2 ranking on this chart
        -- (docs/audits/optimization-2026-09).
        SELECT
          g.id, g.name, g.type AS group_type,
          COALESCE(mem.members, 0)       AS members,
          COALESCE(con.contributions, 0) AS contributions,
          COALESCE(ln.loan_book, 0)      AS loan_book
        FROM public.groups g
        LEFT JOIN LATERAL (
          SELECT COUNT(*) AS members FROM public.group_members gm
          WHERE gm.group_id = g.id AND gm.status = 'active'
        ) mem ON true
        LEFT JOIN LATERAL (
          SELECT COALESCE(SUM(c.amount) FILTER (WHERE c.status = 'completed'), 0) AS contributions
          FROM public.contributions c WHERE c.group_id = g.id
        ) con ON true
        LEFT JOIN LATERAL (
          SELECT COALESCE(SUM(l.principal_amount) FILTER (WHERE l.status IN ('active','disbursed')), 0) AS loan_book
          FROM public.loans l WHERE l.group_id = g.id
        ) ln ON true
