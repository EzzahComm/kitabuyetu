...
function validateSchedule(schedule: FineSchedule): void {
  ...
  if (entries.length === 0)
    throw new ValidationError(
      "Fine schedule must have at least one offence category",
    );
  if (entries.length > 50)
    throw new ValidationError(
      "Fine schedule cannot have more than 50 categories",
    );
  for (const [category, amount] of entries) {
    if (!category.trim())
      throw new ValidationError("Offence category names cannot be blank");
    if (
...
export const finePolicyService = {
  ...
  async getPlatformSchedule(
    client: PoolClient,
  ): Promise<EffectiveFineSchedule> {
    const resolved = await resolvePolicyDetailed<FineSchedule>(
      client,
      DOMAIN,
      POLICY_KEY,
