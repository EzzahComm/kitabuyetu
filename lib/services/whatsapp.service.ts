...
export const whatsappService = {
  ...
  async send(
    ...
    return withTransaction(ctx, async (client) => {
      ...
      if (memberId) {
        const member = await fetchGroupMemberWithPhone(
          client,
          ctx.groupId,
          memberId,
        );
  ...
  async list(ctx: TenantContext, params: WhatsAppQueryInput) {
    return withDb(ctx, async (client) => {
      ...
      if (params.status) {
        conds.push(
          `w.status    = $${vals.length + 1}::whatsapp_message_status`,
        );
        vals.push(params.status);
      }
      if (params.direction) {
        conds.push(
          `w.direction = $${vals.length + 1}::whatsapp_message_direction`,
        );
      ...
      const [{ rows: cnt }, { rows: items }] = await Promise.all([
        client.query<{ count: string }>(
          `SELECT COUNT(*) AS count FROM whatsapp_messages w WHERE ${where}`,
          vals,
        ),
...
async function fetchGroupMemberWithPhone(
  ...
  if (!rows[0].phone)
    throw new ValidationError("Member has no phone number on file");
  return rows[0];
}
