...
export async function getUsageAnalytics(
  groupId: string,
): Promise<SmsUsageAnalytics> {
  return withAdminDb(async (db) => {
    const [balance, purchased, consumption, byFeature, byCampaign, rate] =
      await Promise.all([
    ...
    const allowanceLeft = Math.max(
      Number(balance.rows[0]?.allowance_included ?? 0) -
        Number(balance.rows[0]?.allowance_used ?? 0),
      0,
    );
    ...
    return {
      ...
      hasUnattributedHistory: features.some(
        (f) => f.feature === null && f.messages > 0,
      ),
    };
  });
}
