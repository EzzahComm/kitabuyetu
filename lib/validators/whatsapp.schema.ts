...
export const WHATSAPP_STATUSES = ['pending', 'sent', 'delivered', 'read', 'failed', 'dry_run'] as const;
...
export const SendWhatsAppMessageSchema = z
  ...
  .refine((v) => Boolean(v.memberId) || Boolean(v.toPhone), {
    path: ['memberId'],
    message: 'Provide either memberId or toPhone',
  })
  .refine((v) => !v.toPhone || isValidKenyanPhone(v.toPhone), {
    path: ['toPhone'],
    message: 'toPhone must be a valid Kenyan number',
  });
