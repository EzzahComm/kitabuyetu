...
export const MEMBER_VISIBILITIES = [
  "pseudonymous",
  "aggregate",
  "identified",
] as const;
...
export const WATERFALL_COMPONENTS = [
  "penalty",
  "interest",
  "principal",
] as const;
...
export const RepaymentWaterfallSchema = z.object({
  order: z
    ...
    .refine(
      ...
      "waterfall order cannot repeat a component",
    ),
  revenue_split: z
    .record(
      z
        ...
        .refine(
          (s) => Math.abs(s.organization + s.group - 1) < 1e-9,
          "each revenue_split must sum to exactly 1.0",
        ),
    )
    .optional(),
  ...
  .superRefine((v, ctx) => {
    ...
    if (v.isRepayable) {
      ...
      if (v.tenorMonths === undefined) {
        ctx.addIssue({
          code: "custom",
          path: ["tenorMonths"],
          message: "A repayable product needs a tenor",
        });
      }
      if (!v.interestMethod) {
        ctx.addIssue({
      ...
      if (v.tenorMonths !== undefined) {
        ctx.addIssue({
          code: "custom",
          path: ["tenorMonths"],
          message: "A non-repayable product cannot have a tenor",
        });
      }
      if (v.repaymentFrequency && v.repaymentFrequency !== "none") {
        ctx.addIssue({
    ...
    if (v.revenueOwner !== "shared" && v.revenueShareRatio !== undefined) {
      ctx.addIssue({
        ...
        path: ["revenueShareRatio"],
        message:
          "revenueShareRatio is only valid when revenueOwner is 'shared'",
      });
    }
...
export const ProgramActionSchema = z.discriminatedUnion("action", [
  z.object({
    action: z.literal("capitalize"),
    ...CapitalAdjustmentSchema.shape,
  }),
  z.object({
    action: z.literal("decapitalize"),
    ...CapitalAdjustmentSchema.shape,
  }),
]);
...
export type UpdateProgramStatusInput = z.infer<
  typeof UpdateProgramStatusSchema
>;
