...
function normaliseDate(input: unknown): unknown {
  ...
  if (dmyMatch) {
    ...
    const [day, month] = a > 12 && b <= 12 ? [a, b] : b > 12 && a <= 12 ? [b, a] : [a, b]; // DD/MM default
...
const blankableDate = z.preprocess(normaliseDate, z.string().date('Date must be YYYY-MM-DD or DD/MM/YYYY').optional());

const requiredDate = (fieldName: string) =>
  z.preprocess(normaliseDate, z.string().date(`${fieldName} must be a date in YYYY-MM-DD or DD/MM/YYYY format`));
...
export function resolveHeaderFor(kind: ImportKind, header: string): string | null {
...
export const ImportJobStatus = z.enum(['previewed', 'committed', 'cancelled', 'rolled_back', 'failed']);
