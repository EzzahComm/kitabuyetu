...
function normaliseDate(input: unknown): unknown {
  ...
  if (dmyMatch) {
    ...
    const [day, month] =
      a > 12 && b <= 12 ? [a, b] : b > 12 && a <= 12 ? [b, a] : [a, b]; // DD/MM default
  ...
  if (dmy2) {
    ...
    const [day, month] =
      a > 12 && b <= 12 ? [a, b] : b > 12 && a <= 12 ? [b, a] : [a, b];
...
const blankableDate = z.preprocess(
  normaliseDate,
...
const requiredDate = (fieldName: string) =>
  z.preprocess(
    ...
    z
      .string()
      .date(`${fieldName} must be a date in YYYY-MM-DD or DD/MM/YYYY format`),
  );
...
const LOAN_HISTORICAL_STATUSES = [
  "active",
  "completed",
  "defaulted",
  "written_off",
...
export const LoanCsvRowSchema = z.object({
  ...
  principal_amount: z.coerce
    .number()
    .positive("principal_amount must be greater than zero"),
  interest_rate: z.coerce
    .number()
    .nonnegative("interest_rate must be ≥ 0")
    .max(200, "interest_rate is unrealistically high"),
  term_months: z.coerce
    .number()
    .int()
    .positive("term_months must be a positive integer"),
  ...
  interest_method: z.preprocess(
    (v) => {
      ...
      const s = v
        ...
        .toLowerCase()
        .replace(/[\s-]+/g, "_");
      if (s === "reducing" || s === "declining" || s === "declining_balance")
...
export function resolveHeaderFor(
  ...
  switch (kind) {
    case "members":
      return MEMBER_HEADER_ALIASES[norm] ?? null;
    case "contributions":
      return CONTRIBUTION_HEADER_ALIASES[norm] ?? null;
    case "loans":
      return LOAN_HEADER_ALIASES[norm] ?? null;
  }
}
...
export function resolveCsvHeader(header: string): MemberCsvColumn | null {
  return (MEMBER_HEADER_ALIASES[normaliseCsvHeader(header)] ??
    null) as MemberCsvColumn | null;
}
...
const blankableEmail = z.preprocess(
  (v) => (typeof v === "string" && v.trim() === "" ? undefined : v),
