...
export const CreateContributionSplitSchema =
  baseRuleShape.superRefine(exclusiveAmountCheck);
 ...
 */
export const ReplaceContributionSplitsSchema = z
  .object({
  ...
  .superRefine((data, ctx) => {
    ...
    if (totalPct > 100.01) {
      // 0.01 fuzz for float accumulation
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export type CreateContributionSplitInput = z.infer<
  typeof CreateContributionSplitSchema
>;
export type UpdateContributionSplitInput = z.infer<
  typeof UpdateContributionSplitSchema
>;
export type ReplaceContributionSplitsInput = z.infer<
  typeof ReplaceContributionSplitsSchema
>;
