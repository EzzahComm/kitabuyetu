...
export function requireOneOf(callerRole: AnyRole, allowed: AnyRole[]): void {
  if (!allowed.includes(callerRole)) {
    throw new ForbiddenError(`Role '${callerRole}' is not permitted. Allowed: ${allowed.join(', ')}`);
  }
}

export const ROLES = {
  canManageMembers: (role: AnyRole) => hasRole(role, 'secretary'),

  canManageFinances: (role: AnyRole) => hasRole(role, 'treasurer'),

  canApproveLoan: (role: AnyRole) => hasRole(role, 'treasurer'),

  canPostJournal: (role: AnyRole) => hasRole(role, 'treasurer'),

  canManageSubscription: (role: AnyRole) => hasRole(role, 'chairperson'),

  canViewPII: (role: AnyRole) => hasRole(role, 'treasurer') || role === 'super_admin',

  canAdminGroup: (role: AnyRole) => hasRole(role, 'chairperson'),

  isSuperAdmin: (role: AnyRole) => role === 'super_admin',

  isOrganizationCoordinator: (role: AnyRole) => role === 'organization_coordinator',
};
