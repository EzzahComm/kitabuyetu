...
export function signBackofficeAccessToken(
  payload: BackofficeAccessTokenPayload,
): string {
...
export function refreshTtlSeconds(audience: TokenAudience = "tenant"): number {
  return ttlToSeconds(
    audience === "backoffice" ? BACKOFFICE_REFRESH_TTL : REFRESH_TTL,
  );
}
...
export function signMfaChallenge(
  payload: Omit<MfaChallengePayload, "aud">,
): string {
  return jwt.sign(
...
export function verifyMfaChallenge(
  token: string,
): MfaChallengePayload & { iat: number; exp: number } {
