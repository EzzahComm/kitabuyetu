...
import type {
  ...
  UpgradePlanInput,
} from "@/lib/validators/billing.schema";
import type {
  DepositPayload,
...
export const authApi = {
  ...
  verifyStart: (channel: "email" | "sms") =>
    api.post<{ channel: "email" | "sms"; expiresAt: string }>(
      "/auth/verify/start",
      { channel },
    ),
  ...
  verifyEmailToken: (token: string) =>
    api.post<{ status: string; groupId: string }>("/auth/verify/email", {
      token,
    }),
  ...
  adminForgotPasswordStart: (email: string) =>
    api.post<{ status: string }>("/auth/admin/forgot-password/start", {
      email,
    }),
...
export const orgInvitationApi = {
  lookup: (token: string) =>
    api.post<OrgInvitationLookup>("/organization-invitations/lookup", {
      token,
    }),

  confirmEmail: (token: string) =>
    api.post<{ phone: string }>("/organization-invitations/confirm-email", {
      token,
    }),

  verifyOtp: (token: string, otp: string) =>
    api.post<{ status: string }>("/organization-invitations/verify-otp", {
      token,
      otp,
    }),

  complete: (token: string, password: string) =>
    api.post<{ status: string }>("/organization-invitations/complete", {
      token,
      password,
    }),

  decline: (token: string) =>
    api.post<{ status: string }>("/organization-invitations/decline", {
      token,
    }),
};
...
export const meApi = {
  ...
  passbook: (params?: Partial<MemberPassbookQueryInput>) =>
    api.get<PaginatedResult<PassbookEntry>>(
      `/me/passbook${buildQuery(params ?? {})}`,
    ),
...
export const membersApi = {
  list: (params?: Record<string, unknown>) =>
    api.get<PaginatedResult<GroupMemberRow>>(
      `/members${buildQuery(params ?? {})}`,
    ),
  ...
  transitionStatus: (
    id: string,
    status: MemberStatusTransitionInput["status"],
    reason?: string,
...
export const contributionsApi = {
  list: (params?: Record<string, unknown>) =>
    api.get<PaginatedResult<Contribution & { member_name: string }>>(
      `/contributions${buildQuery(params ?? {})}`,
    ),
  getById: (id: string) =>
...
export const loansApi = {
  list: (params?: Record<string, unknown>) =>
    api.get<PaginatedResult<Loan & { member_name: string }>>(
  ...
  upcomingRepayments: (limit = 5) =>
    api.get<(LoanRepayment & { member_name: string })[]>(
      `/loans/upcoming-repayments?limit=${limit}`,
    ),
};
...
export const accountingApi = {
  ...
  journals: (params?: Record<string, unknown>) =>
    api.get<PaginatedResult<JournalEntry>>(
      `/accounting/journals${buildQuery(params ?? {})}`,
    ),
  ...
  profitAndLoss: (from: string, to: string) =>
    api.get<ProfitAndLoss>(
      `/accounting/reports?type=profit_and_loss&from=${from}&to=${to}`,
    ),
  balanceSheet: (asOf?: string) =>
    api.get<BalanceSheet>(
      `/accounting/reports?type=balance_sheet${asOf ? `&asOf=${asOf}` : ""}`,
    ),
  ...
  cashFlow: (from: string, to: string) =>
    api.get<CashFlowStatement>(
      `/accounting/reports?type=cash_flow&from=${from}&to=${to}`,
    ),
  equityChanges: (from: string, to: string) =>
    api.get<EquityChanges>(
      `/accounting/reports?type=equity_changes&from=${from}&to=${to}`,
    ),
  postingTemplates: () =>
...
export const smsApi = {
  ...
  // Bulk / Campaign
  bulk: (body: BulkSmsPayload) =>
    api.post<{ queued: number }>("/sms/bulk", body),
  campaigns: (params?: Record<string, unknown>) =>
    api.get<PaginatedResult<SmsCampaign>>(
      `/sms/campaign${buildQuery(params ?? {})}`,
    ),
  createCampaign: (body: CampaignCreatePayload) =>
    api.post<SmsCampaign>("/sms/campaign", body),
  cancelCampaign: (id: string) => api.delete<void>(`/sms/campaign?id=${id}`),
  // Templates
  templates: (params?: Record<string, unknown>) =>
    api.get<SmsTemplate[]>(`/sms/templates${buildQuery(params ?? {})}`),
  createTemplate: (body: TemplateCreatePayload) =>
    api.post<SmsTemplate>("/sms/templates", body),
  updateTemplate: (id: string, body: TemplateUpdatePayload) =>
    api.patch<SmsTemplate>(`/sms/templates?id=${id}`, body),
  ...
  createSchedule: (body: ScheduleCreatePayload) =>
    api.post<SmsSchedule>("/sms/schedules", body),
  ...
  // DLR
  dlr: (messageId: string) =>
    api.get<unknown>(`/sms/dlr?messageId=${messageId}`),
  ...
  setPreferences: (optedOut: boolean) =>
    api.put<{ optedOut: boolean }>("/sms/preferences", { optedOut }),
  ...
  failures: (params?: Record<string, unknown>) =>
    api.get<PaginatedResult<SmsFailure>>(
      `/sms/failures${buildQuery(params ?? {})}`,
    ),
  retryFailure: (id: string) =>
    api.post<{ status: string }>(`/sms/failures/${id}/retry`, {}),
  // Reminder/automation history, suppressed outcomes included — the DSAR view.
  reminderHistory: (params?: Record<string, unknown>) =>
    api.get<PaginatedResult<ReminderHistoryRow>>(
      `/sms/reminder-history${buildQuery(params ?? {})}`,
    ),
  ...
  removeOptOut: (phone: string) =>
    api.delete<{ optOuts: SmsOptOut[] }>(
      `/sms/opt-outs?phone=${encodeURIComponent(phone)}`,
    ),
...
export const billingApi = {
  ...
  upgradePlan: (
    planType: UpgradePlanInput["planType"],
    product?: SubscriptionProduct,
  ) =>
    api.post<SubscriptionPublic>("/billing/plans", {
      planType,
      ...(product ? { product } : {}),
    }),
  invoices: () => api.get<unknown[]>("/billing/invoices"),
  recordPayment: (body: RecordManualPaymentPayload) =>
    api.post<unknown>("/billing/payments", body),
  smsTopup: (amount: number) =>
    api.post<unknown>("/billing/payments", { type: "sms_topup", amount }),
};
...
export const mpesaApi = {
  stkPush: (body: StkPushInput) =>
    api.post<{ checkoutRequestId: string; message: string }>(
      "/mpesa/stk-push",
      body,
    ),
...
export const organizationApi = {
  ...
  groups: (params?: { page?: number; limit?: number }) =>
    adminApi.get<PaginatedResult<OrganizationGroupSummary>>(
      `/organization/groups${buildQuery(params ?? {})}`,
    ),
  detail: (groupId: string) =>
    adminApi.get<unknown>(`/organization/reports?groupId=${groupId}`),
  ...
  health: () =>
    adminApi.get<{ health: PortfolioHealth | null; incomplete: string[] }>(
      "/organization/health",
    ),
  ...
  deposit: (body: DepositPayload) =>
    adminApi.post<unknown>("/organization/wallet", body),
  ...
  programs: () =>
    adminApi.get<{ items: FundingProgram[] }>("/organization/programs"),
  createProgram: (body: CreateProgramPayload) =>
    adminApi.post<FundingProgram>("/organization/programs", body),
  ...
  budgetReport: () =>
    adminApi.get<{ items: ProgramBudgetLine[] }>(
      "/organization/programs?report=budget",
    ),
  donorSpendReport: () =>
    adminApi.get<{ items: DonorSpendLine[] }>(
      "/organization/programs?report=donor",
    ),
  ...
  members: (params?: { page?: number; limit?: number; search?: string }) =>
    adminApi.get<PaginatedResult<OrganizationMemberRow>>(
      `/organization/members${buildQuery(params ?? {})}`,
    ),
  ...
  auditLogs: (params?: { page?: number; limit?: number; search?: string }) =>
    adminApi.get<PaginatedResult<OrganizationAuditLogRow>>(
      `/organization/audit-logs${buildQuery(params ?? {})}`,
    ),
...
export const importApi = {
  upload: (type: string, file: File) => {
    ...
    fd.append("file", file);
    return api.upload<{ imported: number; errors: unknown[] }>(
      `/import?type=${type}`,
      fd,
    );
  },
