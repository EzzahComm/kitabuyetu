...
async function request<T>(
  ...
  const headers = {
    ...buildHeaders(body, options?.multipart),
    ...options?.headers,
  };
...
export const api = {
  ...
  upload: <T>(path: string, formData: FormData) =>
    request<T>("POST", path, formData, { multipart: true }),
  openBlob,
};
...
export const adminApi = {
  get: <T>(path: string) =>
    request<T>("GET", path, undefined, { base: ADMIN_BASE }),
  post: <T>(path: string, body: unknown) =>
    request<T>("POST", path, body, { base: ADMIN_BASE }),
  patch: <T>(path: string, body: unknown) =>
    request<T>("PATCH", path, body, { base: ADMIN_BASE }),
  put: <T>(path: string, body: unknown) =>
    request<T>("PUT", path, body, { base: ADMIN_BASE }),
  delete: <T>(path: string) =>
    request<T>("DELETE", path, undefined, { base: ADMIN_BASE }),
};
