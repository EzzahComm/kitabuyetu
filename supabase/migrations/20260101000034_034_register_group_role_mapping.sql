-- =============================================================================
-- 018_register_group_role_mapping.sql
--
-- Update register_group() so the creator's group_members.role reflects their
-- chosen creator_role (chairperson / secretary / treasurer), not a flat
-- 'group_admin' for everyone.
--
-- Mapping (matches ROLES.canManageMembers in lib/auth/rbac.ts — all three
-- creator types pass the secretary-or-higher gate and can onboard members):
--   chairperson → 'group_admin'  (full chair-level admin)
--   secretary   → 'secretary'    (member + meeting management)
--   treasurer   → 'treasurer'    (member + finance management)
--
-- Also adds 'group_role' to the JSONB return so the route can use it directly
-- when signing the access token instead of hardcoding 'group_admin'.
-- =============================================================================
...
BEGIN
  ...
  -- Map officer_role → member_role for the creator's group_members row.
  -- All three give canManageMembers; chairs additionally get full admin.
  ...
  INSERT INTO group_members (
    ...
    v_group_role,  -- ← was hardcoded 'group_admin'; now mapped from creator_role
    'active'
  );
