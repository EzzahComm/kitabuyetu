-- =============================================================================
-- 139: No free plan — lock unpaid groups out, and stop signup granting one
--
-- Completes the paid-only cutover started in migration 138. Two changes.
--
-- 1. register_group() no longer creates a subscription at all. It used to
--    insert starter/active at monthly_fee 0 for every new group -- that WAS
--    the free tier. A new group now holds no subscription until it pays.
--    Everything else the function does (group, founding member, membership,
--    billing account, 16-account chart of accounts) is unchanged; the body
--    below is the live definition with only that INSERT removed, so this is a
--    faithful replacement rather than a rewrite.
--
--    v_product is now assigned but unused. Kept deliberately: `product` stays
--    part of the payload contract and the Phase 4 Chama Reminder portal will
--    need it again.
--
-- 2. Existing free subscriptions are expired. These are the legacy rows the
--    old signup handed out: starter, monthly_fee 0, and no payment behind
--    them. The predicate is narrow on purpose -- payment_id IS NULL and
--    monthly_fee = 0 -- so nothing anyone has actually paid for is touched,
--    including negotiated enterprise deals activated by ops.
--
-- Enforcement itself is application-side (lib/auth/subscription-gate.ts),
-- which every tenant route reaches through withAuth. Sign-in, billing and the
-- M-Pesa endpoints are carved out so a locked group can still see what it
-- owes and pay: a lock that also blocks paying is just an outage.
--
...
-- The grants below restore exactly what the live function had -- service_role
...
-- enterprise deals, which also carry fee 0 and no payment_id and are
-- provisioned by ops through activatePlanWithoutPayment. monthly_fee=0 spares
-- a real paid starter (KES 150). payment_id IS NULL spares anything bought
-- through the M-Pesa path.
UPDATE subscriptions
SET    status        = 'expired',
       cancelled_at  = NOW(),
       cancel_reason = 'Free starter plan retired — all plans are now paid (migration 139)'
WHERE  status      = 'active'
