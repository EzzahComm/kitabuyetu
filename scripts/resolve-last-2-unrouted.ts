...
async function main() {
  ...
  if (allocate && !allocate.resolved) {
    ...
    console.log(
      `SKIP UF6QZ6QA8I — ${!allocate ? "not found" : "already resolved"}`,
    );
  }

  if (dismiss && !dismiss.resolved) {
    console.log(
      `${apply ? "DISMISSING" : "[DRY RUN] would dismiss"}: UETQZ5SNUZ (KES 1) as a test/probe payment`,
    );
    if (apply) {
    ...
    console.log(
      `SKIP UETQZ5SNUZ — ${!dismiss ? "not found" : "already resolved"}`,
    );
  }
